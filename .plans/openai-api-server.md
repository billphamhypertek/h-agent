# OpenAI-Compatible API Server for AETHER
...
SSE streaming. By exposing this endpoint, aether-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  aether-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.aether/.env`
2. Run `aether gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with aether-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns aether-agent as a model) |
...
Authorization: Bearer aether-api-key-here
...
But aether-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "aether-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured aether-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of aether-agent's core prompt
...
Once implemented, aether-agent works as a drop-in backend for:
