...
_AETHER_CORE_TOOLS = [
    ...
    "terminal", "process",
    # Read the desktop GUI's embedded terminal pane (gated on AETHER_DESKTOP
    ...
    # spawned as a kanban worker (AETHER_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `aether tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (AETHER_KANBAN_TASK env "
    ...
    # Coding posture (base AETHER — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in aether_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full AETHER toolsets (CLI + messaging platforms)
    #
    ...
    # the `aether send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "aether-cli": {
        ...
        "tools": _AETHER_CORE_TOOLS,
        "includes": []
    },

    "aether-cron": {
        # Mirrors aether-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `aether tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as aether-cli; gated by `aether tools`",
        "tools": _AETHER_CORE_TOOLS,
        "includes": []
    },
    ...
    "aether-email": {
        "description": "Email bot toolset - interact with AETHER via email (IMAP/SMTP)",
        "tools": _AETHER_CORE_TOOLS,
        "includes": []
    },
    ...
    "aether-sms": {
        "description": "SMS bot toolset - interact with AETHER via SMS (Twilio)",
        "tools": _AETHER_CORE_TOOLS,
        "includes": []
    },
    ...
    "aether-webhook": {
        ...
        "tools": _AETHER_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "aether-gateway": {
        ...
        "tools": [],
        "includes": ["aether-telegram", "aether-discord", "aether-whatsapp", "aether-slack", "aether-signal", "aether-bluebubbles", "aether-homeassistant", "aether-email", "aether-sms", "aether-mattermost", "aether-matrix", "aether-dingtalk", "aether-feishu", "aether-wecom", "aether-wecom-callback", "aether-weixin", "aether-qqbot", "aether-webhook", "aether-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``aether-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_AETHER_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``aether-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (aether-<name>).
        # Gives them _AETHER_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("aether-"):
            platform_name = name[len("aether-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_AETHER_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
