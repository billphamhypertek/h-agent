...
# Do not write .pyc files at runtime: /opt/aether is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/aether/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when aether
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# aether process, the dashboard, and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main aether process, the dashboard,
...
RUN set -eu; \
    ...
    # 'AETHER WebUI' catalog). The image has moved to s6-overlay /init\
...
# Non-root user for runtime; UID can be overridden via AETHER_UID at runtime
RUN useradd -u 10000 -m -d /opt/data aether
...
#
# ui-tui/packages/aether-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/aether-ink/ ui-tui/packages/aether-ink/
...
# lock on the @aether/ink entry, tripped the TUI launcher's
...
# for the same reason: it lazy-installs into /opt/aether/.venv at first
...
# ---------- Permissions ----------
# Link aether-agent itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Keep /opt/aether immutable for the runtime aether user. Hosted/container
...
# supervised AETHER processes drop to the non-root aether user.
USER root
RUN mkdir -p /opt/aether/bin && \
    cp /opt/aether/docker/aether-exec-shim.sh /opt/aether/bin/aether && \
    chmod 0755 /opt/aether/bin/aether && \
    printf 'docker\n' > /opt/aether/.install_method && \
    chown -R root:root /opt/aether && \
    chmod -R a+rX /opt/aether && \
    chmod -R a-w /opt/aether
# The ``.install_method`` stamp is baked next to the running code (the install
# tree), NOT into $AETHER_HOME. $AETHER_HOME (/opt/data) is a shared data
# volume that is commonly bind-mounted from the host and even shared with a
# host-side Desktop/CLI install; stamping it at boot used to clobber that
# host install's marker and wrongly block its ``aether update``. A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the aether user via
# `s6-setuidgid aether` in its run script. If AETHER_UID is unset, services
# run as the default aether user (UID 10000).

# ---------- Bake build-time git revision ----------
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `aether dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the AETHER_GIT_SHA build-arg to
# /opt/aether/.aether_build_sha at build time, and have
# aether_cli/build_info.py read it at runtime.  Both `aether dump` and
...
ARG AETHER_GIT_SHA=
RUN if [ -n "${AETHER_GIT_SHA}" ]; then \
        chmod u+w /opt/aether && \
        printf '%s\n' "${AETHER_GIT_SHA}" > /opt/aether/.aether_build_sha && \
        chmod a-w /opt/aether /opt/aether/.aether_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-aether + dashboard.
...
# `exec aether`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $AETHER_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/aether/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-aether-setup && \
    chmod +x /etc/cont-init.d/01-aether-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV AETHER_WEB_DIST=/opt/aether/aether_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/aether/ui-tui/dist/entry.js`) and skip the
...
ENV AETHER_TUI_DIR=/opt/aether/ui-tui
ENV AETHER_HOME=/opt/data
ENV AETHER_WRITE_SAFE_ROOT=/opt/data
ENV AETHER_DISABLE_LAZY_INSTALLS=1
# The published image seals /opt/aether (root-owned, read-only) so a runtime
...
# is seeded + chowned to the aether user by docker/stage2-hook.sh and lives
...
ENV AETHER_LAZY_INSTALL_TARGET=/opt/data/lazy-packages

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> aether ...` they default to root, and any file the
# command writes under $AETHER_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/aether/bin/aether, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid aether`
# when invoked as root. Non-root callers (supervised processes,
# `--user aether`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/aether/.venv/bin/aether). See the shim source for
# the opt-out env var (AETHER_DOCKER_EXEC_AS_ROOT=1).

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> aether ...` and any
# subprocess that doesn't activate the venv first still find aether.
#
# /opt/aether/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/aether/bin:/opt/aether/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. aether
# subcommand vs. no-args), drops to the aether user via s6-setuidgid,
# and exec's the final program so its exit code becomes the container
# exit code. Without the wrapper-as-ENTRYPOINT, leading-dash args
# like `--version` would be intercepted by /init's POSIX shell.
ENTRYPOINT [ "/init", "/opt/aether/docker/main-wrapper.sh" ]
CMD [ ]
