...
Plugins built on AETHER often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing AETHER
...
:class:`~aether_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors AETHER' main config
...
handles every provider, fallback chain, and per-task override AETHER
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`aether_cli.plugins.PluginContext`
