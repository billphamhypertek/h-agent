...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into AETHER with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.aether/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of AETHER
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``aether tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``aether setup`` for
