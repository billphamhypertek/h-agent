"""Lightweight internationalization (i18n) for AETHER static user-facing messages.

Scope (thin slice, by design): only the highest-impact static strings shown
to the user by AETHER itself -- approval prompts, a handful of gateway slash
...
Language resolution order:
    ...
    2. ``AETHER_LANGUAGE`` environment variable (for tests / quick override)
...
def _locales_dir() -> Path:
    ...
    1. ``AETHER_BUNDLED_LOCALES`` env var -- set by the Nix wrapper (or any
    ...
    """
    override = os.getenv("AETHER_BUNDLED_LOCALES", "").strip()
    if override:
        candidate = Path(override)
        if candidate.is_dir():
            return candidate
        logger.warning(
            "AETHER_BUNDLED_LOCALES points to a non-directory path (%s); "
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already a aether dependency
...
def reset_language_cache() -> None:
    ...
    Call after :func:`aether_cli.config.save_config` if a running process
...
def get_language() -> str:
    ...
    env_lang = os.environ.get("AETHER_LANGUAGE")
    if env_lang:
