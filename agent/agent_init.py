...
from aether_cli.timeouts import get_provider_request_timeout
...
def _build_codex_gpt55_autoraise_notice(autoraise: Dict[str, float]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: aether config set compression.codex_gpt55_autoraise false"
    )
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .aether.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / AETHER_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.aether/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from aether_cli.config import load_config as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind AETHER_DEV_CREDITS.
    ...
    # both live under ~/.aether/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from aether_logging import setup_logging, setup_verbose_logging
    setup_logging(aether_home=_ra()._aether_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that aether_logging.setup_logging() does not
        ...
        # handler level inside aether_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from aether_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from aether_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `aether model` to "
                        "select a provider, or run `aether setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff AETHER_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["AETHER_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.aether/sessions/ alongside gateway sessions
    aether_home = get_aether_home()
    agent.logs_dir = aether_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.aether/sessions/session_{sid}.json)
    ...
    try:
        from aether_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from aether_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "aether"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace AETHER' built-in
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from aether_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from aether_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by AETHER.  Choose a model with at least "
