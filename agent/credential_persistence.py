...
``auth.json``.  They intentionally have no dependency on ``aether_cli.auth`` so
...
# Sources AETHER owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and AETHER-owned OAuth/device-code state)
