...
upstream prefix cache warm.  See ``aether-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  AETHER never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want AETHER_HOME persona while keeping
    ...
    # Pointer to the aether-agent skill + docs for user questions about AETHER itself.
    stable_parts.append(AETHER_AGENT_HELP_GUIDANCE)
    ...
    # AETHER_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base AETHER, any interactive coding surface in a code
    ...
    # Active-profile hint — names the AETHER profile the agent is running
    # under so it doesn't conflate ~/.aether/skills/ (default profile) with
    # ~/.aether/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active AETHER profile: default. Other profiles (if any) live "
            "under ~/.aether/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active AETHER profile: {active_profile}. This session reads "
            f"and writes ~/.aether/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.aether/skills/, ~/.aether/plugins/, "
            f"~/.aether/cron/, ~/.aether/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — AETHER never rebuilds or reinjects parts of it
