...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_AETHER_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, AETHER expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``aether_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_aether_home()`` themselves.
        """
        if "aether_home" not in kwargs:
            from aether_constants import get_aether_home
            kwargs["aether_home"] = str(get_aether_home())
        for provider in self._providers:
            try:
