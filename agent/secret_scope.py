...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the AETHER_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    ...
    "AETHER_HOME", "AETHER_PROFILE", "AETHER_GATEWAY_LOCK_DIR",
    "AETHER_MAX_ITERATIONS", "AETHER_MAX_TOKENS", "AETHER_API_TIMEOUT",
    "AETHER_REDACT_SECRETS", "AETHER_NOUS_TIMEOUT_SECONDS",
    "_AETHER_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "AETHER_KANBAN_DB", "AETHER_KANBAN_WORKSPACES_ROOT", "AETHER_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "AETHER_KANBAN_",
    "AETHER_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(aether_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(aether_home) / ".env")
