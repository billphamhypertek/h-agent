...
as ``kind: backend``) or ``~/.aether/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``aether tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$AETHER_HOME/cache/videos/``, creating parents as needed."""
    from aether_constants import get_aether_home

    path = get_aether_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$AETHER_HOME/cache/videos/``.
