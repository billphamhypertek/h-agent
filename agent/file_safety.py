...
def _aether_home_path() -> Path:
    """Resolve the active AETHER_HOME (profile-aware) without circular imports."""
    try:
        from aether_constants import get_aether_home  # local import to avoid cycles
        return get_aether_home()
    except Exception:
        return Path(os.path.expanduser("~/.aether"))


def _aether_root_path() -> Path:
    """Resolve the AETHER root dir (always the parent of any profile, never per-profile)."""
    try:
        from aether_constants import get_default_aether_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    aether_home = _aether_home_path()
    aether_root = _aether_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(aether_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(aether_root / ".anthropic_oauth.json"),
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved AETHER_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("AETHER_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    aether_dirs = []
    for base in (_aether_home_path(), _aether_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in aether_dirs:
                aether_dirs.append(real)
        except Exception:
            continue

    for base_real in aether_dirs:
        try:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied AETHER path.

    Three categories are blocked:

      * Internal AETHER cache files under ``AETHER_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under AETHER_HOME and the global AETHER
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.aether/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active AETHER_HOME (profile-aware) AND the global
    # AETHER root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (AETHER_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    aether_dirs: list[Path] = []
    for base in (_aether_home_path(), _aether_root_path()):
        try:
            real = base.resolve()
            if real not in aether_dirs:
    ...
    for hd in aether_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal AETHER cache file "
    ...
    for hd in aether_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a AETHER credential store "
    ...
    for hd in aether_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the AETHER MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a AETHER MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# AETHER profiles are separate AETHER_HOME dirs under
...
#
# Reference: May 2026 incident where a aether-security profile session
# edited skills under both ``~/.aether/profiles/aether-security/skills/``
# AND ``~/.aether/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under AETHER_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from AETHER_HOME.
    ...
    """
    try:
        home_real = _aether_home_path().resolve()
        root_real = _aether_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside AETHER scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    AETHER entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to AETHER profile {info['target_profile']!r}, but the "
...
#
#   <AETHER_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.aether/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.aether/…`` segment and warns
# regardless of which AETHER profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.aether/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.aether`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.aether/…`` and returns the
    index where the inner AETHER-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .aether / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".aether":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative AETHER state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.aether``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.aether`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any AETHER resolver to
    succeed, so it works correctly even when called from contexts where
    AETHER_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host AETHER process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real AETHER_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.aether/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.aether/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host AETHER_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative AETHER state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host AETHER "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real AETHER_HOME. Use the "
