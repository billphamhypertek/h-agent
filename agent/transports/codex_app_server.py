...
projection into AETHER' display, approval bridging, transcript projection into
...
"codex_app_server"`. AETHER' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        if spawn_env.get("AETHER_KANBAN_TASK"):
            kanban_db = spawn_env.get("AETHER_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "AETHER_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("AETHER_HOME", os.path.expanduser("~/.aether")),
                        "kanban",
                    ),
                )
            )
