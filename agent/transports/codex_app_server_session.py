...
Owns one Codex thread per AETHER session. Drives `turn/start`, consumes
...
# AETHER' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_AETHER_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse AETHER/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per AETHER session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _AETHER_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("AETHER_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="aether",
            client_title="AETHER",
            client_version=_get_aether_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but AETHER' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into AETHER' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own aether-tools callback we
            # auto-accept — the user already approved AETHER' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map AETHER approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    AETHER returns 'once', 'session', 'always', or 'deny'.
...
def _get_aether_version() -> str:
    """Best-effort AETHER version string for codex's userAgent line."""
    try:
