...
row taxonomy is inferred from the atlas shape so AETHER can render both legacy
...
# rows (1536x1872), while older AETHER/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are AETHER' activity state names. They are not always identical to the
...
# Legacy AETHER/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical AETHER activity names -> accepted row-name aliases in descending
