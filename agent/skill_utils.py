...
from aether_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``AETHER_KANBAN_TASK`` / ``AETHER_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("AETHER_KANBAN_TASK") or os.getenv("AETHER_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The AETHER Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``aether_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``AETHER_PLATFORM`` or
            ``AETHER_SESSION_PLATFORM`` env vars.  Returns the global
...
# which becomes the dominant cost of ``aether`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.aether/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``aether`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against AETHER_HOME, not cwd
        if not p.is_absolute():
            p = (aether_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.aether/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    aether = metadata.get("aether") or {}
    if not isinstance(aether, dict):
        aether = {}
    return {
        "fallback_for_toolsets": aether.get("fallback_for_toolsets", []),
        "requires_toolsets": aether.get("requires_toolsets", []),
        "fallback_for_tools": aether.get("fallback_for_tools", []),
        "requires_tools": aether.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    aether = metadata.get("aether")
    if not isinstance(aether, dict):
        return []
    raw = aether.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes AETHER metadata, VCS, virtualenv/dependency, cache, and skill
