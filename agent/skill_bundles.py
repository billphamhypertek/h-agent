...
Storage
-------
Bundles live in ``~/.aether/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``AETHER_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``aether bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under AETHER_HOME.

    Honors ``AETHER_BUNDLES_DIR`` for tests; falls back to
    ``<AETHER_HOME>/skill-bundles``.
    """
    override = os.environ.get("AETHER_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_aether_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `aether bundles` CLI subcommand.
# ---------------------------------------------------------------------------
