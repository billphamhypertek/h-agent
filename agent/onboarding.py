...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time AETHER starts and finds ``~/.openclaw/``.

    Points users at ``aether claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``aether claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to AETHER, run "
        "`aether claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `aether claw cleanup` (renames it to ~/.openclaw.pre-migration — "
