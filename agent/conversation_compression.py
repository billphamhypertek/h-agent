    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression will drop middle turns without a summary. "
                    "Run `aether setup` or set OPENROUTER_API_KEY."
                )
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by AETHER "
...
def compress_context(
    ...
    *,
    ...
    # long-lived ``aether_state.SessionDB`` class still bound to the
        ...
        try:
            ...
            if getattr(agent, "_last_compression_lock_error_sid", None) != _lock_sid:
                ...
                logger.warning(
                    ...
                    "process (or `aether update`) to resync.",
    ...
    if agent._session_db:
        try:
            ...
            if in_place:
                ...
                # from ``aether_logging._session_context`` (set once per turn in
                ...
                try:
                    agent._session_db.create_session(
                        ...
                        source=agent.platform or os.environ.get("AETHER_SESSION_SOURCE", "cli"),
                        model=agent.model,
                ...
                try:
                    from aether_cli.goals import migrate_goal_to_session
    ...
    # engines (e.g. aether-lcm) use boundary_reason="compression" to preserve
    # DAG lineage / checkpoint per-session state across the boundary instead of
    # re-initializing fresh. See aether-lcm#68. Built-in ContextCompressor
    ...
    def _shrink_data_url(url: str) -> tuple:
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="aether_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
