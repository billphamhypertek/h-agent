...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `aether backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - aether_home (str): The active AETHER_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.aether``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "aether").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'aether memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], aether_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'aether memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``aether_home`` is the active AETHER_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE AETHER_HOME.

        ``aether backup`` only walks AETHER_HOME, so any provider state kept
        ...
        of the archive, and ``aether import`` restores them to their original
