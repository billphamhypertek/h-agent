...
def _codex_note_to_tool_progress(note: dict) -> tuple[str, str, dict] | None:
    """Map a Codex app-server ``item/started`` notification to a AETHER
    ...
    surfaced them as AETHER tool-progress events — so gateways (Telegram, etc.)
...
def _record_codex_app_server_usage(agent, turn) -> dict[str, Any]:
    """Translate Codex app-server token usage into AETHER accounting.

    Codex app-server reports usage via thread/tokenUsage/updated as:
    inputTokens, cachedInputTokens, outputTokens, reasoningOutputTokens,
    totalTokens.

    AETHER' canonical prompt bucket includes uncached input + cached input.
    The Codex app-server protocol does not currently expose cache-write tokens,
    so that bucket remains zero on this runtime.

    Even when Codex omits usage for a turn, AETHER should still count that turn
...
def run_codex_app_server_turn(
    ...
    *,
    ...
    app-server` subprocess and projects its events back into AETHER'
    ...
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        ...
        # Approval callback: defer to AETHER' standard prompt flow if a
        ...
        def _on_codex_event(note: dict) -> None:
            # Bridge Codex app-server item/started notifications to AETHER
