#!/usr/bin/env python3
"""Live test harness for AETHER's Tool Search feature.
...
# Force-isolate the test environment BEFORE any aether imports.
ORIGINAL_HOME = os.environ.get("AETHER_HOME")
ORIGINAL_AUTH = Path.home() / ".aether" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.aether/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.aether/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="aether_ts_live_"))
    aether_home = home_dir / ".aether"
    aether_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, aether_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".aether" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, aether_home / ".env")
        ...
        load_aether_dotenv(aether_home=str(Path.home() / ".aether"))
    ...
    (aether_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return aether_home
...
def reset_module_state():
    """Drop cached modules so the new AETHER_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "aether_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original AETHER_HOME
    if ORIGINAL_HOME is not None:
        os.environ["AETHER_HOME"] = ORIGINAL_HOME
    else:
