...
# Python-driven tool session (e.g. AETHER terminal tool). A pre-set PYTHONPATH
...
#   code at /usr/local/lib/aether-agent, command at /usr/local/bin/aether,
#   data still at /root/.aether (AETHER_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$AETHER_HOME/.no-bundled-skills so future"
            echo "                   'aether update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> AETHER.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.aether/aether-agent"
            echo "                   default (root, Linux): /usr/local/lib/aether-agent"
            echo "  --aether-home PATH  Data directory (default: ~/.aether, or \$AETHER_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, AETHER installs the code under"
            echo "  /usr/local/lib/aether-agent and links the command into"
            echo "  /usr/local/bin/aether (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$AETHER_HOME"
            echo "  (default /root/.aether).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$AETHER_HOME/aether-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs aether setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# intentional, but they leave the checkout dirty — which forces `aether update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (AETHER-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download AETHER","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install aether command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `aether` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $AETHER_HOME/aether-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $AETHER_HOME/aether-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/aether-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $AETHER_HOME/aether-agent — then preserve it)
#
# Always no-op when the user set --dir or $AETHER_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in AETHER_HOME.
    if is_termux; then
        INSTALL_DIR="$AETHER_HOME/aether-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$AETHER_HOME/aether-agent/.git" ]; then
            INSTALL_DIR="$AETHER_HOME/aether-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/aether-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/aether-agent"
        ...
        # /usr/local/bin/aether wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/aether"
        log_info "  Data:    $AETHER_HOME (unchanged)"
...
# Point a AETHER-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $AETHER_HOME/node/bin — which is NOT on
...
# AETHER's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no AETHER-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$AETHER_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$AETHER_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$AETHER_HOME/node/etc/npmrc"
}

get_aether_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/aether" ]; then
        echo "$link_dir/aether"
    else
        echo "aether"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://aether.hypertek.vn/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # AETHER owns its own uv at $AETHER_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (aether_cli/managed_uv.py) looks in the same
    # place, so install.sh and `aether update` stay in sync.
    local _managed_uv="$AETHER_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $AETHER_HOME/bin ..."
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/aether-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/aether-uv-installer.$$.sh")"
    ...
    # directly into $AETHER_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$AETHER_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the AETHER-
...
check_node() {
    ...
    # Repair pre-existing AETHER-managed installs where `npm install -g` lands
    ...
    # Prefer a AETHER-managed Node from a previous run over a too-old system one.
    if [ -x "$AETHER_HOME/node/bin/node" ] && node_satisfies_build "$("$AETHER_HOME/node/bin/node" --version)"; then
        export PATH="$AETHER_HOME/node/bin:$PATH"
        log_success "Node.js $("$AETHER_HOME/node/bin/node" --version) found (AETHER-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing AETHER-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.aether/node/..."
    ...
    # Place into ~/.aether/node/ and symlink binaries into the same bin dir
    # the aether command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$AETHER_HOME/node"
    mkdir -p "$AETHER_HOME"
    mv "$extracted_dir" "$AETHER_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$AETHER_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$AETHER_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$AETHER_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$AETHER_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.aether/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. AETHER install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "AETHER itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `aether update` path
                # (#4735).
                ...
                local stash_name
                stash_name="aether-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if AETHER behaves unexpectedly."
                    else
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"aether-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    log_info "Setting up aether command..."

    if [ "$USE_VENV" = true ]; then
        AETHER_BIN="$INSTALL_DIR/venv/bin/aether"
    else
        AETHER_BIN="$(which hermes 2>/dev/null || echo "")"
        if [ -z "$AETHER_BIN" ]; then
            log_warn "aether not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$AETHER_BIN" ]; then
        log_warn "aether entry point not found at $AETHER_BIN"
    ...
    # Create a user-facing shim for the aether command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $AETHER_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$AETHER_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/aether"
    cat > "$command_link_dir/aether" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$AETHER_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/aether"
    log_success "Installed aether launcher → $command_link_display_dir/aether"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "aether command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v aether' \
                >/dev/null 2>&1; then
        ...
        log_info "aether not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# AETHER — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# AETHER — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# AETHER — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so aether works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.aether directory structure (config at top level, code in subdir)
    mkdir -p "$AETHER_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.aether/.env (top level, easy to find)
    if [ ! -f "$AETHER_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$AETHER_HOME/.env"
            log_success "Created ~/.aether/.env from template"
        else
            touch "$AETHER_HOME/.env"
            log_success "Created ~/.aether/.env"
        fi
    else
        log_info "~/.aether/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$AETHER_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.aether/config.yaml (top level, easy to find)
    if [ ! -f "$AETHER_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$AETHER_HOME/config.yaml"
            log_success "Created ~/.aether/config.yaml from template"
        fi
    else
        log_info "~/.aether/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$AETHER_HOME/SOUL.md" ]; then
        cat > "$AETHER_HOME/SOUL.md" << 'SOUL_EOF'
# AETHER Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how AETHER communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.aether/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.aether/"

    # Seed bundled skills into ~/.aether/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `aether update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'aether update'." \
            > "$AETHER_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $AETHER_HOME/.no-bundled-skills"
        log_info "  Future 'aether update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.aether/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.aether/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$AETHER_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$AETHER_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.aether/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$AETHER_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# AETHER browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "AETHER will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# AETHER browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (aether --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `aether update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'aether setup' after install."
        return 0
    fi
    ...
    # Run aether setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m aether_cli.main setup < /dev/tty
    else
        python -m aether_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$AETHER_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for AETHER to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$AETHER_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'aether whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                AETHER_CMD="$(get_aether_command_path)"
                $AETHER_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'aether whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'aether gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $AETHER_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $AETHER_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: aether gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: aether gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $AETHER_CMD gateway > "$AETHER_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.aether/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: aether gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: aether gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $AETHER_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $AETHER_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $AETHER_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}aether${NC}              Start chatting"
    echo -e "   ${GREEN}aether setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}aether config${NC}       View/edit configuration"
    echo -e "   ${GREEN}aether config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}aether gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}aether update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'aether' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'aether' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'aether' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$AETHER_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$AETHER_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$AETHER_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$AETHER_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$AETHER_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up AETHER for pip install"
    ...
    AETHER_CMD="$(command -v hermes 2>/dev/null || echo "")"
    if [ -n "$AETHER_CMD" ]; then
        log_info "Running aether setup..."
        "$AETHER_CMD" setup
    else
        log_warn "aether command not found on PATH"
        log_info "Try: python -m aether_cli.main setup"
    fi
}
...
# extract a tree MISSING the electron binary, so the `electron`->`AETHER` rename
...
# _purge_electron_build_cache() used by `aether desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the AETHER-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `aether desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/AETHER" ]; then
            app="$desktop_dir/release/linux-unpacked/AETHER"
        elif [ -x "$desktop_dir/release/linux-unpacked/aether" ]; then
            app="$desktop_dir/release/linux-unpacked/aether"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/AETHER.app" \
            "$desktop_dir/release/mac/AETHER.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("AETHER is damaged and can't be opened").
    ...
    # checkout stays clean for the next `aether update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the AETHER-managed Node
            # provisioned during prerequisites/node-deps (at $AETHER_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $AETHER_HOME.
    # $AETHER_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'aether update' on this host install.
