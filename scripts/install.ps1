...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$AetherHome = $(if ($env:AETHER_HOME) { $env:AETHER_HOME } else { "$env:LOCALAPPDATA\aether" }),
    [string]$InstallDir = $(if ($env:AETHER_HOME) { "$env:AETHER_HOME\aether-agent" } else { "$env:LOCALAPPDATA\aether\aether-agent" }),
    ...
    # builds apps/desktop into a launchable AETHER.exe.
    #
    # Why opt-in:
    #   * AETHER-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched AETHER.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live AETHER.exe
    ...
    #     `aether desktop` already builds on demand.
...
function Write-Banner {
    ...
    Write-Host "|             * AETHER Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $AetherHome)) {
        New-Item -ItemType Directory -Force -Path $AetherHome | Out-Null
    }
    $envFile = Join-Path $AetherHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $AetherHome "node"
...
function Install-Uv {
    # AETHER owns its own uv at $AetherHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (aether_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `aether update` stay in sync.
    $managedUv = Join-Path $AetherHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $AetherHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $AetherHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $AetherHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $AetherHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and AETHER_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under AETHER-Setup.exe
...
function Install-Git {
    ...
    which AETHER uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\aether\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    AETHER needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the AETHER copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\aether\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``AETHER_GIT_BASH_PATH`` (User scope) so AETHER can find it in a fresh
    ...
    # Download PortableGit into $AetherHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $AetherHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent AETHER features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  AETHER needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``AETHER_GIT_BASH_PATH`` (User env scope) so AETHER can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $AetherHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $AetherHome\git\usr\bin\bash.exe
    $candidates += "$AetherHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$AetherHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("AETHER_GIT_BASH_PATH", $candidate, "User")
            $env:AETHER_GIT_BASH_PATH = $candidate
            Write-Info "Set AETHER_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- AETHER may not find Git Bash."
    Write-Info "If needed, set AETHER_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$AetherHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$AetherHome\node;$env:Path"
        Write-Success "Node.js $version found (AETHER-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing AETHER-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $AetherHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $AetherHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\aether-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$AetherHome\node") { Remove-Item -Recurse -Force "$AetherHome\node" }
                Move-Item $extractedDir.FullName "$AetherHome\node"
                ...
                $version = & "$AetherHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $AetherHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\aether-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `aether
                    # update` path (#4735).
                    ...
                    $stashName = "aether-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if AETHER behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running aether processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                $zipPath = "$env:TEMP\aether-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\aether-agent-extract"
    ...
    # next `aether update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under AETHER-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running aether process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running aether processes before recreating venv..."
            # The launcher CLI (aether.exe) plus its child tree.
            & taskkill /F /T /IM aether.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM aether.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m aether_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not aether.exe.
            ...
            # user (see aether_cli/gateway_windows.py), so the installer always
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `aether.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install aether-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and aether.exe broken with
    ...
    # here, not 30 seconds later when the user runs `aether`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `aether dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `aether dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `aether dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up aether command..."
    
    if ($NoVenv) {
        $aetherBin = "$InstallDir"
    } else {
        $aetherBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so aether is globally available
    # On Windows, the aether.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$aetherBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$aetherBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $aetherBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set AETHER_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\aether instead
    # of the Unix default ~/.aether
    $currentAetherHome = [Environment]::GetEnvironmentVariable("AETHER_HOME", "User")
    if (-not $currentAetherHome -or $currentAetherHome -ne $AetherHome) {
        [Environment]::SetEnvironmentVariable("AETHER_HOME", $AetherHome, "User")
        Write-Success "Set AETHER_HOME=$AetherHome"
    }
    $env:AETHER_HOME = $AetherHome
    
    # Update current session
    $env:Path = "$aetherBin;$env:Path"
    
    Write-Success "aether command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.aether-bootstrap-complete which tells the AETHER
    ...
    # AETHER-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `aether update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".aether-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the AETHER_HOME directory structure ($AetherHome, default %LOCALAPPDATA%\aether)
    New-Item -ItemType Directory -Force -Path "$AetherHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AetherHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AetherHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AetherHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AetherHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AetherHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AetherHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AetherHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AetherHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$AetherHome\.env"
    ...
    # Create config.yaml
    $configPath = "$AetherHome\config.yaml"
    ...
    # (the default PS5 behaviour), and AETHER's prompt-injection scanner
...
"@
    ...
    Write-Success "Configuration directory ready: $AetherHome"
    
    # Seed bundled skills into $AetherHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $AetherHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $AetherHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$AetherHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $AetherHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (AETHER-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'aether setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\aether-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\aether-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\aether-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `AETHER` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable AETHER.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_aether_desktop command
    # resolves apps/desktop/release/win-unpacked/AETHER.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the AETHER-managed
    ...
    # The --dir mode produces an unpacked AETHER.exe in
    ...
    # are NOT trying to work around). The AETHER icon + product name are
    # stamped onto AETHER.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\aether-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\AETHER.exe",
        "$desktopDir\release\win-arm64-unpacked\AETHER.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no AETHER.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The AETHER icon + identity are stamped onto AETHER.exe by the
    ...
    #    AETHER.exe. We deliberately do NOT point them at `aether desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'AETHER.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'AETHER.lnk')
        )
        ...
        # with the AETHER icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.aether/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'aether setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m aether_cli.main setup
    } else {
        python -m aether_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $aetherCmd = "$InstallDir\venv\Scripts\aether.exe"
    if (-not (Test-Path $aetherCmd)) {
        $aetherCmd = "aether"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$AetherHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'aether whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: aether gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$AetherHome\logs\gateway.log"
            Start-Process -FilePath $aetherCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$AetherHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: aether gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: aether gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$AetherHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$AetherHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$AetherHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$AetherHome\aether-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   aether              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   aether setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   aether config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   aether config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   aether gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   aether update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning AETHER repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (AETHER-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding AETHER to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://aether.hypertek.vn/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
