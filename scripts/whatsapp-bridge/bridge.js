 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.aether/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.aether', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (AETHER_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.AETHER_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.aether', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.AETHER_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.aether', 'document_cache');
const AUDIO_CACHE_DIR = process.env.AETHER_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.aether', 'audio_cache');
...
// `aether update` updates bridge.js on disk but a long-lived bridge process
...
const DEFAULT_REPLY_PREFIX = '⚕ *AETHER*\n────────────\n';
...
async function startSocket() {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Ignore AETHER' own reply messages in self-chat mode to avoid loops.
  ...
  try {
    ...
    switch (type) {
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `aether_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execSync(
