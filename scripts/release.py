...
VERSION_FILE = REPO_ROOT / "aether_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "aether@marian.local": "Schrotti77",
    ...
    "274096618+aether-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "aether@nousresearch.com": "NousResearch",
    ...
    "aether@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "github@nadyaaether.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (AETHER_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (AETHER_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: aether-eval community link)
    ...
    "aetheragent26@gmail.com": "aetheragent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+aetheragent26@users.noreply.github.com": "aetheragent26",
    ...
    "vigo@aether": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.aether@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local AETHER runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level AETHER_HOME state files)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live AETHER
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"aether-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "aether@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/hermes-agent",
    ...
    lines.append(f"# AETHER v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for AETHER. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="AETHER Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  AETHER Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"AETHER v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"AETHER v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'AETHER v{new_version} ({calver_date})' "
