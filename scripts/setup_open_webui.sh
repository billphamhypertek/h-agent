...
# Bootstrap Open WebUI against AETHER's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.aether/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-aether.sh
...
AETHER_ENV_FILE="${AETHER_ENV_FILE:-$HOME/.aether/.env}"
AETHER_API_PORT="${AETHER_API_PORT:-8642}"
AETHER_API_HOST="${AETHER_API_HOST:-127.0.0.1}"
AETHER_API_CONNECT_HOST="${AETHER_API_CONNECT_HOST:-127.0.0.1}"
AETHER_API_MODEL_NAME="${AETHER_API_MODEL_NAME:-AETHER}"
AETHER_API_BASE_URL="http://${AETHER_API_CONNECT_HOST}:${AETHER_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-aether.sh"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$AETHER_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$AETHER_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.aether.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.aether</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.aether"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.aether"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-aether.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to AETHER
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-aether.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.aether/logs/openwebui.log
StandardError=append:%h/.aether/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-aether.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$AETHER_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring AETHER API server is configured...'
  upsert_env API_SERVER_ENABLED true "$AETHER_ENV_FILE"
  upsert_env API_SERVER_HOST "$AETHER_API_HOST" "$AETHER_ENV_FILE"
  upsert_env API_SERVER_PORT "$AETHER_API_PORT" "$AETHER_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$AETHER_API_MODEL_NAME" "$AETHER_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$AETHER_ENV_FILE"
  ensure_env_permissions

  log 'Restarting AETHER gateway so API server settings take effect...'
  aether gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${AETHER_API_CONNECT_HOST}:${AETHER_API_PORT}/health" >/dev/null; then
    log 'AETHER API server did not answer on the first check. Trying to start gateway in the background...'
    nohup aether gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${AETHER_API_CONNECT_HOST}:${AETHER_API_PORT}/health" >/dev/null
  ...
  log "AETHER API endpoint: ${AETHER_API_BASE_URL}"
