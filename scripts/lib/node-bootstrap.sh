...
#   2. ~/.aether/node/ from a prior AETHER-managed install
#   3. fnm, proto, nvm (in that order) if the user already uses a version manager
#   4. Termux `pkg`, macOS Homebrew
#   5. pinned nodejs.org tarball into ~/.aether/node/ (always works, zero shell rc edits)
#
# Usage:
#   source scripts/lib/node-bootstrap.sh
#   ensure_node   # returns 0 on success, non-zero on failure
#   if [ "$AETHER_NODE_AVAILABLE" = true ]; then ...; fi
#
# Env inputs (set before sourcing to override defaults):
#   AETHER_NODE_MIN_VERSION   (default: 20)   — accepted on PATH
#   AETHER_NODE_TARGET_MAJOR  (default: 22)   — installed when we install
#   AETHER_HOME               (default: $HOME/.aether)
# ============================================================================

AETHER_NODE_MIN_VERSION="${AETHER_NODE_MIN_VERSION:-20}"
AETHER_NODE_TARGET_MAJOR="${AETHER_NODE_TARGET_MAJOR:-22}"
AETHER_HOME="${AETHER_HOME:-$HOME/.aether}"
AETHER_NODE_AVAILABLE=false
...
# Redirect a AETHER-managed Node's `npm install -g` to the command link dir
# (already on PATH) instead of the default $AETHER_HOME/node/bin, which is off
...
_nb_configure_npm_prefix() {
    ...
    printf 'prefix=%s\n' "$(dirname "$_link_dir")" > "$AETHER_HOME/node/etc/npmrc"
}
...
_nb_have_modern_node() {
    ...
    [ "$(_nb_node_major)" -ge "$AETHER_NODE_MIN_VERSION" ]
}
...
_nb_try_fnm() {
    ...
    _nb_log "fnm detected — installing Node $AETHER_NODE_TARGET_MAJOR..."
    eval "$(fnm env 2>/dev/null)" || true
    fnm install "$AETHER_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    fnm use     "$AETHER_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
...
_nb_try_proto() {
    ...
    _nb_log "proto detected — installing Node $AETHER_NODE_TARGET_MAJOR..."
    proto install node "$AETHER_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_nvm() {
    ...
    _nb_log "nvm detected — installing Node $AETHER_NODE_TARGET_MAJOR..."
    nvm install "$AETHER_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    nvm use     "$AETHER_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_brew() {
    ...
    brew install "node@${AETHER_NODE_TARGET_MAJOR}" >/dev/null 2>&1 \
        || brew install node >/dev/null 2>&1 \
        || return 1
    brew link --overwrite --force "node@${AETHER_NODE_TARGET_MAJOR}" >/dev/null 2>&1 || true
    _nb_have_modern_node || return 1
...
_nb_install_bundled_node() {
    ...
    local index_url="https://nodejs.org/dist/latest-v${AETHER_NODE_TARGET_MAJOR}.x/"
    local tarball
    tarball=$(curl -fsSL "$index_url" \
        | grep -oE "node-v${AETHER_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.xz" \
        | head -1)
    if [ -z "$tarball" ]; then
        tarball=$(curl -fsSL "$index_url" \
            | grep -oE "node-v${AETHER_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.gz" \
            | head -1)
    fi
    if [ -z "$tarball" ]; then
        _nb_warn "Could not resolve Node $AETHER_NODE_TARGET_MAJOR binary for $node_os-$node_arch"
        return 1
    fi
    ...
    _nb_log "Extracting to $AETHER_HOME/node/..."
    ...
    mv "$extracted" "$AETHER_HOME/node"
    rm -rf "$tmp"

    local _link_dir
    _link_dir="$(_nb_get_link_dir)"
    mkdir -p "$_link_dir"
    ln -sf "$AETHER_HOME/node/bin/node" "$_link_dir/node"
    ln -sf "$AETHER_HOME/node/bin/npm"  "$_link_dir/npm"
    ln -sf "$AETHER_HOME/node/bin/npx"  "$_link_dir/npx"
    ...
    _nb_have_modern_node || return 1
    _nb_ok "Node $(node --version) installed to $AETHER_HOME/node/"
    return 0
}
...
ensure_node() {
    ...
    if _nb_have_modern_node; then
        ...
        AETHER_NODE_AVAILABLE=true
        return 0
    fi

    if [ -x "$AETHER_HOME/node/bin/node" ]; then
        export PATH="$AETHER_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (AETHER-managed)"
            AETHER_NODE_AVAILABLE=true
            return 0
        fi
    fi
    ...
    _nb_try_fnm   && { AETHER_NODE_AVAILABLE=true; return 0; }
    _nb_try_proto && { AETHER_NODE_AVAILABLE=true; return 0; }
    _nb_try_nvm   && { AETHER_NODE_AVAILABLE=true; return 0; }

    # Platform package managers.
    _nb_try_termux_pkg && { AETHER_NODE_AVAILABLE=true; return 0; }
    _nb_try_brew       && { AETHER_NODE_AVAILABLE=true; return 0; }

    # Last resort: pinned nodejs.org tarball.
    _nb_install_bundled_node && { AETHER_NODE_AVAILABLE=true; return 0; }

    _nb_warn "Node.js install failed — TUI and browser tools will be unavailable."
    _nb_warn "Install manually: https://nodejs.org/en/download/  (or: \`brew install node\`, \`fnm install $AETHER_NODE_TARGET_MAJOR\`, etc.)"
    return 1
}
