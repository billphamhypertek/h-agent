...
#
# Wired into the image as /etc/cont-init.d/01-aether-setup by the
...
AETHER_HOME="${AETHER_HOME:-/opt/data}"
INSTALL_DIR="/opt/aether"

# Drop to aether via s6-setuidgid, but skip it when already non-root.
as_aether() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid aether "$@"; }
...
# starts non-root. The baked install tree under /opt/aether is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $AETHER_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass AETHER_UID/AETHER_GID — or the PUID/PGID aliases — which the
...
# `id -u aether` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# aether UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u aether)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-aether UID).
...
and the baked /opt/aether install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e AETHER_UID=\$(id -u) -e AETHER_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the aether user to that UID/GID at boot and chowns the data
...
# --- Bootstrap AETHER_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid aether mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom AETHER_HOME paths whose parents only
# root can create (e.g. `AETHER_HOME=/home/aether/.aether` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for AETHER_UID/AETHER_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  AETHER_UID/
# AETHER_GID still win when both are set.  See #15290, salvages #25872.
AETHER_UID="${AETHER_UID:-${PUID:-}}"
AETHER_GID="${AETHER_GID:-${PGID:-}}"

if [ -n "${AETHER_UID:-}" ] && validate_uid_gid "$AETHER_UID" && [ "$AETHER_UID" != "$(id -u aether)" ]; then
    echo "[stage2] Changing aether UID to $AETHER_UID"
    usermod -u "$AETHER_UID" aether
fi
if [ -n "${AETHER_GID:-}" ] && validate_uid_gid "$AETHER_GID" && [ "$AETHER_GID" != "$(id -g aether)" ]; then
    echo "[stage2] Changing aether GID to $AETHER_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$AETHER_GID" hermes 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised aether user (UID 10000)
...
# aether process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes aether. Idempotent across container
...
#   - aether is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G hermes 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] aether already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" hermes 2>/dev/null; then
        echo "[stage2] Added aether to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group aether failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When AETHER_UID is remapped or the top-level $AETHER_HOME isn't owned by
# the runtime aether UID, restore ownership to aether — but ONLY for the
# directories aether actually writes to. The full $AETHER_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of aether-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_aether_uid=$(id -u aether)
needs_chown=false
if [ "$(stat -c %u "$AETHER_HOME" 2>/dev/null)" != "$actual_aether_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $AETHER_HOME (targeted) to aether ($actual_aether_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $AETHER_HOME: chown the directory itself (not its contents)
    # so aether can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    chown aether:aether "$AETHER_HOME" 2>/dev/null || \
        echo "[stage2] Warning: chown $AETHER_HOME failed (rootless container?) — continuing"
    # AETHER-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by aether (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$AETHER_HOME/$sub" ]; then
            chown -R aether:aether "$AETHER_HOME/$sub" 2>/dev/null || \
                echo "[stage2] Warning: chown $AETHER_HOME/$sub failed (rootless container?) — continuing"
        fi
    done
fi
...
# aether user. Hosted/container instances keep mutable state under
# $AETHER_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# AETHER_DISABLE_LAZY_INSTALLS=1. Keeping /opt/aether root-owned and
...
# $AETHER_HOME/lazy-packages dir on the data volume (Dockerfile sets
# AETHER_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to aether
...
# Always reset ownership of $AETHER_HOME/profiles to aether on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> aether …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as aether and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$AETHER_HOME/profiles" ]; then
    chown -R aether:aether "$AETHER_HOME/profiles" 2>/dev/null || true
fi

# Always reset ownership of $AETHER_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged aether runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$AETHER_HOME/cron" ]; then
    chown -R aether:aether "$AETHER_HOME/cron" 2>/dev/null || true
fi

# Reset ownership of aether-owned top-level state files on every boot.
# The targeted data-volume chown above only covers aether-owned
# *subdirectories*; loose state files living directly under $AETHER_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> aether …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged aether runtime then hits
...
# sweep so host-owned files in a bind-mounted $AETHER_HOME are never
...
# entries of aether_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$AETHER_HOME/$f" ]; then
        chown aether:aether "$AETHER_HOME/$f" 2>/dev/null || true
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the aether runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$AETHER_HOME/config.yaml" ]; then
    chown aether:aether "$AETHER_HOME/config.yaml" 2>/dev/null || true
    chmod 640 "$AETHER_HOME/config.yaml" 2>/dev/null || true
fi

# --- Seed directory structure as aether user ---
# Run as aether via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $AETHER_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_aether mkdir -p \
    "$AETHER_HOME/cron" \
    "$AETHER_HOME/sessions" \
    "$AETHER_HOME/logs" \
    "$AETHER_HOME/logs/gateways" \
    "$AETHER_HOME/hooks" \
    "$AETHER_HOME/memories" \
    "$AETHER_HOME/skills" \
    "$AETHER_HOME/skins" \
    "$AETHER_HOME/plans" \
    "$AETHER_HOME/workspace" \
    "$AETHER_HOME/home" \
    "$AETHER_HOME/pairing" \
    "$AETHER_HOME/platforms/pairing" \
    "$AETHER_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/aether/.install_method (see Dockerfile), NOT written here into
# $AETHER_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $AETHER_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.aether:/opt/data) and sometimes shared with a
...
# 'aether update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $AETHER_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/aether stamp, so deleting the data-dir copy is safe).
if [ -f "$AETHER_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$AETHER_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$AETHER_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$AETHER_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        as_aether cp "$INSTALL_DIR/$src" "$AETHER_HOME/$dest"
    fi
}
...
if [ -f "$AETHER_HOME/.env" ]; then
    chown aether:aether "$AETHER_HOME/.env" 2>/dev/null || true
    chmod 600 "$AETHER_HOME/.env" 2>/dev/null || true
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $AETHER_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `aether update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set AETHER_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$AETHER_HOME/config.yaml" ]; then
    s6-setuidgid aether "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$AETHER_HOME/auth.json" ] && [ -n "${AETHER_AUTH_JSON_BOOTSTRAP:-}" ]; then
    printf '%s' "$AETHER_AUTH_JSON_BOOTSTRAP" > "$AETHER_HOME/auth.json"
    chown aether:aether "$AETHER_HOME/auth.json" 2>/dev/null || true
    chmod 600 "$AETHER_HOME/auth.json"
fi
...
# can set AETHER_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# AETHER_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$AETHER_HOME/gateway_state.json" ] && \
        [ "${AETHER_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    printf '{"gateway_state":"running"}\n' > "$AETHER_HOME/gateway_state.json"
    chown aether:aether "$AETHER_HOME/gateway_state.json" 2>/dev/null || true
    chmod 644 "$AETHER_HOME/gateway_state.json"
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_aether "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/aether/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI AETHER spawns for the
...
# main-wrapper.sh propagates it into the supervised ``aether`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-aether, dashboard, etc.).
