...
El instalador se encarga de todo: uv, Python 3.11, Node.js, ripgrep, ffmpeg, **y un Git Bash portátil** (MinGit, descomprimido en `%LOCALAPPDATA%\aether\git` — no requiere administrador, completamente aislado de cualquier instalación de Git del sistema). AETHER usa este Git Bash incluido para ejecutar comandos de shell.
...
> **Android / Termux:** La ruta manual probada está documentada en la [guía de Termux](https://aether.hypertek.vn/docs/getting-started/termux). En Termux, AETHER instala el extra `.[termux]` curado porque el extra completo `.[all]` actualmente incluye dependencias de voz incompatibles con Android.
>
> **Windows:** Windows nativo es totalmente compatible — el comando de PowerShell de arriba instala todo. Si prefieres usar WSL2, el comando de Linux también funciona allí. La instalación nativa de Windows se encuentra en `%LOCALAPPDATA%\aether`; WSL2 instala en `~/.aether` como en Linux.

Después de la instalación:

```bash
source ~/.bashrc    # recargar shell (o: source ~/.zshrc)
aether              # ¡empieza a chatear!
```
...
```bash
aether              # CLI interactiva — inicia una conversación
aether model        # Elige tu proveedor y modelo LLM
aether tools        # Configura qué herramientas están habilitadas
aether config set   # Establece valores de configuración individuales
aether gateway      # Inicia el gateway de mensajería (Telegram, Discord, etc.)
aether setup        # Ejecuta el asistente de configuración completo
aether claw migrate # Migra desde OpenClaw (si vienes de OpenClaw)
aether update       # Actualiza a la última versión
aether doctor       # Diagnostica cualquier problema
```

📖 **[Documentación completa →](https://aether.hypertek.vn/docs/)**

---

## Evita la colección de claves API — Nous Portal

AETHER funciona con cualquier proveedor que quieras — eso no cambiará. Pero si prefieres no recopilar cinco claves API separadas para el modelo, búsqueda web, generación de imágenes, TTS y un navegador en la nube, **[Nous Portal](https://portal.nousresearch.com)** las cubre todas bajo una sola suscripción:
...
Esto te autentica vía OAuth, establece Nous como tu proveedor y activa el Tool Gateway. Comprueba qué está conectado en cualquier momento con `aether portal info`. Detalles completos en la [página de documentación del Tool Gateway](https://aether.hypertek.vn/docs/user-guide/features/tool-gateway).
...
AETHER tiene dos puntos de entrada: inicia la interfaz de terminal con `aether`, o ejecuta el gateway y habla con él desde Telegram, Discord, Slack, WhatsApp, Signal o Email. Una vez en una conversación, muchos comandos de barra son compartidos entre ambas interfaces.

| Acción                              | CLI                                           | Plataformas de mensajería                                                         |
| ----------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------- |
| Empezar a chatear                   | `aether`                                      | Ejecuta `aether gateway setup` + `aether gateway start`, luego envía un mensaje al bot |
...
Para las listas de comandos completas, consulta la [guía de CLI](https://aether.hypertek.vn/docs/user-guide/cli) y la [guía del Gateway de Mensajería](https://aether.hypertek.vn/docs/user-guide/messaging).

---

## Documentación

Toda la documentación está en **[aether.hypertek.vn/docs](https://aether.hypertek.vn/docs/)**:

| Sección                                                                                             | Contenido                                                    |
| --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| [Inicio rápido](https://aether.hypertek.vn/docs/getting-started/quickstart)              | Instalar → configurar → primera conversación en 2 minutos   |
| [Uso de CLI](https://aether.hypertek.vn/docs/user-guide/cli)                             | Comandos, atajos de teclado, personalidades, sesiones        |
| [Configuración](https://aether.hypertek.vn/docs/user-guide/configuration)               | Archivo de configuración, proveedores, modelos, todas las opciones |
| [Gateway de Mensajería](https://aether.hypertek.vn/docs/user-guide/messaging)           | Telegram, Discord, Slack, WhatsApp, Signal, Home Assistant   |
| [Seguridad](https://aether.hypertek.vn/docs/user-guide/security)                        | Aprobación de comandos, emparejamiento por DM, aislamiento en contenedor |
| [Herramientas y Toolsets](https://aether.hypertek.vn/docs/user-guide/features/tools)   | Más de 40 herramientas, sistema de toolsets, backends de terminal |
| [Sistema de Habilidades](https://aether.hypertek.vn/docs/user-guide/features/skills)   | Memoria procedimental, Skills Hub, creación de habilidades   |
| [Memoria](https://aether.hypertek.vn/docs/user-guide/features/memory)                   | Memoria persistente, perfiles de usuario, mejores prácticas  |
| [Integración MCP](https://aether.hypertek.vn/docs/user-guide/features/mcp)              | Conecta cualquier servidor MCP para capacidades extendidas   |
| [Programación Cron](https://aether.hypertek.vn/docs/user-guide/features/cron)           | Tareas programadas con entrega a plataforma                  |
| [Archivos de Contexto](https://aether.hypertek.vn/docs/user-guide/features/context-files) | Contexto de proyecto que da forma a cada conversación      |
| [Arquitectura](https://aether.hypertek.vn/docs/developer-guide/architecture)            | Estructura del proyecto, bucle del agente, clases principales |
| [Contribuir](https://aether.hypertek.vn/docs/developer-guide/contributing)              | Configuración de desarrollo, proceso de PR, estilo de código |
| [Referencia de CLI](https://aether.hypertek.vn/docs/reference/cli-commands)             | Todos los comandos y flags                                   |
| [Variables de Entorno](https://aether.hypertek.vn/docs/reference/environment-variables) | Referencia completa de variables de entorno                  |

---

## Migración desde OpenClaw

Si vienes de OpenClaw, AETHER puede importar automáticamente tu configuración, memorias, habilidades y claves API.

**Durante la configuración inicial:** El asistente de configuración (`aether setup`) detecta automáticamente `~/.openclaw` y ofrece migrar antes de que comience la configuración.

**En cualquier momento después de instalar:**

```bash
aether claw migrate              # Migración interactiva (preset completo)
aether claw migrate --dry-run    # Vista previa de qué se migraría
aether claw migrate --preset user-data   # Migrar sin secretos
aether claw migrate --overwrite  # Sobreescribir conflictos existentes
```
...
- **Habilidades** — habilidades creadas por el usuario → `~/.aether/skills/openclaw-imports/`
...
Consulta `aether claw migrate --help` para todas las opciones, o usa la habilidad `openclaw-migration` para una migración guiada interactiva por el agente con vistas previas de dry-run.
