"""
Tests for mcp_serve — AETHER MCP server.
...
def _isolate_aether_home(tmp_path, monkeypatch):
    """Redirect AETHER_HOME to a temp directory."""
    monkeypatch.setenv("AETHER_HOME", str(tmp_path))
    try:
        import aether_constants
        monkeypatch.setattr(aether_constants, "get_aether_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking aether_state schema."""
