...
def _restore_tool_modules():
    original_aether_home = os.environ.get("AETHER_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_aether_home is None:
            os.environ.pop("AETHER_HOME", None)
        else:
            os.environ["AETHER_HOME"] = original_aether_home
        _reset_modules(("tools", "aether_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    aether_cli = types.ModuleType("aether_cli")
    aether_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["aether_cli"] = aether_cli
    aether_home = tmp_path / "aether-home"
    os.environ["AETHER_HOME"] = str(aether_home)
    sys.modules["aether_cli.config"] = types.SimpleNamespace(
        get_aether_home=lambda: aether_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="aether-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": aether_home / "modal_snapshots.json",
