...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad AETHER_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: AETHER_GATEWAY_SESSION set, a bound session
    ...
    monkeypatch.delenv("AETHER_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("AETHER_CRON_SESSION", "1")
    monkeypatch.delenv("AETHER_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_aether_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "AETHER_HOME": "/h", "AETHER_PROFILE": "p",
        "AETHER_CONFIG": "/c.yaml", "AETHER_ENV": "/e",
        # other AETHER_* → dropped (broad prefix removed)
        "AETHER_BASE_URL": "https://x", "AETHER_INTERACTIVE": "1",
        "AETHER_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("AETHER_HOME", "AETHER_PROFILE", "AETHER_CONFIG", "AETHER_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "AETHER_BASE_URL", "AETHER_INTERACTIVE", "AETHER_KANBAN_DB",
...
def test_env_scrub_logs_dropped_aether_vars(caplog):
    """Dropping a non-allowlisted, non-secret AETHER_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset AETHER_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "AETHER_HOME": "/h",          # allowlisted → kept, not logged
        "AETHER_BASE_URL": "https://x",   # dropped → logged
        "AETHER_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "AETHER_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "AETHER_HOME" in out and "PATH" in out
    assert "AETHER_BASE_URL" not in out and "AETHER_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "AETHER_BASE_URL" in msgs and "AETHER_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "AETHER_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped AETHER_* vars."""
    import logging
