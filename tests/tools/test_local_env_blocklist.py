...
Verifies that AETHER-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed AETHER secrets.
...
from tools.environments.local import (
    LocalEnvironment,
    _AETHER_PROVIDER_ENV_BLOCKLIST,
    _AETHER_PROVIDER_ENV_FORCE_PREFIX,
)
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.aether/.env must not leak."""
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is a AETHER inference secret
        ...
        models`` run inside a AETHER terminal enumerated the entire Bedrock
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        refuses to re-allow anything in _AETHER_PROVIDER_ENV_BLOCKLIST
        (GHSA-rhgp-j443-p4rf). Only the Bedrock inference bearer token is
        AETHER-managed; the rest belongs to the user.
        """
        general_chain = {
    ...
    def test_tool_and_gateway_vars_are_stripped(self):
        ...
        leaked_vars = {
            ...
            "AETHER_DASHBOARD_SESSION_TOKEN": "dashboard-session-secret",
...
class TestForceEnvOptIn:
    """Callers can opt in to passing a blocked var via _AETHER_FORCE_ prefix."""

    def test_force_prefix_passes_blocked_var(self):
        """_AETHER_FORCE_OPENAI_API_KEY in self.env should inject OPENAI_API_KEY."""
        result_env = _run_with_env(self_env={
            f"{_AETHER_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY": "sk-explicit",
        })
        ...
        assert f"{_AETHER_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY" not in result_env
    ...
    def test_force_prefix_overrides_os_environ_block(self):
        ...
        result_env = _run_with_env(
            ...
            self_env={f"{_AETHER_PROVIDER_ENV_FORCE_PREFIX}OPENAI_BASE_URL": "http://intended/v1"},
        )
...
class TestBlocklistCoverage:
    ...
    def test_issue_1002_offenders(self):
        ...
        assert must_block.issubset(_AETHER_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_registry_vars_are_in_blocklist(self):
        ...
        for pconfig in PROVIDER_REGISTRY.values():
            for var in pconfig.api_key_env_vars:
                assert var in _AETHER_PROVIDER_ENV_BLOCKLIST, (
                    f"Registry var {var} (provider={pconfig.id}) missing from blocklist"
                )
            if pconfig.base_url_env_var:
                assert pconfig.base_url_env_var in _AETHER_PROVIDER_ENV_BLOCKLIST, (
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their AETHER-managed
        ...
        assert "AWS_BEARER_TOKEN_BEDROCK" in _AETHER_PROVIDER_ENV_BLOCKLIST
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        operator shell (SECURITY.md §3.2), not to AETHER, and blocklisting
        ...
        leaked_block = general_chain & _AETHER_PROVIDER_ENV_BLOCKLIST
    ...
    def test_extra_auth_vars_covered(self):
        ...
        assert extras.issubset(_AETHER_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_optional_tool_and_messaging_vars_are_in_blocklist(self):
        ...
        for name, metadata in OPTIONAL_ENV_VARS.items():
            ...
            if category in {"tool", "messaging"}:
                assert name in _AETHER_PROVIDER_ENV_BLOCKLIST, (
...
class TestSanePathIncludesHomebrew:
    ...
    def _disable_aether_bin_injection(self):
        """These tests assert the sane-path merge in isolation. Disable the
        aether-install-dir prepend (a separate concern, covered by
        TestAetherBinDirOnPath) so a real ``aether`` on the test runner's PATH
        doesn't shift the asserted PATH layout."""
        from tools.environments import local as local_mod
        saved = local_mod._AETHER_BIN_DIR
        local_mod._AETHER_BIN_DIR = None  # resolved -> no dir to inject
        yield
        local_mod._AETHER_BIN_DIR = saved
...
class TestAetherBinDirOnPath:
    """The aether install dir is reachable in the terminal subshell PATH.

    Plugins shelling out to bare ``aether`` via the terminal tool must work
    even when the gateway was launched without the aether install dir on
    PATH (systemd, service managers, cron). See the discussion that motivated
    _resolve_aether_bin_dir / _prepend_aether_bin_dir.
    """

    def _reset_cache(self):
        from tools.environments import local as local_mod
        local_mod._AETHER_BIN_DIR = local_mod._SENTINEL

    def test_resolves_via_which(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        monkeypatch.setattr(local_mod.shutil, "which",
                            lambda name: "/opt/aether/bin/aether" if name == "aether" else None)
        monkeypatch.setattr(local_mod.os.path, "isdir", lambda p: p == "/opt/aether/bin")
        assert local_mod._resolve_aether_bin_dir() == "/opt/aether/bin"
    ...
    def test_resolves_via_sys_executable_dir(self, monkeypatch, tmp_path):
        ...
        (venv_bin / "aether").write_text("#!/bin/sh\n")
        ...
        assert local_mod._resolve_aether_bin_dir() == str(venv_bin)
    ...
    def test_returns_none_when_unresolvable(self, monkeypatch):
        ...
        assert local_mod._resolve_aether_bin_dir() is None

    def test_prepend_adds_missing_dir_at_front(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._AETHER_BIN_DIR = "/opt/aether/bin"
        out = local_mod._prepend_aether_bin_dir("/usr/bin:/bin")
        assert out.split(os.pathsep)[0] == "/opt/aether/bin"
    ...
    def test_prepend_is_idempotent(self, monkeypatch):
        ...
        self._reset_cache()
        local_mod._AETHER_BIN_DIR = "/opt/aether/bin"
        once = local_mod._prepend_aether_bin_dir("/usr/bin:/bin")
        twice = local_mod._prepend_aether_bin_dir(once)
        assert twice == once
        assert once.split(os.pathsep).count("/opt/aether/bin") == 1

    def test_prepend_noop_when_unresolved(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._AETHER_BIN_DIR = None
        assert local_mod._prepend_aether_bin_dir("/usr/bin:/bin") == "/usr/bin:/bin"

    def test_make_run_env_injects_aether_bin_dir(self, monkeypatch):
        """A gateway env missing the aether dir gets it back in the subshell PATH."""
        from tools.environments import local as local_mod
        from tools.environments.local import _make_run_env
        self._reset_cache()
        local_mod._AETHER_BIN_DIR = "/opt/aether/bin"
        ...
        assert entries[0] == "/opt/aether/bin"
