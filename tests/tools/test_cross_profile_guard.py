...
def fake_aether(tmp_path, monkeypatch):
    """Build a two-profile AETHER layout and point AETHER_HOME at
    the aether-security profile (matching the original-incident shape).
    """
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_aether):
        from tools.file_tools import write_file_tool
        target = fake_aether["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_aether):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_aether["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "aether-security" in result["error"]
        # File untouched.
        assert target.read_text() == original

    def test_cross_profile_True_bypass(self, fake_aether):
    ...
    def test_non_aether_path_unaffected(self, fake_aether, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_aether):
    ...
    def test_cross_profile_patch_bypass(self, fake_aether):
        from tools.file_tools import patch_tool
        target = fake_aether["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_aether):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_aether, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_aether["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up AETHER_HOME (set in
        ...
        assert "not found in active profile 'aether-security'" in err
        assert "default" in err
        assert "cross_profile=True" in err

    def test_error_names_multiple_profiles(self, fake_aether, monkeypatch):
        """When the skill exists in TWO other profiles, both should be named."""
        self._make_skill_in_profile(fake_aether["root"], "everywhere-skill")
        self._make_skill_in_profile(fake_aether["coder_home"], "everywhere-skill")
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.aether/profiles/<name>/."""
        # Don't set AETHER_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_aether_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_aether_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_aether):
        """When active profile is 'aether-security', the prompt warns
        ...
        assert "Active AETHER profile" in src
        assert "cross_profile=True" in src
        assert "~/.aether/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active AETHER profile: default" in src
        assert "Active AETHER profile: {active_profile}" in src
