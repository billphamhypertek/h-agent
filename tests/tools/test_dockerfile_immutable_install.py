"""Contract tests for the Docker image's immutable /opt/aether install tree."""
...
def test_dockerfile_makes_opt_aether_root_owned_and_non_writable() -> None:
    text = _dockerfile_text()

    assert "COPY --chown=aether:aether . ." not in text
    assert "COPY . ." in text
    assert "chown -R root:root /opt/aether" in text
    assert "chmod -R a+rX /opt/aether" in text
    assert "chmod -R a-w /opt/aether" in text

    immutable_block = re.search(
        r"RUN mkdir -p /opt/aether/bin && \\\n"
        r"(?:.*\\\n)+?"
        r"\s+chmod -R a-w /opt/aether",
        text,
    )
    assert immutable_block, "Dockerfile must lock /opt/aether after installing code/deps"


def test_dockerfile_keeps_mutable_state_under_opt_data() -> None:
    text = _dockerfile_text()

    assert "ENV AETHER_HOME=/opt/data" in text
    assert "ENV AETHER_WRITE_SAFE_ROOT=/opt/data" in text
...
def test_dockerfile_disables_runtime_install_mutations() -> None:
    ...
    assert "ENV AETHER_DISABLE_LAZY_INSTALLS=1" in text
    assert "AETHER_TUI_DIR=/opt/aether/ui-tui" in text


def test_dockerfile_does_not_chown_install_trees_to_aether() -> None:
    text = _dockerfile_text()
    forbidden_patterns = (
        r"chown\s+-R\s+aether:aether\s+/opt/aether/\.venv",
        r"chown\s+-R\s+aether:aether\s+/opt/aether/ui-tui",
        r"chown\s+-R\s+aether:aether\s+/opt/aether/gateway",
        r"chown\s+-R\s+aether:aether\s+/opt/aether/node_modules",
    )
    for pattern in forbidden_patterns:
        assert not re.search(pattern, text), (
            "runtime install trees under /opt/aether must stay immutable; "
...
def test_dockerfile_bakes_code_scoped_install_method_stamp() -> None:
    ...
    (/opt/aether/.install_method) first; baking it at build time keeps the
    published image self-identifying as 'docker' WITHOUT writing into the
    shared $AETHER_HOME data volume (which a host install may also use).
    It must live inside the immutable block so the runtime user can't alter it.
    """
    text = _dockerfile_text()
    assert "printf 'docker\\n' > /opt/aether/.install_method" in text
...
def test_dockerfile_redirects_lazy_installs_to_durable_target() -> None:
    ...
    # never under the immutable /opt/aether tree.
    assert f"ENV AETHER_LAZY_INSTALL_TARGET={target}" in text
    assert target.startswith("/opt/data/"), "target must be on the durable volume"
    assert "ENV AETHER_LAZY_INSTALL_TARGET=/opt/aether" not in text
    ...
    # succeed as the unprivileged aether runtime user.
    stage2 = (REPO_ROOT / "docker" / "stage2-hook.sh").read_text()
    assert '"$AETHER_HOME/lazy-packages"' in stage2, (
