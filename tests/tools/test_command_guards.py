...
def _clean_state():
    ...
    saved = {}
    for k in ("AETHER_INTERACTIVE", "AETHER_GATEWAY_SESSION", "AETHER_EXEC_ASK", "AETHER_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["AETHER_INTERACTIVE"] = "1"
        session_key = os.getenv("AETHER_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No AETHER_INTERACTIVE or AETHER_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
...
class TestGatewayApprovalAllowPermanent:
    ...
    def _capture_gateway_payload(self, command, session_key):
        ...
        os.environ["AETHER_SESSION_KEY"] = session_key
        try:
            check_all_command_guards(command, "local")
        finally:
            os.environ.pop("AETHER_GATEWAY_SESSION", None)
            os.environ.pop("AETHER_EXEC_ASK", None)
            os.environ.pop("AETHER_SESSION_KEY", None)
