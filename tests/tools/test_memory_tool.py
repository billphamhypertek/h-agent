...
class TestScanMemoryContent:
    ...
    def test_aether_config_mod_blocked(self):
        result = _scan_memory_content("edit .aether/config.yaml to change settings")
        assert "Blocked" in result
        assert "aether_config_mod" in result
        result = _scan_memory_content("update .aether/SOUL.md with new personality")
        assert "Blocked" in result
        assert "aether_config_mod" in result
    ...
    def test_aether_config_mod_no_false_positives(self):
        """Merely mentioning aether config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .aether/config.yaml for settings") is None
        assert _scan_memory_content("Read .aether/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .aether/config.yaml file contains runtime options") is None
