...
and the shared slash-command handlers (aether_cli/write_approval_commands.py).
...
@pytest.fixture
def aether_home(monkeypatch):
    d = tempfile.mkdtemp(prefix="aether_wa_test_")
    home = os.path.join(d, ".aether")
    os.makedirs(home)
    monkeypatch.setenv("AETHER_HOME", home)
    yield home
    shutil.rmtree(d, ignore_errors=True)


def _set_approval(subsystem, enabled):
    import aether_cli.config as cfg
    c = cfg.load_config()
...
def test_default_gate_is_off(aether_home):
...
def test_invalid_subsystem_is_off(aether_home):
    from tools import write_approval as wa
...
def test_memory_gate_off_allows_write(aether_home):
...
def test_memory_gate_on_no_interactive_stages(aether_home):
...
def test_memory_gate_on_then_apply(aether_home):
...
def test_cli_memory_approve_without_live_agent_uses_fresh_store(aether_home, capsys):
...
def test_load_on_disk_store_honors_configured_char_limits(aether_home, monkeypatch):
    ...
    monkeypatch.setattr("aether_cli.config.load_config", _boom)
...
def test_skill_gate_off_allows_create(aether_home):
...
def test_skill_gate_on_always_stages(aether_home):
...
def test_skill_gate_on_then_apply_writes_file(aether_home):
    # SKILLS_DIR is resolved at import time, so reload the skill module under
    # this test's AETHER_HOME to exercise the real on-disk write path.
    import importlib
    import tools.skill_manager_tool as smt
    importlib.reload(smt)
    from tools import write_approval as wa
...
def test_skill_create_diff_is_full_content(aether_home):
    from tools.skill_manager_tool import skill_manage
    from tools import write_approval as wa
    _set_approval("skills", True)
...
def test_pending_store_roundtrip(aether_home):
    from tools import write_approval as wa
...
def test_handle_pending_list_empty(aether_home):
...
def test_handle_approve_all(aether_home):
    from aether_cli.write_approval_commands import handle_pending_subcommand
...
def test_handle_reject(aether_home):
    from aether_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_handle_approval_on(aether_home):
    from aether_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
...
def test_handle_approval_off(aether_home):
    from aether_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
    out = handle_pending_subcommand(
...
def test_handle_mode_alias_still_works(aether_home):
...
def test_handle_approval_invalid(aether_home):
    from aether_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_handle_unknown_subcommand_returns_none(aether_home):
    from aether_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_memory_inline_approve_writes(aether_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(aether_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_callback_error_stages(aether_home, approval_callback_cleanup):
...
def test_gateway_context_stages_not_prompts(aether_home, monkeypatch):
    ...
    from tools import write_approval as wa
    _set_approval("memory", True)
    monkeypatch.setenv("AETHER_GATEWAY_SESSION", "1")
...
def test_skills_never_prompt_inline_even_with_callback(aether_home, approval_callback_cleanup):
...
def test_memory_invalid_params_rejected_before_staging(aether_home):
