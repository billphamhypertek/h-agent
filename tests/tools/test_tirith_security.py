...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._aether_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(aether_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(aether_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_aether_bin(self):
        """After _INSTALL_FAILED, manual install in AETHER_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        aether_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(aether_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(aether_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._aether_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == aether_bin
            assert _tirith_mod._resolved_path == aether_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# AETHER_HOME isolation
# ---------------------------------------------------------------------------

class TestAetherHomeIsolation:
    def test_aether_bin_dir_respects_aether_home(self):
        """_aether_bin_dir must use AETHER_HOME, not hardcoded ~/.aether."""
        from tools.tirith_security import _aether_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"AETHER_HOME": tmpdir}):
            result = _aether_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_aether_home(self):
        """_failure_marker_path must use AETHER_HOME, not hardcoded ~/.aether."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"AETHER_HOME": "/custom/aether"}):
            result = _failure_marker_path()
        assert result == "/custom/aether/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets AETHER_HOME; verify it's active."""
        aether_home = os.getenv("AETHER_HOME")
        assert aether_home is not None, "AETHER_HOME should be set by conftest"
        assert "aether_test" in aether_home, "Should point to test temp dir"

    def test_get_aether_home_fallback(self):
        """Without AETHER_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_aether_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove AETHER_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".aether")
            result = _get_aether_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._aether_bin_dir",
