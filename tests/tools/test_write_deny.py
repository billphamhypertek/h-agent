...
class TestWriteDenyExactPaths:
    ...
    def test_aether_env(self):
        # ``.env`` under the active AETHER_HOME (profile-aware, not just
        # ``~/.aether``) must be write-denied. The hermetic test conftest
        # points AETHER_HOME at a tempdir — resolve via get_aether_home()
        # to match the denylist.
        from aether_constants import get_aether_home
        path = str(get_aether_home() / ".env")
        assert _is_write_denied(path) is True

    def test_aether_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.aether/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "aether_root"
        ...
        # Sanity check: AETHER_HOME does point to the profile dir, not the root.
        from aether_constants import get_aether_home, get_default_aether_root
        assert get_aether_home() == profile_home
        assert get_default_aether_root() == root
...
class TestWriteAllowed:
    ...
    def test_aether_control_files_requested_writable(self):
        from aether_constants import get_aether_home
