...
Older images made /opt/aether/ui-tui and /opt/aether/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV AETHER_TUI_DIR=/opt/aether/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R aether:aether /opt/aether/ui-tui" not in text
    assert "chown -R aether:aether /opt/aether/node_modules" not in text
