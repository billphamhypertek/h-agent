"""Regression test for the ``AetherMCPOAuthProvider.async_auth_flow`` bidirectional
generator bridge.
...
@pytest.mark.asyncio
async def test_aether_provider_forwards_asend_values(tmp_path, monkeypatch):
    ...
    assert _AETHER_PROVIDER_CLS is not None, "SDK OAuth types must be available"
...
@pytest.mark.asyncio
async def test_aether_provider_forwards_401_triggers_refresh(tmp_path, monkeypatch):
