...
def test_aether_provider_subclass_exists():
    """AetherMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _AETHER_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _AETHER_PROVIDER_CLS is not None
    assert issubclass(_AETHER_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_aether_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns AetherMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _AETHER_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("AETHER_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _AETHER_PROVIDER_CLS is not None
    assert isinstance(provider, _AETHER_PROVIDER_CLS)
    assert provider._aether_server_name == "srv"
