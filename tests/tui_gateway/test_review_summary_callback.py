...
def server():
    with patch.dict(
        "sys.modules",
        {
            "aether_constants": MagicMock(
                get_aether_home=MagicMock(return_value="/tmp/aether_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'aether-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'aether-release' patched"
    }
