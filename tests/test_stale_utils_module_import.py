...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.aether/aether-agent/utils.py)
...
2. ``aether update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``aether_cli/main.py`` (the ``aether update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``aether_constants`` after the
