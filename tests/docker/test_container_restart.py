...
$AETHER_HOME/profiles/<name>/gateway_state.json on every boot and
...
def restart_container(request, built_image: str):
    ...
    preserves $AETHER_HOME/profiles/."""
    safe = request.node.name.replace("[", "_").replace("]", "_")
    name = f"aether-restart-{safe}"
    volume = f"aether-restart-vol-{safe}"
    _docker("rm", "-f", name)
...
def test_running_gateway_survives_container_restart(restart_container: str) -> None:
    ...
    r = _exec(container, "aether", "profile", "create", "coder")
    assert r.returncode == 0, f"profile create failed: {r.stderr}"

    r = _exec(container, "aether", "-p", "coder", "gateway", "start", timeout=60)
...
def test_stopped_gateway_stays_stopped_after_restart(restart_container: str) -> None:
    ...
    _exec(container, "aether", "profile", "create", "writer").check_returncode()
...
def test_stale_gateway_pid_cleaned_up_on_restart(restart_container: str) -> None:
    ...
    _exec(container, "aether", "profile", "create", "ghost").check_returncode()
...
def test_live_gateway_autostarts_after_real_restart_without_manual_state_stamp(
    ...
    _exec(container, "aether", "profile", "create", "live").check_returncode()
    r = _exec(container, "aether", "-p", "live", "gateway", "start", timeout=60)
    assert r.returncode == 0, f"gateway start failed: {r.stderr}"
    ...
    # operator `aether gateway stop`), so the shutdown is signal-initiated.
    _docker("restart", container, timeout=60).check_returncode()
