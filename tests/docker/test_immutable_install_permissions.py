...
def test_container_sets_hosted_write_policy_env(built_image: str) -> None:
    script = (
        'test "$AETHER_HOME" = "/opt/data" && '
        'test "$AETHER_WRITE_SAFE_ROOT" = "/opt/data" && '
        'test "$AETHER_DISABLE_LAZY_INSTALLS" = "1" && '
...
def test_aether_user_cannot_modify_install_but_can_write_data(built_image: str) -> None:
    script = textwrap.dedent(
        ...
        install_file = Path("/opt/aether/agent/message_sanitization.py")
        try:
