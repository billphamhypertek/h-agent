...
Override the image with ``AETHER_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("AETHER_TEST_IMAGE", "aether-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``AETHER_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("AETHER_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged aether user
# ---------------------------------------------------------------------------
#
# Background: every AETHER runtime path inside the container drops to UID
# 10000 (the ``aether`` user) via ``s6-setuidgid aether``. ``docker exec``
...
# fails (PermissionError swallowed) for aether, so a test that ran as root
...
# helpers below run every probe under ``-u aether`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: aether).
