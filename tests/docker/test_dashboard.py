"""Harness: dashboard opt-in via AETHER_DASHBOARD.

Today (tini): dashboard starts once when AETHER_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``aether`` user
...
def test_dashboard_not_running_by_default(
    ...
    """Without AETHER_DASHBOARD, no dashboard process should be running."""
    subprocess.run(
    ...
    time.sleep(5)
    r = docker_exec(container_name, "pgrep", "-f", "aether dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without AETHER_DASHBOARD"
    )
...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    """Without AETHER_DASHBOARD, s6-svstat should report the dashboard
    slot as DOWN (not up-with-sleep-infinity, which would
    false-positive `aether doctor` and any other health check).
    ...
    AETHER_DASHBOARD is unset, so the slot reflects reality.
    """
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name, built_image,
         "sleep", "60"],
    ...
    assert "down" in r.stdout, (
        f"Dashboard slot should be 'down' without AETHER_DASHBOARD; "
...
def test_dashboard_slot_reports_up_when_enabled(
    ...
    """Symmetry: with AETHER_DASHBOARD=1, s6-svstat reports the slot as up."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         ...
         # AETHER_DASHBOARD_OAUTH_CLIENT_ID in this test env), start_server
         ...
         "-e", "AETHER_DASHBOARD_BASIC_AUTH_USERNAME=admin",
         "-e", "AETHER_DASHBOARD_BASIC_AUTH_PASSWORD=test-dashboard-pw",
...
def test_dashboard_opt_in_starts(
    ...
    """With AETHER_DASHBOARD=1, a dashboard process should be visible."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "AETHER_DASHBOARD=1",
    ...
    ok, _ = _poll(
        container_name, "pgrep -f 'aether dashboard'", deadline_s=30.0,
    )
    assert ok, "Dashboard should be running with AETHER_DASHBOARD=1"


def test_dashboard_port_override(
    built_image: str, container_name: str,
) -> None:
    """AETHER_DASHBOARD_PORT changes the dashboard's listen port."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "AETHER_DASHBOARD=1", "-e", "AETHER_DASHBOARD_PORT=9120",
...
def test_dashboard_restarts_after_crash(
    ...
    for _attempt in range(10):
        first_pid_result = docker_exec(
            container_name, "pgrep", "-f", "aether dashboard",
        )
    ...
    # Kill the dashboard. The dashboard process runs as aether, so the
    # aether user can kill it (same UID).
...
# whenever `AETHER_DASHBOARD_HOST` was non-loopback, silently disabling
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    provider activates whenever ``AETHER_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "AETHER_DASHBOARD=1",
         "-e", "AETHER_DASHBOARD_HOST=0.0.0.0",
         "-e", "AETHER_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
         built_image, "sleep", "120"],
        check=True, capture_output=True, timeout=30,
    )
    ...
    assert "nous" in provider_names, (
        ...
        f"AETHER_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and AETHER_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``AETHER_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate
