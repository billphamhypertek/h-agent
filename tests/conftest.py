"""Shared fixtures for the aether-agent test suite.
...
2. **Isolated AETHER_HOME.** AETHER_HOME points to a per-test tempdir so
   code reading ``~/.aether/*`` via ``get_aether_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".aether"`` instead of the canonical
   ``get_aether_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No AETHER_SESSION_* inheritance** — the agent's current gateway
...
# AETHER_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_AETHER_BEHAVIORAL_VARS = frozenset({
    "AETHER_YOLO_MODE",
    "AETHER_INTERACTIVE",
    "AETHER_QUIET",
    "AETHER_TOOL_PROGRESS",
    "AETHER_TOOL_PROGRESS_MODE",
    "AETHER_MAX_ITERATIONS",
    "AETHER_SESSION_PLATFORM",
    "AETHER_SESSION_CHAT_ID",
    "AETHER_SESSION_CHAT_NAME",
    "AETHER_SESSION_THREAD_ID",
    "AETHER_SESSION_SOURCE",
    "AETHER_SESSION_KEY",
    "AETHER_GATEWAY_SESSION",
    "AETHER_CRON_SESSION",
    "_AETHER_GATEWAY",
    "AETHER_PLATFORM",
    "AETHER_MODEL",
    "AETHER_INFERENCE_MODEL",
    "AETHER_INFERENCE_PROVIDER",
    "AETHER_TUI_PROVIDER",
    "AETHER_MANAGED",
    "AETHER_MANAGED_DIR",
    "AETHER_DEV",
    ...
    "AETHER_TIMEZONE",
    "AETHER_REDACT_SECRETS",
    "AETHER_BACKGROUND_NOTIFICATIONS",
    "AETHER_EXEC_ASK",
    "AETHER_HOME_MODE",
    "AETHER_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.aether/kanban.db instead of the per-test AETHER_HOME.
    "AETHER_KANBAN_DB",
    "AETHER_KANBAN_BOARD",
    "AETHER_KANBAN_HOME",
    "AETHER_KANBAN_WORKSPACES_ROOT",
    "AETHER_KANBAN_LOGS_ROOT",
    "AETHER_KANBAN_TASK",
    "AETHER_KANBAN_WORKSPACE",
    "AETHER_KANBAN_RUN_ID",
    "AETHER_KANBAN_CLAIM_LOCK",
    "AETHER_KANBAN_DISPATCH_IN_GATEWAY",
    "AETHER_TENANT",
    ...
    "AETHER_DASHBOARD_OAUTH_CLIENT_ID",
    "AETHER_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and AETHER_HOME to per-test tempdirs so code that
    reads ``~/.aether/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral AETHER_* vars that could change test semantics.
    for name in _AETHER_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect AETHER_HOME to a per-test tempdir. Code that reads
    #    ``~/.aether/*`` via ``get_aether_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.aether/*`` via
    #    ``Path.home() / ".aether"`` instead of ``get_aether_home()``
    #    is a bug to fix at the callsite.
    fake_aether_home = tmp_path / "aether_test"
    fake_aether_home.mkdir()
    (fake_aether_home / "sessions").mkdir()
    (fake_aether_home / "cron").mkdir()
    (fake_aether_home / "memories").mkdir()
    (fake_aether_home / "skills").mkdir()
    monkeypatch.setenv("AETHER_HOME", str(fake_aether_home))
    ...
    #    ~/.aether/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import aether_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_aether_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal aether config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``aether-gateway`` process
...
# correlated with a ``tests/aether_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> aether-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart aether-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting aether/python patterns are also blocked.
    """
    ...
    def _matches_aether_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _AETHER_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_aether_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch aether-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m aether_cli.main".
                if (
                    "aether" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live aether-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting aether/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `aether update` (or the
        # equivalent `python -m aether_cli.main update`).  These commands
        ...
        if "update" in low and (
            # aether update / aether update --gateway / setsid bash -c ... aether update
            ("aether" in low and "update" in low.split())
            or
            # python -m aether_cli.main update --gateway
            ("aether_cli" in low and "update" in low.split())
            or
            # venv/bin/aether update  (absolute path variant used in tests)
            (".venv/bin/aether" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`aether update` against the real checkout, fetching "
