...
class TestToolsetConsistency:
    ...
    def test_aether_platforms_share_core_tools(self):
        """All aether-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on aether-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["aether-cli", "aether-telegram", "aether-discord", "aether-whatsapp", "aether-slack", "aether-signal", "aether-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_aether_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("aether-whatsapp")

    def test_aether_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("aether-api-server")
