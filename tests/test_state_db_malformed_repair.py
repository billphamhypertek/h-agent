...
def test_sessiondb_auto_heals_on_open(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(aether_state, "_repair_attempted_paths", set())
...
def test_auto_heal_attempted_once_per_process(tmp_path, monkeypatch):
    ...
    calls = {"n": 0}
    real_repair = aether_state.repair_state_db_schema
    ...
    monkeypatch.setattr(aether_state, "repair_state_db_schema", fake_repair)
    ...
    monkeypatch.setattr(aether_state, "repair_state_db_schema", real_repair)
...
def test_strategy_b_rebuild_when_dedup_insufficient(tmp_path, monkeypatch):
    ...
    # check and passes.
    real_check = aether_state._db_opens_cleanly
    calls = {"n": 0}
    ...
    monkeypatch.setattr(aether_state, "_db_opens_cleanly", flaky_check)
    report = repair_state_db_schema(db_path)
    monkeypatch.undo()
...
def test_non_malformed_error_is_not_auto_repaired(tmp_path, monkeypatch):
    ...
    called = {"n": 0}
    orig = aether_state.repair_state_db_schema

    def spy(*a, **kw):
        called["n"] += 1
        return orig(*a, **kw)

    monkeypatch.setattr(aether_state, "repair_state_db_schema", spy)
    with pytest.raises(sqlite3.DatabaseError):
        SessionDB(db_path=db_path)
