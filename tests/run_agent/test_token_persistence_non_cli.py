...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    aether_state = ModuleType("aether_state")
    aether_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "aether_state", aether_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "AETHER"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "AETHER"
