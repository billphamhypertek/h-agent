...
def test_aether_client_tag_includes_current_version():
    """The client tag must reflect aether_cli.__version__ verbatim."""
    from aether_cli import __version__
    from agent.portal_tags import aether_client_tag

    assert aether_client_tag() == f"client=aether-client-v{__version__}"


def test_aether_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import aether_client_tag

    tag = aether_client_tag()
    assert tag.startswith("client=aether-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import aether_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=aether-agent" in tags
    assert aether_client_tag() in tags
    assert len(tags) == 2
