...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. AETHER passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("AETHER_HOME", "/users/alice/.aether/profiles/backend-worker")
        monkeypatch.setenv("AETHER_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "AETHER_KANBAN_DB",
            "/users/alice/.aether/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.aether/kanban/boards/smoke"]'
            in cmd
        )
