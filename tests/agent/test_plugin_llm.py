...
""",
        ...
        from aether_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        aether_home = tmp_path / ".aether"
        aether_home.mkdir()
        (aether_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("AETHER_HOME", str(aether_home))
        from aether_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from aether_cli.plugins import PluginContext, PluginManifest, PluginManager
