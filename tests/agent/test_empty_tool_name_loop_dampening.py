...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated AETHER_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="aether_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".aether"))
    prev_home = os.environ.get("AETHER_HOME")
    os.environ["AETHER_HOME"] = os.path.join(test_home, ".aether")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("aether_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["AETHER_HOME"] = prev_home
