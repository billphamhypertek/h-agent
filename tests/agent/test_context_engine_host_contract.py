...
engine plugins (e.g. aether-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``aether plugins`` picker."""
    from aether_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "aether_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "aether_cli.plugins.get_plugin_context_engine",
