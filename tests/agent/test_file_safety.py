...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_aether_env(self):
        """AETHER' own .env inside AETHER_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: aether internal .env is in ~/.aether/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.aether/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.aether/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal AETHER cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        aether_home = tmp_path / ".aether"
        cache = aether_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._aether_home_path", return_value=aether_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal AETHER cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        aether_home = tmp_path / ".aether"
        hub = aether_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_aether_home(self, tmp_path):
        """The env basename guard does not depend on AETHER_HOME resolution."""
        aether_home = tmp_path / ".aether"
        aether_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        aether_home = tmp_path / ".aether"
        cache = aether_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
