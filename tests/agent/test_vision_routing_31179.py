...
def isolated_home(monkeypatch):
    """Temp AETHER_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="aether_test_31179_")
    aether_home = os.path.join(test_home, ".aether")
    os.makedirs(aether_home)
...
def _fresh_modules():
    """Drop cached aether modules so each test reloads against current env."""
