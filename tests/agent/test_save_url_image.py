...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate AETHER_HOME under tmp_path."""
    monkeypatch.setenv("AETHER_HOME", str(tmp_path / ".aether"))
    (tmp_path / ".aether").mkdir()

    # Force the constants/image cache helpers to re-read AETHER_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("aether_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_aether_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under AETHER_HOME — gateway cleanup
