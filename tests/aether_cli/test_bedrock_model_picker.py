...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (AETHER_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via AETHER_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in AETHER_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from aether_cli.providers import AETHER_OVERLAYS
        assert "bedrock" in AETHER_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from aether_cli.providers import AETHER_OVERLAYS
        assert AETHER_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from aether_cli.providers import AETHER_OVERLAYS
        assert AETHER_OVERLAYS["bedrock"].auth_type == "aws_sdk"
