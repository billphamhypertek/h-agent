...
aether_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `aether config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_aether_home = get_aether_home()`` at import time (module
    singleton), so monkeypatching AETHER_HOME after import doesn't move it.
    ...
    this only matters for tests that swap AETHER_HOME.)
    """
    import cli

    cli._aether_home = home
