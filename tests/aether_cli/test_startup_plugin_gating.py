...
``aether_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``aether --help``, ``aether version``,
``aether logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``aether --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from aether_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["aether", "--tui", "chat"], "chat"),
        (["aether", "-w", "logs"], "logs"),
        (["aether", "chat", "hello world"], "chat"),
        (["aether", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["aether", "-m", "gpt5", "chat"], "chat"),
        (["aether", "--model", "gpt5", "chat", "hi"], "chat"),
        (["aether", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["aether", "-z", "hello world"], None),
        (["aether", "-z", "hello", "chat"], "chat"),
        (["aether", "--model=gpt5", "chat"], "chat"),     # inline form
        (["aether", "--", "chat"], "chat"),               # -- terminator
        (["aether", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["aether", "some-plugin-cmd"], "some-plugin-cmd"),
        (["aether", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["aether"],                          # bare → chat
        ["aether", "--help"],                # top-level help
        ["aether", "-h"],
        ["aether", "version"],               # known built-in
        ["aether", "logs"],
        ["aether", "gateway", "run"],
        ["aether", "--tui"],
        ["aether", "-w", "--tui"],
        ["aether", "chat", "hi"],
        ["aether", "help"],                  # accepted built-in-ish
        ["aether", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["aether", "meet", "join"],          # potential google_meet plugin
        ["aether", "honcho", "status"],      # potential memory plugin
        ["aether", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``aether help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"aether_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
