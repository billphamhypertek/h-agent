"""Tests for IRC gateway configuration via `aether setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh AETHER install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new AETHER install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#aether")
            monkeypatch.setenv("IRC_NICKNAME", "aether-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `aether setup gateway` experience with IRC present."""
