"""Tests for Bug #12905 fix — stale OAuth token detection in aether model flow.

Bug 3: `aether model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.aether/.env but no valid
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr("aether_cli.secret_prompt.masked_secret_prompt", lambda _: "")

        from aether_cli.main import _model_flow_anthropic
        cfg = {}
