"""Tests for aether_cli.status model/provider display."""

from types import SimpleNamespace

from aether_cli.nous_account import NousPaidServiceAccessInfo, NousPortalAccountInfo
...
def _patch_common_status_deps(monkeypatch, status_mod, tmp_path, *, openai_base_url=""):
    ...
    monkeypatch.setattr(status_mod, "get_aether_home", lambda: tmp_path, raising=False)
...
def test_show_status_displays_configured_dict_model_and_provider_label(monkeypatch, capsys, tmp_path):
    from aether_cli import status as status_mod
...
def test_show_status_reports_managed_nous_features(monkeypatch, capsys, tmp_path):
    monkeypatch.setattr("aether_cli.status.managed_nous_tools_enabled", lambda: True)
    from aether_cli import status as status_mod
...
def test_show_status_hides_nous_subscription_section_when_feature_flag_is_off(monkeypatch, capsys, tmp_path):
    monkeypatch.setattr("aether_cli.status.managed_nous_tools_enabled", lambda: False)
    from aether_cli import status as status_mod
...
def test_show_status_reports_empty_lmstudio_listing_as_reachable(monkeypatch, capsys, tmp_path):
    ...
    monkeypatch.setattr(
        "aether_cli.models.probe_lmstudio_models",
