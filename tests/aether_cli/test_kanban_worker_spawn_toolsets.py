...
def test_default_spawn_pins_assignee_profile_cli_toolsets(monkeypatch, tmp_path):
    ...
    AETHER_KANBAN_TASK: the worker must not collapse to only kanban lifecycle
...
""".lstrip(),
    ...
    monkeypatch.setattr(kb, "_resolve_aether_argv", lambda: ["aether"])
    ...
    assert pid == 4242
    assert captured["env"]["AETHER_HOME"] == str(profile)
    assert captured["env"]["AETHER_KANBAN_TASK"] == "t_spawn_tools"
