"""Tests for --ignore-user-config and --ignore-rules flags on `aether chat`.
...
files. In AETHER the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.aether/config.yaml`` in ``load_cli_config()``
...
argparse → cmd_chat → cli.main() → AetherCLI → AIAgent call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("AETHER_IGNORE_USER_CONFIG", "AETHER_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``AETHER_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<aether_home>/config.yaml`` is
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._aether_home at tmp_path and return a fresh load_cli_config."""
        import cli
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "anthropic/claude-sonnet-4.6")
        monkeypatch.setenv("AETHER_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``AetherCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting AETHER_IGNORE_RULES=1 flips AetherCLI.ignore_rules True."""
        monkeypatch.setenv("AETHER_IGNORE_RULES", "1")

        # Import AetherCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of AetherCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.AetherCLI)
        # Replicate the exact logic from cli.py AetherCLI.__init__:
        ignore_rules = False  # constructor default
        obj.ignore_rules = ignore_rules or os.environ.get("AETHER_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True

    def test_constructor_flag_alone_enables_ignore_rules(self, monkeypatch):
        monkeypatch.delenv("AETHER_IGNORE_RULES", raising=False)
        import cli
        obj = object.__new__(cli.AetherCLI)
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``aether_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("AETHER_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("AETHER_IGNORE_RULES", raising=False)
    ...
    def test_only_ignore_user_config(self, monkeypatch):
        ...
        assert os.environ.get("AETHER_IGNORE_USER_CONFIG") == "1"
        assert "AETHER_IGNORE_RULES" not in os.environ
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "AETHER_IGNORE_USER_CONFIG" not in os.environ
        assert "AETHER_IGNORE_RULES" not in os.environ
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real aether parser accepts both flags."""
        ...
        import inspect
        import aether_cli.main as hm
        src = inspect.getsource(hm)
        assert "AETHER_IGNORE_USER_CONFIG" in src
        assert "AETHER_IGNORE_RULES" in src
