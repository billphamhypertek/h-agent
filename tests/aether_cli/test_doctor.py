...
import aether_cli.doctor as doctor
import aether_cli.gateway as gateway_cli
from aether_cli import doctor as doctor_mod
from aether_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `aether doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "AETHER_HOME", aether_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("AETHER_KANBAN_TASK", "probe")
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("AETHER_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_aether_home(self, tmp_path, provider=""):
        """Create a minimal AETHER_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_aether_home(tmp_path, provider)
        monkeypatch.setattr(doctor_mod, "AETHER_HOME", home)
        ...
        try:
            from aether_cli import auth as _auth_mod
...
def test_run_doctor_accepts_aether_provider_ids_that_catalog_aliases(
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from aether_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("aether_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "aether_cli.doctor", raising=False)
        from aether_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale AETHER_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        aether_home = tmp_path / ".aether"
        aether_home.mkdir(parents=True)
        (aether_home / "config.yaml").write_text(
            f"agent:\n  max_turns: {cfg_turns}\n", encoding="utf-8"
        )
        env_lines = ["OPENAI_API_KEY=sk-test\n"]
        if ghost is not None:
            env_lines.append(f"AETHER_MAX_ITERATIONS={ghost}\n")
        (aether_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "AETHER_HOME", aether_home)
        monkeypatch.setattr(doctor_mod, "get_aether_home", lambda: aether_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("AETHER_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("AETHER_MAX_ITERATIONS", raising=False)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale AETHER_MAX_ITERATIONS" in out
        env_after = (aether_home / ".env").read_text(encoding="utf-8")
        assert "AETHER_MAX_ITERATIONS" not in env_after
...
def test_npm_audit_fix_hint_avoids_crashing_workspace_flag(monkeypatch, tmp_path):
    """`aether doctor` must not hand users `npm audit fix --workspace <name>`:
    ...
    # left thinking a crashing command means a broken AETHER install.
