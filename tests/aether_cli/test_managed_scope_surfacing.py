"""Surfacing tests — managed scope shown in `config show` and `aether doctor`."""
import pytest
...
def test_config_show_flags_managed(homes, capsys):
    from aether_cli.config import show_config
...
def test_doctor_silent_with_no_managed_scope(tmp_path, monkeypatch, capsys):
    monkeypatch.setenv("AETHER_MANAGED_DIR", str(tmp_path / "nope"))
    from aether_cli import managed_scope, doctor
