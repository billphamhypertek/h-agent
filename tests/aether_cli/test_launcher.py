"""Tests for the top-level `./aether` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./aether` should use `aether_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "aether"
    called = []

    fake_main_module = types.ModuleType("aether_cli.main")

    def fake_main():
        called.append("aether_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "aether_cli.main", fake_main_module)
