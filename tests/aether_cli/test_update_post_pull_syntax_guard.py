"""Tests for the post-pull syntax guard in ``aether update``.

When a bad commit lands on ``main`` with a syntax error in a critical file
(e.g. orphan merge-conflict markers in ``aether_cli/config.py``), the CLI
becomes unbootable — every ``aether`` invocation imports those files at
...
markers in ``aether_cli/config.py``; users who ran ``aether update`` in
the 7-minute window before #28458 landed could not run any ``aether``
command afterward.
"""
...
def test_capture_head_sha_returns_stripped_sha(monkeypatch, tmp_path):
    ...
    assert aether_main._capture_head_sha(["git"], tmp_path) == "deadbeefcafe"
...
def test_capture_head_sha_returns_none_on_git_failure(monkeypatch, tmp_path):
    ...
    assert aether_main._capture_head_sha(["git"], tmp_path) is None
...
def _populate_critical_tree(root: Path, *, broken_file: str | None = None) -> None:
    ...
    for relpath in aether_main._UPDATE_CRITICAL_FILES:
        path = root / relpath
...
def test_validate_critical_files_syntax_ok_when_all_files_parse(tmp_path):
    ...
    ok, failing_path, error = aether_main._validate_critical_files_syntax(tmp_path)
...
def test_validate_critical_files_syntax_detects_conflict_markers(tmp_path):
    ...
    _populate_critical_tree(tmp_path, broken_file="aether_cli/config.py")

    ok, failing_path, error = aether_main._validate_critical_files_syntax(tmp_path)

    assert ok is False
    assert failing_path is not None and failing_path.endswith("aether_cli/config.py")
...
def test_validate_critical_files_syntax_detects_break_in_main_py(tmp_path):
    _populate_critical_tree(tmp_path, broken_file="aether_cli/main.py")

    ok, failing_path, _ = aether_main._validate_critical_files_syntax(tmp_path)

    assert ok is False
    assert failing_path is not None and failing_path.endswith("aether_cli/main.py")
...
def test_validate_critical_files_syntax_tolerates_missing_files(tmp_path):
    ...
    # Populate everything except aether_constants.py
    for relpath in aether_main._UPDATE_CRITICAL_FILES:
        if relpath == "aether_constants.py":
            continue
...
# release; if a future ``aether update`` would brick users, this test fails
# in CI first.
# ---------------------------------------------------------------------------
...
def test_production_tree_passes_syntax_guard():
    ...
    ok, failing_path, error = aether_main._validate_critical_files_syntax(repo_root)

    assert ok is True, (
        f"Critical-path file {failing_path} fails to parse on current main; "
        f"aether update would brick users. Error: {error}"
    )
