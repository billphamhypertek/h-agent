...
which broke ``aether --tui`` because ``ui-tui/`` had vanished.
...
from aether_cli.main import _atomic_replace_dir
...
def test_atomic_replace_swaps_content_on_success(tmp_path: Path) -> None:
    ...
    assert not (dst.parent / "ui-tui.aether-update-staging").exists()
    assert not (dst.parent / "ui-tui.aether-update-old").exists()
...
def test_atomic_replace_clears_stale_staging_leftovers(tmp_path: Path) -> None:
    ...
    stale_staging = dst.parent / "ui-tui.aether-update-staging"
    stale_backup = dst.parent / "ui-tui.aether-update-old"
    stale_staging.mkdir()
    stale_backup.mkdir()
