...
def test_user_env_overrides_stale_shell_values(tmp_path, monkeypatch):
    ...
    loaded = load_aether_dotenv(aether_home=home)
...
def test_project_env_overrides_stale_shell_values_when_user_env_missing(tmp_path, monkeypatch):
    ...
    loaded = load_aether_dotenv(aether_home=home, project_env=project_env)
...
def test_main_import_applies_user_env_over_shell_values(tmp_path, monkeypatch):
    ...
    home.mkdir()
    (home / ".env").write_text(
        "OPENAI_BASE_URL=https://new.example/v1\nAETHER_INFERENCE_PROVIDER=custom\n",
        encoding="utf-8",
    )
    ...
    sys.modules.pop("aether_cli.main", None)
    importlib.import_module("aether_cli.main")

    assert os.getenv("OPENAI_BASE_URL") == "https://new.example/v1"
    assert os.getenv("AETHER_INFERENCE_PROVIDER") == "custom"
