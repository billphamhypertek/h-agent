...
def isolated_kanban_home(monkeypatch):
    """Spin up a fresh AETHER_HOME with a clean kanban DB."""
    test_home = tempfile.mkdtemp(prefix="kanban_cli_passthrough_")
    os.makedirs(os.path.join(test_home, "profiles", "default"), exist_ok=True)
    monkeypatch.setenv("AETHER_HOME", test_home)
    for mod in list(sys.modules.keys()):
        if mod.startswith("aether_cli") or mod.startswith("aether_state") or mod == "aether_constants":
            del sys.modules[mod]
    yield test_home


def test_cli_dispatch_passes_max_in_progress_from_config(isolated_kanban_home, monkeypatch):
    """#33488: aether kanban dispatch must pass kanban.max_in_progress from
    ...
    monkeypatch.setattr(
        "aether_cli.config.load_config", lambda: fake_config
    )
...
def test_cli_max_flag_overrides_config_max_spawn(isolated_kanban_home, monkeypatch):
    ...
    monkeypatch.setattr("aether_cli.config.load_config", lambda: fake_config)
