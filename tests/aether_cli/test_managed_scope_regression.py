...
def aether_home(tmp_path, monkeypatch):
    ...
    # /etc/aether on the dev/CI box can't influence the test.
    monkeypatch.setenv("AETHER_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(aether_home, monkeypatch):
    from aether_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(aether_home, monkeypatch):
    from aether_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(aether_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_aether_dotenv(aether_home=str(home))
