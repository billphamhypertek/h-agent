...
Before the fix, ``aether whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``aether gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("AETHER_HOME", str(aether))
...
def _env_value(aether_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from aether_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("aether_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``aether whatsapp`` with an existing paired session and
