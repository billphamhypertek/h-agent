"""Tests for aether_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $AETHER_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the AETHER-managed node dir.
"""
...
def _make_aether_node(aether_home: Path) -> Path:
    """Create a fake $AETHER_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = aether_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_aether_node(fake_home):
    aether_home = fake_home / ".aether"
    node_bin = _make_aether_node(aether_home)
    local_bin = fake_home / ".local" / "bin"

    for name in ("node", "npm", "npx"):
        (local_bin / name).symlink_to(node_bin / name)

    removed = uninstall.remove_node_symlinks(aether_home)
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(aether_home) == []


def test_removes_dangling_symlink_into_aether_node(fake_home):
    """A link into the AETHER node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the AETHER links that exist; ignores the ones that don't."""
    aether_home = fake_home / ".aether"
    node_bin = _make_aether_node(aether_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are AETHER-managed; node is a real user binary.
