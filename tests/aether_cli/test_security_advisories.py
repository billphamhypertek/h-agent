"""Tests for aether_cli.security_advisories.
...
import aether_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect AETHER_HOME so banner cache and config writes are sandboxed."""
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full aether_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "aether_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "aether_cli.config.save_config",
...
class TestRendering:
    def test_short_banner_lines_includes_id_and_version(self, fake_advisory):
        ...
        assert "aether doctor" in joined
