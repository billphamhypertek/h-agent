...
def container_env(tmp_path, monkeypatch):
    """Set up a fake AETHER_HOME with .container-mode file."""
    aether_home = tmp_path / ".aether"
    aether_home.mkdir()
    monkeypatch.setenv("AETHER_HOME", str(aether_home))
    monkeypatch.delenv("AETHER_DEV", raising=False)

    container_mode = aether_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=aether-agent\n"
        "exec_user=aether\n"
        "aether_bin=/data/current-package/bin/aether\n"
    )
    return aether_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("aether_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "aether-agent"
    assert info["exec_user"] == "aether"
    assert info["aether_bin"] == "/data/current-package/bin/aether"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("aether_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_aether_dev(container_env, monkeypatch):
    """Returns None when AETHER_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("AETHER_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_aether_dev_zero(container_env, monkeypatch):
    """AETHER_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("AETHER_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        aether_home = Path(tmpdir) / ".aether"
        aether_home.mkdir()
        (aether_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("aether_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_aether_home": lambda: aether_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("AETHER_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=aether-custom\n"
        "exec_user=myuser\n"
        "aether_bin=/opt/aether/bin/aether\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "aether-custom"
    assert info["exec_user"] == "myuser"
    assert info["aether_bin"] == "/opt/aether/bin/aether"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from aether_cli.main import _exec_in_container
