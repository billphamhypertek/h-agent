"""Tests for ``aether debug`` CLI command and debug utilities."""
...
def aether_home(tmp_path, monkeypatch):
    """Set up an isolated AETHER_HOME with minimal logs."""
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO aether_cli.web_server: dashboard request\n"
    )
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from aether_cli.debug import _upload_paste_rs
        ...
        with patch("aether_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "aether_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from aether_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from aether_cli.debug import upload_to_pastebin

        with patch("aether_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("aether_cli.debug._upload_paste_rs",
             ...
             patch("aether_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, aether_home):
        from aether_cli.debug import _capture_log_snapshot
    ...
    def test_empty_primary_reports_file_empty(self, aether_home):
        """Empty primary (no .1 fallback) surfaces as '(file empty)', not missing."""
        (aether_home / "logs" / "agent.log").write_text("")
    ...
    def test_race_truncate_after_resolve_reports_empty(self, aether_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = aether_home / "logs" / "agent.log"
        from aether_cli import debug
    ...
    def test_truncates_large_file(self, aether_home):
        """Files larger than max_bytes get tail-truncated."""
        from aether_cli.debug import _capture_log_snapshot

        # Write a file larger than 1KB
        big_content = "x" * 100 + "\n"
        (aether_home / "logs" / "agent.log").write_text(big_content * 200)

        snap = _capture_log_snapshot("agent", tail_lines=10, max_bytes=1024)
        assert snap.full_text is not None
        assert "truncated" in snap.full_text

    def test_keeps_first_line_when_truncation_on_boundary(self, aether_home):
        ...
        (aether_home / "logs" / "agent.log").write_text(line * num_lines)
    ...
    def test_drops_partial_when_truncation_mid_line(self, aether_home):
    ...
    def test_unknown_log_returns_none(self, aether_home):
        from aether_cli.debug import _capture_log_snapshot
        snap = _capture_log_snapshot("nonexistent", tail_lines=10)
        assert snap.full_text is None

    def test_falls_back_to_rotated_file(self, aether_home):
        """When gateway.log doesn't exist, falls back to gateway.log.1."""
        from aether_cli.debug import _capture_log_snapshot

        logs_dir = aether_home / "logs"
    ...
    def test_prefers_primary_over_rotated(self, aether_home):
    ...
    def test_falls_back_when_primary_empty(self, aether_home):
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of AETHER_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def aether_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated AETHER_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".aether"
        home.mkdir()
        monkeypatch.setenv("AETHER_HOME", str(home))
        ...
        monkeypatch.delenv("AETHER_REDACT_SECRETS", raising=False)
    ...
    def test_default_redacts_tail_and_full_text(self, aether_home_with_secret):
        from aether_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, aether_home_with_secret):
        from aether_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, aether_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when AETHER_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("AETHER_REDACT_SECRETS", "false")

        from aether_cli.debug import _capture_log_snapshot

        assert os.environ.get("AETHER_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, aether_home_with_secret
    ):
        from aether_cli.debug import _capture_log_snapshot

        log_path = aether_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, aether_home_with_secret):
        from aether_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from aether_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, aether_home):
        from aether_cli.debug import collect_debug_report

        with patch("aether_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- aether dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- aether dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, aether_home):
        from aether_cli.debug import collect_debug_report

        with patch("aether_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)

        assert "--- agent.log" in report
        assert "session started" in report

    def test_report_includes_errors_log(self, aether_home):
        from aether_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, aether_home):
        from aether_cli.debug import collect_debug_report
    ...
    def test_report_includes_gui_log(self, aether_home):
        from aether_cli.debug import collect_debug_report
    ...
    def test_report_includes_desktop_log(self, aether_home):
        from aether_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, aether_home, capsys):
        ...
        with patch("aether_cli.dump.run_dump"), \
             patch("aether_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("aether_cli.debug.upload_to_pastebin",
    ...
    def test_share_survives_sweep_failure(self, aether_home, capsys):
        ...
        with patch("aether_cli.dump.run_dump"), \
             patch(
                 "aether_cli.debug._sweep_expired_pastes",
    ...
    def test_local_flag_prints_full_logs(self, aether_home, capsys):
    ...
    def test_share_uploads_five_pastes(self, aether_home, capsys):
        ...
        with patch("aether_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- aether dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
        ...
        assert "--- aether dump ---" in agent_paste
        assert "--- full agent.log ---" in agent_paste
        gateway_paste = uploaded_content[2]
        assert "--- aether dump ---" in gateway_paste
        assert "--- full gateway.log ---" in gateway_paste
        gui_paste = uploaded_content[3]
        assert "--- aether dump ---" in gui_paste
        assert "--- full gui.log ---" in gui_paste
        desktop_paste = uploaded_content[4]
        assert "--- aether dump ---" in desktop_paste
        assert "--- full desktop.log ---" in desktop_paste

    def test_share_keeps_report_and_full_log_on_same_snapshot(self, aether_home, capsys):
        """A mid-run rotation must not make full agent.log older than the report."""
        from aether_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("aether_cli.dump.run_dump"), \
             patch("aether_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("aether_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
    ...
    def test_share_continues_on_log_upload_failure(self, aether_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, aether_home, capsys):
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, aether_home_with_secret, capsys
    ):
        ...
        with patch("aether_cli.dump.run_dump"), \
             patch("aether_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("aether_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "aether debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, aether_home):
        from aether_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from aether_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from aether_cli.debug import delete_paste
        ...
        with patch("aether_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.aether/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``aether debug``
    invocation.
    """

    def test_does_not_spawn_subprocess(self, aether_home):
        ...
        import ast
        import inspect
        from aether_cli.debug import _schedule_auto_delete
    ...
    def test_records_pending_to_json(self, aether_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from aether_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_skips_non_paste_rs_urls(self, aether_home):
    ...
    def test_merges_with_existing_pending(self, aether_home):
        """Subsequent calls merge into existing pending.json."""
        from aether_cli.debug import _schedule_auto_delete, _load_pending
    ...
    def test_dedupes_same_url(self, aether_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, aether_home):
    ...
    def test_sweep_deletes_expired_entries(self, aether_home):
        from aether_cli.debug import (
        ...
        with patch("aether_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, aether_home):
        from aether_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("aether_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, aether_home):
    ...
    def test_sweep_drops_entries_past_grace_window(self, aether_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, aether_home):
        from aether_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("aether_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)

        mock_sweep.assert_called_once()

    def test_run_debug_survives_sweep_failure(self, aether_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: aether debug" in out


class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from aether_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("aether_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("aether_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, aether_home, capsys):
        from aether_cli.debug import run_debug_share
        ...
        with patch("aether_cli.dump.run_dump"), \
             ...
             patch("aether_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, aether_home, capsys):
        from aether_cli.debug import run_debug_share
        ...
        with patch("aether_cli.dump.run_dump"), \
             patch("aether_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("aether_cli.debug._schedule_auto_delete"):
            run_debug_share(args)

        out = capsys.readouterr().out
        assert "public paste service" in out

    def test_local_no_privacy_notice(self, aether_home, capsys):
        from aether_cli.debug import run_debug_share
...
class TestBuildDebugShare:
    ...
    Backs both ``aether debug share`` (CLI) and ``POST /api/ops/debug-share``
    (dashboard). The dashboard renders ``urls`` as real, copyable links, so the
    contract here is the return value, not stdout.
    """

    def test_returns_structured_urls(self, aether_home):
        from aether_cli.debug import build_debug_share, DebugShareResult
        ...
        with patch("aether_cli.dump.run_dump"), patch(
            "aether_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("aether_cli.debug._schedule_auto_delete"):
    ...
    def test_skips_missing_logs_without_failure(self, aether_home):
        from aether_cli.debug import build_debug_share

        # Remove desktop.log so it should be neither uploaded nor reported failed.
        (aether_home / "logs" / "desktop.log").unlink()

        with patch("aether_cli.dump.run_dump"), patch(
            "aether_cli.debug.upload_to_pastebin",
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, aether_home):
        from aether_cli.debug import build_debug_share
    ...
    def test_optional_log_failure_is_collected_not_raised(self, aether_home):
        from aether_cli.debug import build_debug_share
    ...
    def test_required_report_failure_raises(self, aether_home):
        from aether_cli.debug import build_debug_share
