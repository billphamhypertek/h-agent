"""Tests for the update check mechanism in aether_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    repo_dir = tmp_path / "aether-agent"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
    ...
    with patch("aether_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_invalidates_on_version_change(tmp_path, monkeypatch):
    ...
    """
    import aether_cli.banner as banner

    # No local git checkout -> the PyPI path is exercised (pip-install class).
    fake_banner = tmp_path / "aether_cli" / "banner.py"
    ...
    monkeypatch.setenv("AETHER_HOME", str(tmp_path))
    monkeypatch.delenv("AETHER_REVISION", raising=False)
    with patch("aether_cli.banner.subprocess.run") as mock_run, \
         patch("aether_cli.banner.check_via_pypi", return_value=0) as mock_pypi:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("AETHER_HOME", str(tmp_path))
    with patch("aether_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_official_ssh_origin_uses_https_probe(tmp_path):
    ...
    with patch("aether_cli.banner.subprocess.run", side_effect=fake_run):
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("AETHER_HOME", str(tmp_path))
    with patch("aether_cli.banner.subprocess.run") as mock_run:
        with patch("aether_cli.banner.check_via_pypi", return_value=0):
            result = banner.check_for_updates()
    assert result == 0
    mock_run.assert_not_called()
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when AETHER_HOME has no git repo."""
    import aether_cli.banner as banner
    ...
    # Point AETHER_HOME at a temp dir with no aether-agent/.git
    monkeypatch.setenv("AETHER_HOME", str(tmp_path))
    with patch("aether_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_docker_returns_none(tmp_path, monkeypatch):
    ...
    AETHER_REVISION (nix-only), so without a docker guard check_for_updates()
    ...
    container, and `aether update` correctly refusing to run there. The guard
    ...
    with patch("aether_cli.config.detect_install_method", return_value="docker"), \
         patch("aether_cli.banner.subprocess.run") as mock_run, \
         patch("aether_cli.banner.check_via_pypi") as mock_pypi:
        result = banner.check_for_updates()
...
def test_check_for_updates_non_docker_still_checks(tmp_path, monkeypatch):
    ...
    with patch("aether_cli.config.detect_install_method", return_value="pip"), \
         patch("aether_cli.banner.subprocess.run") as mock_run, \
         patch("aether_cli.banner.check_via_pypi", return_value=1) as mock_pypi:
        result = banner.check_for_updates()
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from aether_cli.main import _invalidate_update_cache

    # Build a fake ~/.aether with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"AETHER_HOME": str(default_home)}):
