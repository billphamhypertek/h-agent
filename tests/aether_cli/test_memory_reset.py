"""Tests for the `aether memory reset` CLI command.
...
- Profile-scoped reset (uses AETHER_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake AETHER_HOME with memory files."""
    aether_home = tmp_path / ".aether"
    memories = aether_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("AETHER_HOME", str(aether_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nAETHER repo is at ~/.aether/aether-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `aether memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        aether_home = tmp_path / ".aether"
        (aether_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("AETHER_HOME", str(aether_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's AETHER_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_aether_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
