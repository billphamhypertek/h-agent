...
When ``AETHER_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.aether/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.aether            (has its own auth.json fixture)
    * Profile     -> tmp_path/.aether/profiles/coder   (active, AETHER_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"AETHER is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from aether_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    aether_home = tmp_path / "classic"
    aether_home.mkdir()
    monkeypatch.setenv("AETHER_HOME", str(aether_home))

    _write(aether_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (AETHER_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.aether as the "real home" — which is NOT equal
    # to the AETHER_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    aether_home = tmp_path / "classic"
    ...
    _write(aether_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from aether_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: AETHER_HOME is set to a custom path that is NOT under
    # ~/.aether/profiles/ — get_default_aether_root() returns AETHER_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from aether_cli.auth import read_credential_pool, write_credential_pool
