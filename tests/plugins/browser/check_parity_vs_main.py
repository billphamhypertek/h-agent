...
Run from the PR worktree:
    ...
    cd ~/.aether/aether-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.aether/aether-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.aether/aether-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a aether-agent checkout"
)
...
# Isolated AETHER_HOME for the config write.
