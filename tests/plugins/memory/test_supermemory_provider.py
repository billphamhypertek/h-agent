...
def provider(monkeypatch, tmp_path):
    ...
    p.initialize("session-1", aether_home=str(tmp_path), platform="cli")
    return p
...
def test_format_prefetch_context_deduplicates_overlap():
    result = _format_prefetch_context(
        ...
        dynamic_facts=["Jordan prefers short answers", "Uses AETHER"],
        search_results=[{"memory": "Uses AETHER", "similarity": 0.9}],
        max_results=10,
    )
    assert result.count("Jordan prefers short answers") == 1
    assert result.count("Uses AETHER") == 1
...
def test_prefetch_includes_profile_on_first_turn(provider):
    provider._client.profile_response = {
        ...
        "search_results": [{"memory": "Working on AETHER memory provider", "similarity": 0.88}],
    }
...
def test_merge_metadata_stamps_sm_source():
    # sm_source routes AETHER writes into the "AETHER" Space in the Supermemory
    ...
    assert merged["sm_source"] == "aether"
    ...
    assert merged2["sm_source"] == "aether"
...
def test_identity_template_resolved_in_container_tag(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("SUPERMEMORY_API_KEY", "test-key")
    monkeypatch.setattr("plugins.memory.supermemory._SupermemoryClient", FakeClient)
    _save_supermemory_config({"container_tag": "aether-{identity}"}, str(tmp_path))
    p = SupermemoryMemoryProvider()
    p.initialize("s1", aether_home=str(tmp_path), platform="cli", agent_identity="coder")
    assert p._container_tag == "aether_coder"
...
def test_identity_template_default_profile(monkeypatch, tmp_path):
    ...
    _save_supermemory_config({"container_tag": "aether-{identity}"}, str(tmp_path))
    p = SupermemoryMemoryProvider()
    p.initialize("s1", aether_home=str(tmp_path), platform="cli")
    assert p._container_tag == "aether_default"
...
def test_multi_container_enabled_adds_schema_param(monkeypatch, tmp_path):
    ...
    assert p._allowed_containers == ["aether", "project_alpha", "shared"]
