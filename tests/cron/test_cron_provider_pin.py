...
resolve_runtime_provider against a temp AETHER_HOME) and the create_job
...
class TestCreateJobSnapshot:
    ...
    def test_pinned_job_skips_snapshot(self, monkeypatch):
        ...
        with patch("aether_cli.runtime_provider.resolve_runtime_provider", resolver):
            job = jobs.create_job(
    ...
    def test_unpinned_model_captures_model_snapshot(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "cron.jobs.get_aether_home", lambda: tmp_path, raising=True
        )
        with patch(
            "aether_cli.runtime_provider.resolve_runtime_provider",
            return_value={"provider": "openrouter"},
...
def _run_with_current_provider_and_model(job, current_provider, current_model, tmp_path):
    ...
    fake_db = MagicMock()
    with patch("cron.scheduler._aether_home", tmp_path), \
         patch("cron.scheduler._get_aether_home", return_value=tmp_path), \
         patch("cron.scheduler._resolve_origin", return_value=None), \
         patch("dotenv.load_dotenv"), \
         patch("aether_state.SessionDB", return_value=fake_db), \
         patch(
