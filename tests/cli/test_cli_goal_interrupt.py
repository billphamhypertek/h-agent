...
minimal ``AetherCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def aether_home(tmp_path, monkeypatch):
    """Isolated AETHER_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".aether"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("AETHER_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves AETHER_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal AetherCLI stub with an active goal wired in."""
...
class TestInterruptAutoPause:
    def test_interrupted_turn_pauses_goal_and_skips_continuation(self, aether_home):
        ...
        with patch("aether_cli.goals.judge_goal") as judge_mock:
    ...
    def test_interrupted_turn_is_resumable(self, aether_home):
        ...
        with patch("aether_cli.goals.judge_goal"):
            cli._maybe_continue_goal_after_turn()
...
class TestEmptyResponseSkip:
    def test_empty_response_does_not_invoke_judge(self, aether_home):
    ...
    def test_no_assistant_message_skipped(self, aether_home):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, aether_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, aether_home):
        ...
        src = inspect.getsource(AetherCLI.chat)
