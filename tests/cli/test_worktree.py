...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("aether/aether-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned aether/* and pr-* branches."""

    def test_prunes_orphaned_aether_branch(self, git_repo):
        """aether/aether-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "aether/aether-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "aether/aether-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "aether/aether-deadbeef" in result.stdout
        ...
        assert "aether/aether-deadbeef" in orphaned
        ...
        assert "aether/aether-deadbeef" not in result.stdout
