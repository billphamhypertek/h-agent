...
binary download are exercised manually by `aether secrets bitwarden
...
def aether_home(tmp_path, monkeypatch):
    """Point AETHER at an isolated home directory."""
    home = tmp_path / ".aether"
    home.mkdir()
    monkeypatch.setenv("AETHER_HOME", str(home))
    # Some modules cache get_aether_home; clear if needed.
    import aether_constants
    if hasattr(aether_constants, "_AETHER_HOME_CACHE"):
        aether_constants._AETHER_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(aether_home, monkeypatch):
...
def test_install_bws_happy_path(aether_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(aether_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(aether_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from aether_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under aether_home/cache/."""
    home = tmp_path / ".aether"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
