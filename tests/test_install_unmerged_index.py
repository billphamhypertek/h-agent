...
The ``aether update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m aether_cli.main gateway run`` — image name
    ``pythonw``, not ``aether.exe`` — so the ``taskkill /IM aether.exe`` guard
    ...
    # The aether.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM aether.exe' in text
