...
def _make_runner_with_adapter(session_id: str = None):
    ...
    # module-import time, defeating per-test AETHER_HOME monkeypatches).
...
@pytest.mark.asyncio
async def test_goal_verdict_done_sent_via_adapter_send(aether_home):
    ...
    with patch("aether_cli.goals.judge_goal", return_value=("done", "the feature shipped", False, None)):
...
@pytest.mark.asyncio
async def test_goal_verdict_continue_enqueues_continuation(aether_home):
    ...
    with patch("aether_cli.goals.judge_goal", return_value=("continue", "still needs work", False, None)):
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
...
@pytest.mark.asyncio
async def test_goal_verdict_budget_exhausted_sends_pause(aether_home):
    """When the budget is exhausted, a '⏸ Goal paused' message must be sent
    and no further continuation enqueued."""
    runner, adapter, session_entry, src = _make_runner_with_adapter()

    from aether_cli.goals import GoalManager, save_goal
    ...
    with patch("aether_cli.goals.judge_goal", return_value=("continue", "keep going", False, None)):
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
...
@pytest.mark.asyncio
async def test_goal_verdict_skipped_when_no_active_goal(aether_home):
...
@pytest.mark.asyncio
async def test_goal_verdict_survives_adapter_without_send(aether_home):
    ...
    with patch("aether_cli.goals.judge_goal", return_value=("done", "ok", False, None)):
        # must not raise
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
