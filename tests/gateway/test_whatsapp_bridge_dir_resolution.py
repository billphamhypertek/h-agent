...
(/opt/aether/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable AETHER_HOME location instead.
"""
import importlib
...
def test_writable_install_returns_install_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "aether_constants.get_aether_home", lambda: aether_home
    )

    resolved = whatsapp_common.resolve_whatsapp_bridge_dir()
    assert resolved == install_bridge
    # Nothing mirrored into AETHER_HOME.
    assert not (aether_home / "scripts" / "whatsapp-bridge").exists()


def test_readonly_install_mirrors_to_aether_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable AETHER_HOME."""
    install_root = tmp_path / "install"
    install_bridge = install_root / "scripts" / "whatsapp-bridge"
    _seed_install_tree(install_bridge)
    ...
    expected = aether_home / "scripts" / "whatsapp-bridge"
...
def test_readonly_install_reuses_existing_mirror(tmp_path, monkeypatch):
    """If the AETHER_HOME mirror already exists, return it without re-copying."""
    install_root = tmp_path / "install"
    install_bridge = install_root / "scripts" / "whatsapp-bridge"
    _seed_install_tree(install_bridge)

    aether_home = tmp_path / "aether_home"
    mirror = aether_home / "scripts" / "whatsapp-bridge"
