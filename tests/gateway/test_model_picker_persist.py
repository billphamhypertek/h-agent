...
closure the PR changed, against a real temp ``AETHER_HOME``.
"""
...
def _fake_switch_result():
    ...
    from aether_cli.model_switch import ModelSwitchResult
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "aether_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
    # save_config writes to ``get_aether_home() / config.yaml`` — point it here.
    monkeypatch.setattr("aether_constants.get_aether_home", lambda: aether_home)
    monkeypatch.setattr("aether_cli.config.get_aether_home", lambda: aether_home)
    return cfg_path
