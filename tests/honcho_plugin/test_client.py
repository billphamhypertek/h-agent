...
from aether_cli.profiles import _get_default_aether_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "aether"
        assert config.workspace_id == "aether"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.aether/honcho.json
        monkeypatch.setenv("AETHER_HOME", str(tmp_path / "isolated"))
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"aether": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"aether": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"aether": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="aether-agent"
        ):
            result = config.resolve_session_name("/home/user/aether-agent/subdir")
        assert result == "aether-agent"
...
class TestResolveConfigPath:
    def test_prefers_aether_home_when_exists(self, tmp_path):
        aether_home = tmp_path / "aether"
        aether_home.mkdir()
        local_cfg = aether_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: AETHER_HOME points at ~/.aether/profiles/<name>, so
        # _get_default_aether_home() must resolve back to ~/.aether — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".aether"
        ...
        assert _get_default_aether_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_aether_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".aether".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".aether"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "aether_coder"
        assert profile_host_key("default") == "aether"

    def test_default_returns_aether(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("AETHER_HONCHO_HOST", None)
            os.environ.pop("AETHER_HOME", None)
            assert resolve_active_host() == "aether"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"AETHER_HONCHO_HOST": "aether.coder"}):
            assert resolve_active_host() == "aether.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("AETHER_HONCHO_HOST", None)
            with patch("aether_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "aether_coder"

    def test_default_profile_returns_aether(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("AETHER_HONCHO_HOST", None)
            with patch("aether_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "aether"

    def test_custom_profile_returns_aether(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("AETHER_HONCHO_HOST", None)
            with patch("aether_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "aether"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("AETHER_HONCHO_HOST", None)
            # Temporarily remove aether_cli.profiles to simulate import failure
            saved = sys.modules.get("aether_cli.profiles")
            sys.modules["aether_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "aether"
            finally:
                if saved is not None:
                    sys.modules["aether_cli.profiles"] = saved
                else:
                    sys.modules.pop("aether_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="aether_coder")
        assert config.host == "aether_coder"
        assert config.workspace_id == "aether"  # shared workspace
        assert config.ai_peer == "aether_coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="aether")
        assert config.host == "aether"
        assert config.workspace_id == "aether"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "aether": {"aiPeer": "aether", "peerName": "alice"},
                "aether_coder": {
                    "aiPeer": "aether_coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="aether_coder", config_path=config_file,
        )
        assert config.host == "aether_coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "aether_dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="aether_dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "aether_dreamer"
    ...
    def test_from_global_config_reads_legacy_dot_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "aether.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        config = HonchoClientConfig.from_global_config(
            host="aether_dreamer",
        ...
        assert config.host == "aether_dreamer"
        assert config.peer_name == "dreamer-user"
        assert config.workspace_id == "aether_dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"aether": {"enabled": True, "aiPeer": "aether"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"aether": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_aether_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="aether",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("aether_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_defaults_to_30s_when_no_timeout_configured(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("aether_cli.config.load_config", return_value={}):
            client = get_honcho_client(cfg)
    ...
    def test_aether_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="aether",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("aether_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
