...
def test_root_session_no_compression(db):
    ...
    assert prov["currentAetherSessionId"] == "root1"
    assert prov["rootAetherSessionId"] == "root1"
    assert prov["parentAetherSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_aether_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentAetherSessionId"] == "old"
    assert prov["rootAetherSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousAetherSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootAetherSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootAetherSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_aether_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousAetherSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"aether"}
    assert "sessionProvenance" in meta["aether"]
    assert meta["aether"]["sessionProvenance"]["currentAetherSessionId"] == "root1"
