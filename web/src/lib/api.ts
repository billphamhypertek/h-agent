...
// (e.g. https://mission-control.tilos.com/aether/). The Python backend
// injects ``window.__AETHER_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__AETHER_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const AETHER_BASE_PATH = readBasePath();
const BASE = AETHER_BASE_PATH;
...
declare global {
  interface Window {
    ...
    __AETHER_AUTH_REQUIRED__?: boolean;
  }
}
let _sessionToken: string | null = null;
const SESSION_HEADER = "X-AETHER-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__AETHER_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``aether update``, ``aether gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__AETHER_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__AETHER_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("aether.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("aether.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__AETHER_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("aether.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__AETHER_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the AETHER dashboard server");
}

/**
 * Fetch a single-use ticket for a WebSocket upgrade in gated mode.
 *
 * The dashboard's gated-mode WS auth (``aether_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__AETHER_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-AETHER-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``aether_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__AETHER_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateAETHER: () =>
    fetchJSON<ActionResponse>("/api/aether/update", { method: "POST" }),
  checkAetherUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/aether/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** aether-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``aether update``. */
  can_update_aether?: boolean;
  config_path: string;
