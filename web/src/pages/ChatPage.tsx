/**
 * ChatPage — embeds `aether --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (aether_cli/web_server.py)                          .
 *          ▼                                                              .
...
import { AETHER_BASE_PATH, buildWsAuthParam } from "@/lib/api";
...
function buildWsUrl(
  ...
  // Profile-scoped chat: the PTY child gets AETHER_HOME pointed at the
  // selected profile, so the conversation runs with that profile's model,
  // skills, memory, and sessions (see web_server._resolve_chat_argv).
  if (profile) qs.set("profile", profile);
  return `${proto}//${window.location.host}${AETHER_BASE_PATH}/api/pty?${qs.toString()}`;
}
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__AETHER_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `aether dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__AETHER_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner AETHER TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[aether-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__AETHER_AUTH_REQUIRED__``) this
    ...
    // forwarding them into AETHER. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="aether-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
