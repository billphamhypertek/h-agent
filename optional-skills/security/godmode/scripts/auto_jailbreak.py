...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("AETHER_HOME", os.path.expanduser("~/.aether")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("AETHER_HOME", Path.home() / ".aether")) / "skills" / "red-teaming" / "godmode"
...
AETHER_HOME = Path(os.getenv("AETHER_HOME", Path.home() / ".aether"))
CONFIG_PATH = AETHER_HOME / "config.yaml"
PREFILL_PATH = AETHER_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/AETHER models — already uncensored, just needs clean prompt
    "aether": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "aether" in model_lower or "nous" in model_lower:
        return "aether"
...
def _get_current_model() -> tuple:
    """Read current model and provider from AETHER config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.aether/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart AETHER for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart AETHER for changes to take effect.")
