...
aether kanban watch --tenant <project-slug>

# Snapshot of the board
aether kanban list --tenant <project-slug>
aether kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
aether kanban stats --tenant <project-slug>

# Visual dashboard (browser)
aether dashboard

# Inspect a specific task (includes comments + events)
aether kanban show <task-id>

# Follow a single task's event stream
aether kanban tail <task-id>
```

Verify available subcommands with `aether kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `aether kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `aether kanban block <id>` then `aether kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `aether kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `aether kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `aether profile list` (profile exists?), `aether status` (gateway/dispatcher up?), and verify tenant. |
...
aether kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
aether kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(aether kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
aether kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
aether kanban block <task-id>
aether kanban archive <task-id>

# Diagnose what happened
aether kanban show <task-id>      # task body, comments, recent events
aether kanban tail <task-id>      # follow the live event stream
aether kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `aether kanban create` for the director
...
while true; do
    clear
    aether kanban list --tenant <slug>
    echo "---"
    aether kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `aether kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `aether profile list` and start
