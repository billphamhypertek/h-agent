...
the actual `setup.sh` that creates the project workspace, configures AETHER
...
2. **Create profiles** — `aether profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.aether/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `aether kanban create` assigned to the director
...
```bash
aether profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.aether/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.aether/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most AETHER
...
Each profile gets a `SOUL.md` at `~/.aether/profiles/<name>/SOUL.md` that
...
```bash
aether kanban create "Direct production of <video title>" \
    --assignee director \
...
the AETHER `.env` (`${AETHER_HOME:-$HOME/.aether}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _aether_env="${AETHER_HOME:-$HOME/.aether}/.env"
    if grep -q "^${var}=" "$_aether_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_aether_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_aether_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY aether ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY aether OPENROUTER_API_KEY || exit 1
# ...
```
