...
Maps each role archetype to the AETHER skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public aether-agent
...
## AETHER skills relevant to video production

### Visual / rendering skills (`aether-agent/skills/creative/`)
...
### Design / pre-production skills (`aether-agent/skills/creative/`)
...
### Audio / media skills (`aether-agent/skills/creative/` + `skills/media/`)
...
These are **not** AETHER skills but external CLIs / APIs that profiles invoke.
...
## Built-in AETHER tools for media review

These are native AETHER tools — not invoked via terminal but through their own
...
key is present in `${AETHER_HOME:-~/.aether}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every AETHER profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${AETHER_HOME:-~/.aether}/.env` → environment variable.
...
When a new AETHER-public video skill ships:
