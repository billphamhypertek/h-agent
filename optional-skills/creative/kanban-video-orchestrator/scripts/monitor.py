#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `aether kanban list` and
...
def aether_available() -> bool:
    return shutil.which("aether") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed aether CLI."""
    try:
        out = subprocess.run(
            ["aether", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `aether kanban list`
    out = subprocess.run(
        ["aether", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["aether", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not aether_available():
        print("ERROR: 'aether' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
