...
Usage (via cron with --no-agent):
    ...
    aether cron create aether-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$AETHER_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name aether-issues --repo NousResearch/hermes-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the AETHER .env file
(``${AETHER_HOME:-~/.aether}/.env``) to avoid the 60 req/hr
