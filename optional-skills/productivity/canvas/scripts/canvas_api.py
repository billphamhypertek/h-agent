...
def _check_config():
    ...
    if missing:
        aether_env = os.path.join(
            os.environ.get("AETHER_HOME", os.path.expanduser("~/.aether")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {aether_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for AETHER"
    )
