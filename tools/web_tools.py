...
Backend is selected during ``aether tools`` setup (web.backend in config.yaml).
When available, AETHER can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via AETHER config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through AETHER' config/.env layer (``aether config set``, ``aether tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.aether/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``aether tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `aether tools` repaint.
        try:
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `aether tools` to set one up."
                ),
            }
        else:
            logger.info(
