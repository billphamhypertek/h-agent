    ...
    if not pconfig or not pconfig.enabled:
        ...
        if platform_name == "weixin":
            ...
            if wx_token and wx_account:
                ...
                return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.aether/config.yaml or environment variables.")
        else:
            return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.aether/config.yaml or environment variables.")
    ...
    if not chat_id:
        ...
        if home:
            ...
            return json.dumps({
                ...
                f"or set a home channel via: aether config set {home_env} <channel_id>"
            })
    ...
    try:
        ...
        if isinstance(result, dict) and result.get("success") and mirror_text:
            try:
                ...
                source_label = get_session_env("AETHER_SESSION_PLATFORM", "cli")
...
def _get_cron_auto_delivery_target():
    ...
    from gateway.session_context import get_session_env
    platform = get_session_env("AETHER_CRON_AUTO_DELIVER_PLATFORM", "").strip().lower()
    chat_id = get_session_env("AETHER_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
    if not platform or not chat_id:
        return None
    thread_id = get_session_env("AETHER_CRON_AUTO_DELIVER_THREAD_ID", "").strip() or None
    return {
        "platform": platform,
        "chat_id": chat_id,
        "thread_id": thread_id,
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None, force_document=False):
    ...
    if platform == Platform.FEISHU and media_files:
        ...
        from aether_cli.plugins import discover_plugins as _dp_feishu
        _dp_feishu()
...
def _check_send_message():
    ...
    Also passes for kanban workers — the dispatcher sets ``AETHER_KANBAN_TASK``
    on every spawned worker, but those workers run with the assignee profile's
    ``AETHER_HOME`` which has no ``gateway.pid``, so the gateway-running check
...
#   - the ``aether send`` CLI command (aether_cli/send_cmd.py)
