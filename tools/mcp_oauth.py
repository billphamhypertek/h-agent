...
This module provides the glue:
    - ``AetherTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "AETHER"
"""
...
from aether_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__aether_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses AETHER_HOME so each profile gets its own OAuth tokens.
    Layout: ``AETHER_HOME/mcp-tokens/``
    """
    try:
        from aether_constants import get_aether_home
        base = Path(get_aether_home())
    except ImportError:
        base = Path(os.environ.get("AETHER_HOME", str(Path.home() / ".aether")))
...
# ---------------------------------------------------------------------------
# AetherTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class AetherTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        AETHER_HOME/mcp-tokens/<server_name>.json         -- tokens
        AETHER_HOME/mcp-tokens/<server_name>.client.json   -- client info
        AETHER_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # AETHER records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to AETHER.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://aether.hypertek.vn/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `aether mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "AETHER")
    scope = cfg.get("scope")
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`aether mcp login {server_name}` interactively first to complete "
