...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``aether tools`` repaint, tool-registration scans,
    ...
    2. ``~/.aether/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_aether_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.aether/.env`` first, then ``os.environ``.

    Wraps :func:`aether_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from aether_cli.config import get_env_value as _aether_get_env_value

        value = _aether_get_env_value(name)
...
def aether_xai_user_agent() -> str:
    """Return a stable AETHER-specific User-Agent for xAI HTTP calls."""
    try:
        from aether_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"AETHER-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers AETHER-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``aether_cli.config.get_env_value`` so keys
    stored in ``~/.aether/.env`` (the standard AETHER location) are honored —
