...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.aether",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``AETHER_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside AETHER_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    aether_home = _resolve_aether_home()

    # Reject absolute paths — they bypass the AETHER_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to AETHER_HOME)",
            relative_path,
        )
        return False

    host_path = aether_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape AETHER_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, aether_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.aether/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="aether-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching aether_constants.get_aether_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_aether_dir()`` for backward compatibility with old directory layouts.
    """
    from aether_constants import get_aether_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_aether_dir(new_subpath, old_name)
        if host_dir.is_dir():
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.aether``,
    SSH ``<remote_home>/.aether``, etc.) and whether translation is wanted.
