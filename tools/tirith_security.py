...
it is automatically downloaded from GitHub releases to $AETHER_HOME/bin/tirith.
...
def _get_aether_home() -> str:
    """Return the AETHER home directory, respecting AETHER_HOME env var."""
    return str(get_aether_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_aether_home(), ".tirith-install-failed")
...
def _aether_bin_dir() -> str:
    """Return $AETHER_HOME/bin, creating it if needed."""
    d = os.path.join(_get_aether_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $AETHER_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_aether_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $AETHER_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $AETHER_HOME/bin/tirith
    ...
    aether_bin = os.path.join(_aether_bin_dir(), "tirith")
    if os.path.isfile(aether_bin) and os.access(aether_bin, os.X_OK):
        _resolved_path = aether_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return aether_bin
