...
One ``CDPSupervisor`` runs per AETHER ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "aether-dialog-bridge.invalid"
...
(() => {
  if (window.__aetherDialogBridgeInstalled) return;
  window.__aetherDialogBridgeInstalled = true;
  const ENDPOINT = "http://aether-dialog-bridge.invalid/";
