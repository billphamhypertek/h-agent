...
`mcp_tool.py`, and `aether_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# AetherMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_aether_provider_class() -> Optional[type]:
    ...
    class AetherMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._aether_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`AetherTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, AetherTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._aether_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import AetherTokenStorage
                        if isinstance(storage, AetherTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._aether_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import AetherTokenStorage
            if not isinstance(storage, AetherTokenStorage):
                return
    ...
    return AetherMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_AETHER_PROVIDER_CLS: Optional[type] = _make_aether_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`AetherMCPOAuthProvider` directly using the helpers
        ...
        storage = AetherTokenStorage(server_name)

        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                "MCP OAuth for "
                f"'{server_name}': non-interactive environment and no "
                "cached tokens found. Run `aether mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``aether mcp remove <name>`` and (indirectly) by
        ``aether mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
