...
something different on each:
  ...
  * macOS — explicit TCC grants (Accessibility + Screen Recording). cua-driver
    ...
    the installed ``CuaDriver.app``), NOT AETHER — so no AETHER entitlement is
...
``aether computer-use permissions`` CLI, and ``/api/tools/computer-use/status``.
"""
...
def _child_env() -> Dict[str, str]:
    """cua-driver child env honoring the AETHER telemetry opt-in policy."""
    try:
