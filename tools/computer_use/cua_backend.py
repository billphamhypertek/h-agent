...
etc.) surface as specific blocked checks via `aether computer-use doctor`
...
# fetches the latest release, so a `AETHER_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `AETHER_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD = os.environ.get("AETHER_CUA_DRIVER_CMD", "cua-driver")
...
def _cua_telemetry_disabled() -> bool:
    """True when AETHER should disable cua-driver telemetry for this user.
...
def cua_driver_binary_available() -> bool:
    """True if `cua-driver` is on $PATH or AETHER_CUA_DRIVER_CMD resolves."""
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `aether computer-use install --upgrade`."
    )
...
def cua_driver_install_hint() -> str:
    ...
    return (
        ...
        f"{installer}\n"
        "Or run `aether tools` and enable the Computer Use toolset to install it automatically."
    )
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize AETHER-managed secrets out of the child env.
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self) -> None:
        ...
        #   - Gets a distinct agent-cursor color per AETHER run, with
        ...
        #     concurrent AETHER runs / subagents don't step on each
        #     other.
        # We mint a UUID4-based id once per CuaDriverBackend instance —
        # one AETHER run = one backend = one session — and pass it as
        ...
        self._session_id: str = f"aether-{uuid.uuid4().hex[:12]}"

    # ── Lifecycle ──────────────────────────────────────────────────
    def start(self) -> None:
        _maybe_nudge_update()
        # The MCP client SDK (`mcp`) is an optional dependency (the
        # `computer-use` / `mcp` extras), not part of AETHER' minimal core.
    ...
    def capture(self, mode: str = "som", app: Optional[str] = None) -> CaptureResult:
        ...
        Maps aether `capture(mode, app)` → cua-driver `list_windows` +
