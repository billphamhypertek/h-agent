...
Lets the LLM write a Python script that calls AETHER tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `aether_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `aether_tools.py` with file-based RPC stubs
...
# available on every platform AETHER itself runs on.
...
# match a safe prefix, the operational AETHER_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "AETHER_" prefix was deliberately removed (#27303) — it leaked
# AETHER_*-named config that lacks a secret substring (e.g. AETHER_BASE_URL,
# AETHER_KANBAN_DB, AETHER_*_WEBHOOK).  The child only needs the few
# location/profile vars in _AETHER_CHILD_ALLOWED below; AETHER_RPC_SOCKET /
# AETHER_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational AETHER_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set aether_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_AETHER_CHILD_ALLOWED = frozenset({
    "AETHER_HOME",
    "AETHER_PROFILE",
    "AETHER_CONFIG",
    "AETHER_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational AETHER_* vars (_AETHER_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret AETHER_* vars dropped by the tightened allowlist (#27303). The
    # broad "AETHER_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like AETHER_KANBAN_DB / AETHER_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if k.startswith("AETHER_"):
            ...
            # allowlist — a deliberately-dropped AETHER_* var.
            _dropped_aether.append(k)
    if _dropped_aether:
        logger.debug(
            "execute_code: dropped %d non-allowlisted AETHER_* var(s) from the "
...
# ---------------------------------------------------------------------------
# aether_tools.py code generator
# ---------------------------------------------------------------------------
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-AETHER-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-AETHER-profile soft guard."""',
...
def generate_aether_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the aether_tools.py stub module.
...
"""Auto-generated AETHER tools RPC stubs."""
...
def _connect():
    ...
    AETHER_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["AETHER_RPC_SOCKET"]
...
"""Auto-generated AETHER tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("AETHER_RPC_DIR") or os.path.join(tempfile.gettempdir(), "aether_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated aether_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/aether_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_aether_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/aether_tools.py", tools_src)
        ...
        env_prefix = (
            f"AETHER_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("AETHER_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # --- Set up temp directory with aether_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="aether_sandbox_")
    ...
    # selector from AETHER_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"aether_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated aether_tools module.
        ...
        tools_src = generate_aether_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "aether_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  AETHER_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["AETHER_RPC_SOCKET"] = rpc_endpoint
        ...
        # Ensure the aether-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from aether_tools import ...`` resolves even
        ...
        _pp_parts = [tmpdir, _aether_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # AETHER_TIMEZONE is an internal AETHER setting and must not leak
        # into child processes.
        _tz_name = os.getenv("AETHER_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("AETHER_TIMEZONE", None)

        from aether_constants import apply_subprocess_home_env
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.aether/.env')).
...
def _load_config() -> dict:
    ...
    ``aether --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (aether-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``aether tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and aether-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.aether/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call AETHER tools programmatically. "
        ...
        f"Available via `from aether_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into aether_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from aether_tools import {import_str}` "
