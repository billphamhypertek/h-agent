...
`aether skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.aether.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.aether.blueprint`` is a mapping containing
    ...
    aether = meta.get("aether") if isinstance(meta, dict) else None
    blueprint = aether.get("blueprint") if isinstance(aether, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.aether.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.aether.blueprint`` block) they can hand
    to ``aether skills publish`` to share. ``body`` is the plain-language
