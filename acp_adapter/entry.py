"""CLI entry point for the aether-agent ACP adapter.

Loads environment variables from ``~/.aether/.env``, configures logging
...
try:
    ...
    # shadowing AETHER's own modules — ``aether acp`` can be started from any
    # cwd, including a project that has same-named packages on its path.
    aether_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from AETHER_HOME (default ``~/.aether``)."""
    from aether_cli.env_loader import load_aether_dotenv

    aether_home = get_aether_home()
    loaded = load_aether_dotenv(aether_home=aether_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", aether_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="aether-acp",
        description="Run AETHER as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print AETHER version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive AETHER provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.aether/node/ "
...
def _print_version() -> None:
    ...
    print(aether_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import AetherACPAgent  # noqa: F401

    print("AETHER ACP check OK")


def _run_setup() -> None:
    from aether_cli.main import main as aether_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "aether", "model"]
        aether_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``aether postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting aether-agent ACP adapter")
    ...
    import acp
    from .server import AetherACPAgent
