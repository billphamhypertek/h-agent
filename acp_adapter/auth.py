"""ACP auth helpers — detect and advertise AETHER authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "aether-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active AETHER runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if AETHER can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for AETHER.
    ...
    not have AETHER provider credentials configured yet, so AETHER always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate AETHER using the currently configured "
