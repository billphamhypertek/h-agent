"""ACP agent server — exposes AETHER via the Agent Client Protocol."""
...
try:
    from aether_cli import __version__ as AETHER_VERSION
except Exception:
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so AETHER running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; AETHER cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a AETHER/OpenAI-compatible user content payload."""
...
class AetherACPAgent(acp.Agent):
    """ACP Agent implementation wrapping AETHER AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show AETHER version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so AETHER maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from aether_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from aether_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  AETHER estimates ``used`` from the same
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_aether_session_id: str,
        previous_aether_session_id: Optional[str] = None,
    ) -> Optional[dict]:
        """Best-effort ``_meta.aether.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(),
                acp_session_id,
                current_aether_session_id,
                previous_aether_session_id=previous_aether_session_id,
            )
        except Exception:
            logger.debug(
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_aether_session_id: Optional[str] = None,
        previous_aether_session_id: Optional[str] = None,
    ) -> None:
        """Send ACP native session metadata after AETHER changes it.

        When the internal AETHER head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_aether_session_id`` so the
        attached ``_meta.aether.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # aether_state.py schema — only started_at/ended_at). Use "now" as
        ...
        meta = self._provenance_meta(
            session_id,
            current_aether_session_id or session_id,
            previous_aether_session_id,
        )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["aether-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="aether-agent", version=AETHER_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # AETHER' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches AETHER setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes AETHER
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run AETHER on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive AETHER' local kawaii waiting/status
        ...
        # not the executor's. Also set AETHER_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue AETHER_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind AETHER_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("AETHER_INTERACTIVE")
            ...
            previous_session_id = os.environ.get("AETHER_SESSION_ID")
            os.environ["AETHER_SESSION_ID"] = session_id
            try:
                ...
                # Restore AETHER_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("AETHER_INTERACTIVE", None)
                else:
                    os.environ["AETHER_INTERACTIVE"] = previous_interactive
                # Restore AETHER_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("AETHER_SESSION_ID", None)
                else:
                    os.environ["AETHER_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal AETHER DB session id before the turn so we
            ...
            pre_turn_aether_id = getattr(state.agent, "session_id", None)
            # Wrap the executor call in a fresh copy of the current context so
            # concurrent ACP sessions on the shared ThreadPoolExecutor don't
            # stomp on each other's ContextVar writes (AETHER_SESSION_KEY in
        ...
        # _meta.aether.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_aether_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_aether_id
            and pre_turn_aether_id
            and post_turn_aether_id != pre_turn_aether_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_aether_session_id=post_turn_aether_id,
                    previous_aether_session_id=pre_turn_aether_id,
                )
            except Exception:
                logger.debug(
        ...
        # AETHER' local "waiting for model response" interrupt status is metadata,
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"AETHER v{AETHER_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when AETHER has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
