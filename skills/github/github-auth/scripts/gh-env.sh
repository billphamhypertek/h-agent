#!/usr/bin/env bash
# GitHub environment detection helper for AETHER skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _aether_env="${AETHER_HOME:-$HOME/.aether}/.env"; [ -f "$_aether_env" ] && grep -q "^GITHUB_TOKEN=" "$_aether_env" 2>/dev/null; then
