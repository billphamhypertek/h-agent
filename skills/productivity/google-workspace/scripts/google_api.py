#!/usr/bin/env python3
"""Google Workspace API CLI for AETHER.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing AETHER-facing JSON contract and falls back to the Python client
...
from _aether_home import get_aether_home
...
def _gws_binary() -> str | None:
    override = os.getenv("AETHER_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for AETHER")
