...
AETHER_HOME_DIR="${AETHER_HOME:-$HOME/.aether}"
AETHER_CFG="${AETHER_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure AETHER config has twozero_td MCP entry ──
if [[ ! -f "$AETHER_CFG" ]]; then
    echo -e " ${FAIL} AETHER config not found at ${AETHER_CFG}"
    manual_steps+=("Create ${AETHER_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$AETHER_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in AETHER config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to AETHER config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${AETHER_CFG} manually"); }
    manual_steps+=("Restart AETHER session to pick up config change")
fi
