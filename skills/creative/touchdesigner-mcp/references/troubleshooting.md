...
## 5. AETHER Configuration

### Config location

`$AETHER_HOME/config.yaml` (defaults to `~/.aether/config.yaml` when `AETHER_HOME` is unset)
...
Restart the AETHER session for changes to take effect. The MCP connection is
