"""
AETHER MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (AETHER-specific extra)

Usage:
    aether mcp serve
    aether mcp serve --verbose
...
logger = logging.getLogger("aether.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using AETHER_HOME."""
    try:
        from aether_constants import get_aether_home
        return get_aether_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("AETHER_HOME", Path.home() / ".aether")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from aether_constants import get_aether_home
        directory_file = get_aether_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("AETHER_HOME", Path.home() / ".aether")
...
class EventBridge:
    ...
    This is the AETHER equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from aether_constants import get_aether_home
            db_file = get_aether_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("AETHER_HOME", Path.home() / ".aether")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the AETHER MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "aether",
        instructions=(
            "AETHER messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the AETHER MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
