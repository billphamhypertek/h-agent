...
Optional AETHER observability plugin that maps AETHER observer hooks to
...
not replace AETHER's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets AETHER emit NeMo Relay lifecycle events for
the work AETHER already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, AETHER can:

- Preserve AETHER execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
aether plugins enable observability/nemo_relay
```

The `AETHER_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `AETHER_HOME` that the
agent run will use:

```bash
env AETHER_HOME=/tmp/aether-nemo-relay-test \
  aether plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`AETHER_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`AETHER_HOME` is the AETHER profile/config home used by both
`aether plugins enable ...` and the later `aether chat ...` run. If unset,
AETHER uses the user's default home, usually `~/.aether`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export AETHER_HOME=/tmp/aether-nemo-relay-test
aether plugins enable observability/nemo_relay
aether chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `aether` command you run is built from the
...
uv run aether plugins enable observability/nemo_relay
uv run aether chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/aether_agent-*.whl
...
The plugin can configure exporters directly from `AETHER_NEMO_RELAY_*`
...
```bash
export AETHER_NEMO_RELAY_ATOF_ENABLED=1
export AETHER_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export AETHER_NEMO_RELAY_ATIF_ENABLED=1
export AETHER_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `AETHER_NEMO_RELAY_ATOF_FILENAME`
- `AETHER_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `AETHER_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `AETHER_NEMO_RELAY_ATIF_AGENT_NAME`
- `AETHER_NEMO_RELAY_ATIF_AGENT_VERSION`
- `AETHER_NEMO_RELAY_ATIF_MODEL_NAME`
- `AETHER_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point AETHER at it:

```bash
export AETHER_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `AETHER_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`AETHER_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`AETHER_NEMO_RELAY_ATIF_*` fallback setup is also skipped so AETHER does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
AETHER keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, AETHER routes LLM and tool
...
For the full generic AETHER middleware contract, see
...
export AETHER_HOME=/tmp/aether-nemo-relay-docs/aether-home
mkdir -p "$AETHER_HOME"

cat > "$AETHER_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent AETHER session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export AETHER_NEMO_RELAY_ATOF_ENABLED=1
export AETHER_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/aether-nemo-relay-docs/subagent/atof
export AETHER_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export AETHER_NEMO_RELAY_ATOF_MODE=overwrite
export AETHER_NEMO_RELAY_ATIF_ENABLED=1
export AETHER_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/aether-nemo-relay-docs/subagent/atif
export AETHER_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export AETHER_NEMO_RELAY_ATIF_AGENT_NAME='AETHER E2E'
export AETHER_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export AETHER_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"aether.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "AETHER E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. AETHER dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/aether-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/aether-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/aether-nemo-relay-docs/workdir/beta.txt
cd /tmp/aether-nemo-relay-docs/workdir

export AETHER_NEMO_RELAY_ATOF_ENABLED=1
export AETHER_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/aether-nemo-relay-docs/parallel/atof
export AETHER_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export AETHER_NEMO_RELAY_ATOF_MODE=overwrite
export AETHER_NEMO_RELAY_ATIF_ENABLED=1
export AETHER_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/aether-nemo-relay-docs/parallel/atif
export AETHER_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export AETHER_NEMO_RELAY_ATIF_AGENT_NAME='AETHER E2E'
export AETHER_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- AETHER sessions map to `agent` scopes.
- AETHER API request hooks map to `llm` scope start/end events.
- AETHER tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses AETHER execution middleware to hand
...
Enable it for AETHER:

```bash
export AETHER_NEMO_RELAY_PLUGINS_TOML=/tmp/aether-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, AETHER routes execution
through these boundaries:

```text
AETHER provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> AETHER provider adapter next_call(...)

AETHER tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> AETHER tool dispatcher next_call(...)
```
...
middleware for a local AETHER run. This path requires a NeMo Relay runtime that
...
```bash
export AETHER_HOME=/tmp/aether-middleware-test/aether-home
mkdir -p "$AETHER_HOME" /tmp/aether-middleware-test/nemo-relay
...
cat > /tmp/aether-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/aether-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/aether-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "AETHER Middleware E2E"
agent_version = "local"
...
export AETHER_NEMO_RELAY_PLUGINS_TOML=/tmp/aether-middleware-test/nemo-relay/plugins.toml
