"""nemo_relay — optional AETHER plugin for NeMo Relay observability."""
...
class _Settings:
    ...
    atof_filename: str = "aether-atof.jsonl"
    atof_mode: str = "append"
    atif_enabled: bool = False
    atif_output_directory: str = ""
    atif_filename_template: str = "aether-atif-{session_id}.json"
    atif_subagent_export_mode: str = "embedded"
    atif_agent_name: str = "AETHER"
...
class _Runtime:
    def __init__(self, nemo_relay: Any, settings: _Settings) -> None:
        ...
        self._atof_subscriber_name = "aether.nemo_relay.atof"
    ...
    def ensure_session(self, kwargs: dict[str, Any]) -> _SessionState:
        ...
        if self.settings.atif_enabled and not self._plugins_toml_owns_exporter("atif"):
            state.atif_exporter = self.nemo_relay.AtifExporter(
                ...
                extra={"source": "aether-agent", "plugin": "observability/nemo_relay"},
            )
            state.atif_subscriber_name = f"aether.nemo_relay.atif.{session_id}"
    ...
    def mark_subagent_stop(self, kwargs: dict[str, Any]) -> None:
        ...
        self.mark("aether.subagent.stop", kwargs)
    ...
    def _run_managed_with_downstream_preservation(
        ...
        # execution so AETHER retry classification still sees it. The LLM and tool
...
def on_session_end(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: (runtime.mark("aether.session.end", kwargs), runtime.export_atif(runtime.ensure_session(kwargs))))
...
def on_pre_llm_call(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("aether.turn.start", kwargs))


def on_post_llm_call(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("aether.turn.end", kwargs))
...
def on_post_api_request(**kwargs: Any) -> None:
    ...
    def _record() -> None:
        ...
        if span is None:
            runtime.mark("aether.api.response.unmatched", kwargs)
            return
...
def on_api_request_error(**kwargs: Any) -> None:
    ...
    def _record() -> None:
        ...
        span = state.llm_spans.pop(_api_key(kwargs), None)
        if span is None:
            runtime.mark("aether.api.error", kwargs)
            return
        runtime.nemo_relay.llm.call_end(
            span,
...
def on_post_tool_call(**kwargs: Any) -> None:
    ...
    def _record() -> None:
        ...
        if span is None:
            runtime.mark("aether.tool.response.unmatched", kwargs)
            return
...
def on_pre_approval_request(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("aether.approval.request", kwargs))


def on_post_approval_response(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("aether.approval.response", kwargs))
...
def _load_settings() -> _Settings:
    plugins_toml_path = _env("AETHER_NEMO_RELAY_PLUGINS_TOML")
    ...
    return _Settings(
        ...
        atof_enabled=_env_bool("AETHER_NEMO_RELAY_ATOF_ENABLED"),
        atof_output_directory=_env("AETHER_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY"),
        atof_filename=_env("AETHER_NEMO_RELAY_ATOF_FILENAME") or "aether-atof.jsonl",
        atof_mode=_env("AETHER_NEMO_RELAY_ATOF_MODE") or "append",
        atif_enabled=_env_bool("AETHER_NEMO_RELAY_ATIF_ENABLED"),
        atif_output_directory=_env("AETHER_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY"),
        atif_filename_template=_env("AETHER_NEMO_RELAY_ATIF_FILENAME_TEMPLATE") or "aether-atif-{session_id}.json",
        atif_subagent_export_mode=_atif_subagent_export_mode(),
        atif_agent_name=_env("AETHER_NEMO_RELAY_ATIF_AGENT_NAME") or "AETHER",
        atif_agent_version=_env("AETHER_NEMO_RELAY_ATIF_AGENT_VERSION") or "unknown",
        atif_model_name=_env("AETHER_NEMO_RELAY_ATIF_MODEL_NAME") or "unknown",
    )
...
def _atif_subagent_export_mode() -> str:
    mode = _env("AETHER_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE").lower()
...
def _original_downstream_error(exc: Exception) -> BaseException:
    # AETHER wraps downstream execution failures in a local/private exception
...
def _is_relay_wrapped_callback_error(exc: Exception, callback_error: Exception | None) -> bool:
    ...
    # leading ``internal error:`` shape entirely, this returns False and AETHER
...
def _resolve_awaitable(value: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="aether-nemo-relay-awaitable",
        daemon=True,
    )
    thread.start()
    thread.join()
