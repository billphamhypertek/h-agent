"""
LINE Messaging API platform adapter for AETHER.
...
to AETHER. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``AETHER_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            aether_home = Path.home().joinpath(".aether").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``aether status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``aether status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``aether setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.aether/.env`` via ``aether_cli.config``.
    """
    print()
    ...
    try:
        from aether_cli.config import get_env_var, set_env_var
    except ImportError:
        print("aether_cli.config not available; set LINE_* vars manually in ~/.aether/.env")
        return
