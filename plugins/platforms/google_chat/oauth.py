...
- Per-user tokens (keyed by sender email):
    ``${AETHER_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${AETHER_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${AETHER_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${AETHER_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${AETHER_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's AETHER_HOME helper so the token follows the user's
# profile (e.g. tests can override via AETHER_HOME=/tmp/...).
try:
    from aether_constants import display_aether_home, get_aether_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where aether_constants isn't importable
    # (mirrors the same fallback used by the google-workspace skill's
    # _aether_home.py shim).
...
def _aether_home() -> Path:
    """Resolve AETHER_HOME at call time (NOT module import).

    Tests and ``AETHER_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.aether/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _aether_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _aether_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _aether_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _aether_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _aether_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install aether-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'aether-agent[google_chat]'")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into AETHER_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_aether_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_aether_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for AETHER (native attachment delivery)"
    )
