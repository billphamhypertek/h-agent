...
# ``model_tools`` import time, so ``aether status``, ``aether chat``, etc.
...
# ``aether_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "AETHER is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a aether MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${AETHER_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _aether_home = _get_aether_home()
        except (ModuleNotFoundError, ImportError):
            _aether_home = _Path.home() / ".aether"
        self._thread_count_store = _ThreadCountStore(
            _aether_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "AETHER is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("AETHER_HOME", str(_Path.home() / ".aether"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${AETHER_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a aether MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'AETHER is thinking…' marker message.
        ...
        "AETHER is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "AETHER is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "AETHER is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "AETHER is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "AETHER is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``aether setup``.

    The setup wizard at ``aether_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.aether/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. aether-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.aether/.env")
    print_info("Restart the gateway: aether gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``aether cron`` running as a
    separate process from ``aether gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the AETHER plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'aether-agent[google_chat]'",
        ...
        # the "AETHER is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'AETHER is thinking…' marker message "
