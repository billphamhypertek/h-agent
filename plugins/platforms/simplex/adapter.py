"""SimpleX Chat platform adapter (AETHER plugin).
...
This adapter ships as a AETHER platform plugin under
``plugins/platforms/simplex/``. The AETHER plugin loader scans the
...
Optional environment variables:
    ...
    AETHER_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``aether setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside AETHER — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("AETHER_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``aether setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.aether/.env`` via
    ``aether_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "aether_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.aether/.env"
        )
        return
