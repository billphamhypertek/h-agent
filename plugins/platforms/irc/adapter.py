"""
IRC Platform Adapter for AETHER.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the AETHER agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "aether-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :AETHER")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :AETHER shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: aether_, aether_1, aether_2...
...
def check_requirements() -> bool:
    ...
    # aether setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `aether gateway setup` flow for the IRC platform.

    Lazy-imports ``aether_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect AETHER to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #aether — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.aether/.env")
    print_info("Restart the gateway for changes to take effect: aether gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "aether-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "aether-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :AETHER (cron)")
