"""ntfy platform adapter (AETHER plugin).
...
``plugins/platforms/ntfy/``. The AETHER plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "aether-in"              # subscribe topic (incoming)
          publish_topic: "aether-out"     # optional — defaults to topic
...
_ECHO_TAG = "aether-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``aether cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a AETHER dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `aether gateway status` without
