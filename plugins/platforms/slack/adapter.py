...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_aether_home() / "slack_tokens.json"
        ...
        try:
            ...
            # AETHER does not currently need to route them into the agent loop.
            ...
            # /model work directly without /aether prefix). A single regex
            ...
            # manifest (see `aether slack manifest`). In Socket Mode, Slack
            ...
            from aether_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/aether$")

            @self._app.command(_slash_pattern)
            async def handle_aether_command(ack, command):
            ...
            for _action_id in (
                "aether_approve_once",
                "aether_approve_session",
                "aether_approve_always",
                "aether_deny",
            ):
            ...
            for _action_id in (
                "aether_confirm_once",
                "aether_confirm_always",
                "aether_confirm_cancel",
            ):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: AETHER handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own AETHER session — matching the per-thread behavior channels already
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        if original_text.startswith("!"):
            try:
                from aether_cli.commands import is_gateway_known_command

                first_token = original_text[1:].split(maxsplit=1)[0]
                # Strip "@suffix" the same way get_command() does, so
                # forms like ``!stop@aether`` still resolve.
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "aether_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "aether_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "aether_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "aether_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "aether_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "aether_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "aether_confirm_once": "once",
            "aether_confirm_always": "always",
            "aether_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "aether_approve_once": "once",
            "aether_approve_session": "session",
            "aether_approve_always": "always",
            "aether_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/aether <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/aether
        ...
        if slash_name in {"aether", ""}:
            # Legacy /aether <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from aether_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/aether   ``).
        ...
        # "Running /cmd…" ack shown by handle_aether_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/aether <question>" must produce public replies so
...
# ``aether_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``aether_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under AETHER_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from aether_cli.slack_cli import _build_full_manifest
            from aether_constants import get_aether_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="AETHER",
                bot_description="Your AETHER agent on Slack",
            )
            ...
            print_info(
                "   Re-run `aether slack manifest --write` anytime to refresh after "
                "AETHER adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `aether update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://aether.hypertek.vn/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``aether_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'aether-agent[slack]'",
        # Interactive setup wizard — replaces aether_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in aether_cli/gateway.py.
