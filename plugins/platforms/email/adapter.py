"""
Email platform adapter for the AETHER gateway.

Allows users to interact with AETHER by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "aether-agent" "version" "{_aether_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `aether config show` already read. Without the
    ...
    async def connect(self) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `aether gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "AETHER")
        ...
        msg_id = f"<aether-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
...
# aether_cli/gateway.py, and the _send_email dispatch in
