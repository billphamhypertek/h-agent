...
Credential storage mirrors every other AETHER channel:

    * runtime SDK creds  -> ``~/.aether/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.aether/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a aether dependency
...
# registers AETHER as its own client_id.
...
# Default name of the project AETHER provisions for the operator.
DEFAULT_PROJECT_NAME = "AETHER"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of aether-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.aether/auth.json`` honouring the active AETHER profile."""
    try:
        from aether_constants import get_aether_home
        return Path(get_aether_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.aether")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.aether/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.aether/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.aether/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: aether_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `aether photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `aether photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `aether photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `aether photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `aether photon setup`)"
