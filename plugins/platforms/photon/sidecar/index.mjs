// AETHER — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> AETHER): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (AETHER -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-AETHER-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `aether photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so AETHER can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply AETHER'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local AETHER allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of AETHER, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-aether-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when AETHER supplied them so a known-good
