...
Small Node helper that bridges AETHER to Photon's Spectrum SDK
(`spectrum-ts`).  AETHER is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-AETHER-Sidecar-Token`)
...
The AETHER plugin's `aether photon setup` command runs `npm install`
here automatically.
