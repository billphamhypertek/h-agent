...
This plugin connects AETHER to iMessage (and other Spectrum
...
TypeScript-only, AETHER runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-AETHER-Sidecar-Token`.
...
There is no separate `login` command; like every other AETHER channel,
...
Run `aether photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.aether/.env` (the same place every other
...
Management metadata lives in `~/.aether/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `aether photon telemetry on\|off` (restart the gateway to apply) |
...
5. Verify end-to-end: `aether photon status`, a DM and a group roundtrip,
