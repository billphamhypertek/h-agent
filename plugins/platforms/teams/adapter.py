"""
Microsoft Teams platform adapter for AETHER.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "AETHER"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        aether_action = data.get("aether_action", "")
        session_key = data.get("session_key", "")

        if not aether_action or not session_key:
        ...
        choice = choice_map.get(aether_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="aether_approve",
                    data={**btn_data_base, "aether_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="aether_approve",
                    data={**btn_data_base, "aether_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="aether_approve",
                    data={**btn_data_base, "aether_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="aether_approve",
                    data={**btn_data_base, "aether_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"AETHER\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.aether/.env")
