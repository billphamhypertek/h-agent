/**
 * AETHER Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.aether/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__AETHER_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__AETHER_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "aether-kanban-dot-triage",
    todo: "aether-kanban-dot-todo",
    ready: "aether-kanban-dot-ready",
    running: "aether-kanban-dot-running",
    blocked: "aether-kanban-dot-blocked",
    done: "aether-kanban-dot-done",
    archived: "aether-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-aether-task";
  ...
  const DOCS_URL = "https://aether.hypertek.vn/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://aether.hypertek.vn/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "aether.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="aether-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])aether-kanban-md-code\1/i.test(attrs)) {
      return ' class="aether-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "aether-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "aether-kanban-md",
  ...
  // standard `drop` event and our `aether-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("aether-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("aether-kanban-column--drop");
          if (target) target.classList.add("aether-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("aether-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("aether-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("aether-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__AETHER_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "aether-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "aether-kanban-attention",
        "aether-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "aether-kanban-attention-bar" },
        h("span", { className: "aether-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "aether-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "aether-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "aether-kanban-attention-row",
                  "aether-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "aether-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "aether-kanban-attention-row-id" }, task.id),
                h("span", { className: "aether-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "aether-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "aether-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "aether-kanban-diag-action-btn",
      action.suggested ? "aether-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " aether-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".aether-kanban-drawer-comment-row input, .aether-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "aether-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("aether-kanban-diag", sevClass) },
      h("div", { className: "aether-kanban-diag-header" },
        h("span", { className: "aether-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "aether-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "aether-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "aether-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "aether-kanban-diag-data-row" },
                  h("span", { className: "aether-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "aether-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "aether-kanban-diag-data-row" },
                h("span", { className: "aether-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "aether-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "aether-kanban-diag-reassign-row" },
            h("span", { className: "aether-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "aether-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "aether-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "aether-kanban-diag-msg",
              msg.ok ? "aether-kanban-diag-msg--ok" : "aether-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "aether-kanban-section" },
      h("div", { className: "aether-kanban-section-head-row" },
        h("span", { className: "aether-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "aether-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "aether-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "aether-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "aether-kanban-docs-link",
      title: "Open AETHER Kanban docs in a new tab",
      "aria-label": "AETHER Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `aether kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "aether-kanban-msg-ok" : "aether-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "aether-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "aether-kanban-boardswitcher" },
      h("div", { className: "aether-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "aether-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "aether-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "aether-kanban-dialog-title" },
        ...
        h("div", { className: "aether-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned AETHER profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "aether-kanban-bulk" },
      h("span", { className: "aether-kanban-bulk-count" },
      ...
      h("div", { className: "aether-kanban-bulk-priority",
      ...
      h("div", { className: "aether-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different AETHER profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "aether-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("aether-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("aether-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "aether-kanban-trash",
        dragOver ? "aether-kanban-trash--drop" : "",
        props.draggingTaskId ? "aether-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "aether-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "aether-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".aether-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "aether-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("aether-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("aether-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "aether-kanban-column",
        dragOver ? "aether-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "aether-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "aether-kanban-col-check",
        ...
        h("span", { className: cn("aether-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "aether-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "aether-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "aether-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "aether-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "aether-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "aether-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "aether-kanban-lane" },
                  h("div", { className: "aether-kanban-lane-head" },
                    h("span", { className: "aether-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "aether-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "aether-kanban-card--stale-red";
    if (age >= tier.amber) return "aether-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".aether-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "aether-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "aether-kanban-card",
        props.selected ? "aether-kanban-card--selected" : "",
        props.failed ? "aether-kanban-card--failed" : "",
        props.draggingSource ? "aether-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "aether-kanban-card-content" },
          h("div", { className: "aether-kanban-card-row" },
            h("label", {
              className: "aether-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "aether-kanban-card-check",
            ...
            h("span", { className: "aether-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or aether kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "aether-kanban-warning-badge",
                    "aether-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "aether-kanban-needs-assignee",
          ...
          h("div", { className: "aether-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "aether-kanban-card-row aether-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "aether-kanban-assignee",
                            title: `Assigned to AETHER profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "aether-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "aether-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "aether-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "aether-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "AETHER profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "AETHER profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "aether-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "aether-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "aether-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "aether-kanban-drawer-close",
        ...
        data ? h("div", { className: "aether-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "aether-kanban-section" },
      h("div", { className: "aether-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "aether-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "aether-kanban-drawer-body" },
      h("div", { className: "aether-kanban-drawer-title" },
        h("span", { className: cn("aether-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "aether-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "aether-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "aether-kanban-section" },
        h("div", { className: "aether-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "aether-kanban-section" },
        h("div", { className: "aether-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "aether-kanban-comment" },
                h("div", { className: "aether-kanban-comment-head" },
                  h("span", { className: "aether-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "aether-kanban-comment-ago" },
      ...
      h("div", { className: "aether-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "aether-kanban-event",
              isDiag ? "aether-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "aether-kanban-event-header" },
                  h("span", { className: "aether-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "aether-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "aether-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "aether-kanban-event-header-plain" },
                  h("span", { className: "aether-kanban-event-kind" }, e.kind),
                  h("span", { className: "aether-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "aether-kanban-event-phantom-row" },
                  h("span", { className: "aether-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "aether-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "aether-kanban-event-payload" },
    ...
    return h("div", { className: "aether-kanban-section" },
      h("div", { className: "aether-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "aether-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `aether-kanban-run--${r.outcome || r.status || "ended"}`
          : "aether-kanban-run--active";
        return h("div", { key: r.id, className: cn("aether-kanban-run", outcomeClass) },
          h("div", { className: "aether-kanban-run-head" },
            h("span", { className: "aether-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "aether-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "aether-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "aether-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "aether-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "aether-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "aether-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "aether-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "aether-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "aether-kanban-pre aether-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "aether-kanban-meta-row" },
      h("span", { className: "aether-kanban-meta-label" }, props.label),
      h("span", { className: "aether-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "aether-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "aether-kanban-meta-row" },
        h("span", { className: "aether-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "aether-kanban-meta-value aether-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "aether-kanban-meta-row" },
        h("span", { className: "aether-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "aether-kanban-meta-value aether-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "aether-kanban-section" },
      h("div", { className: "aether-kanban-section-head-row" },
        h("span", { className: "aether-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "aether-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "aether-kanban-section" },
      h("div", { className: "aether-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "aether-kanban-deps-row" },
        h("span", { className: "aether-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "aether-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "aether-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "aether-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "aether-kanban-dep-chip-x",
      ...
      h("div", { className: "aether-kanban-deps-row" },
        h("span", { className: "aether-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "aether-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "aether-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "aether-kanban-msg-ok"
          : "aether-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "aether-kanban-section" },
      ...
      h("div", { className: "aether-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "aether-kanban-home-sub aether-kanban-home-sub--on"
              : "aether-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__AETHER_PLUGINS__ && typeof window.__AETHER_PLUGINS__.register === "function") {
    window.__AETHER_PLUGINS__.register("kanban", KanbanPage);
  }
})();
