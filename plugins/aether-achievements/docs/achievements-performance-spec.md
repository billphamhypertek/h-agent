# AETHER Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: aether-achievements plugin
...
### `GET /api/plugins/aether-achievements/achievements`
...
### `POST /api/plugins/aether-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/aether-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/aether-achievements/overview`
...
- Persist plugin data under:
  - `~/.aether/plugins/aether-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.aether/plugins/aether-achievements/scan_checkpoint.json`
...
- `~/.aether/plugins/aether-achievements/`
