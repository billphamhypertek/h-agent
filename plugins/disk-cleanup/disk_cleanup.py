"""disk_cleanup — ephemeral file cleanup for AETHER.
...
Rules:
  ...
  - empty dirs    → always delete (under AETHER_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly AETHER_HOME and /tmp/aether-*
Never touches: ~/.aether/logs/ or any system directory.
"""
...
try:
    ...
    def get_aether_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("AETHER_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".aether").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$AETHER_HOME/logs/``."""
    return get_aether_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$AETHER_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under AETHER_HOME or ``/tmp/aether-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    aether_home = get_aether_home()
    try:
        path.resolve().relative_to(aether_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/aether-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("aether-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "aether-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $AETHER_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so AETHER_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        aether_home = get_aether_home()
        for parent in ("cron", "cronjobs"):
            base = aether_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside AETHER_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under AETHER_HOME, but never recurse into known
    # durable state trees.  Some installs place the AETHER checkout, venv,
    # and desktop build under AETHER_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    aether_home = get_aether_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in aether_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    aether_home = get_aether_home()
    try:
        rel = path.resolve().relative_to(aether_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "aether-agent",
        }:
            return None
        ...
        # Path isn't under AETHER_HOME (e.g. /tmp/aether-*) — fall through.
        pass
