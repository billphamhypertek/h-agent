...
Auto-tracks and cleans up ephemeral files created during AETHER
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$AETHER_HOME` and `/tmp/aether-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `AETHER_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `AETHER_HOME` or `/tmp/aether-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$AETHER_HOME/disk-cleanup/` is itself excluded
- `$AETHER_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
