...
session-scoped state file under ``$AETHER_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $AETHER_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_aether_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this aether install, leave it first —
    ...
    env = os.environ.copy()
    env["AETHER_MEET_URL"] = url
    env["AETHER_MEET_OUT_DIR"] = str(out)
    env["AETHER_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["AETHER_MEET_HEADED"] = "1"
    if auth_state:
        env["AETHER_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["AETHER_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if AETHER_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["AETHER_MEET_MODE"] = mode
    if realtime_model:
        env["AETHER_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["AETHER_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["AETHER_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["AETHER_MEET_REALTIME_KEY"] = realtime_api_key
