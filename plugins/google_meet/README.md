...
Let the aether agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where aether runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `aether meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `aether meet` CLI |
...
| `cli.py` | `aether meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$AETHER_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `aether meet node {run,list,approve,remove,status,ping}` |
...
aether meet install                                      # pip + Chromium
aether meet setup                                        # preflight
aether meet auth                                         # optional
aether meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
aether meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.aether/.env
aether meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
aether meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
aether meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.aether/.env
aether meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, aether will **not** switch your system audio input automatically — the
...
aether plugins enable google_meet
aether meet node run --display-name my-mac --host 0.0.0.0 --port 18789
