"""`aether meet node ...` subcommand tree.

Wired into the existing ``aether meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``aether meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="aether-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    ...
    if cmd == "run":
        ...
        print(f"             aether meet node approve <name> ws://<host>:{args.port} {token}")
        try:
