...
Output is saved as PNG under ``$AETHER_HOME/cache/images/``.
"""
...
class OpenAICodexImageGenProvider(ImageGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "env_vars": [],
            "post_setup_hint": (
                "Sign in with `aether auth codex` (or `aether setup` → Codex) "
        ...
        if not _read_codex_access_token():
            return error_response(
                error=(
                    ...
                    "`aether auth codex` (or `aether setup` → Codex) to sign in."
                ),
