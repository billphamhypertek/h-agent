...
``{title, url, description, position}`` rows every other AETHER web
provider produces.
...
prefers AETHER-managed xAI Grok OAuth (via ``aether auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.aether/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``aether tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `aether auth` to sign in with "
