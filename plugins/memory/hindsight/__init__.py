...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: aether)
...
Or via $AETHER_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so AETHER can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $AETHER_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_aether_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "aether": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "aether"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this AETHER config."""
    profile = config.get("profile", "aether")
    return str(profile or "aether")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active AETHER profile name (from agent_identity)
      {workspace} — AETHER workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``aether-{user}`` with no user becomes ``aether``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between AETHER and the User"
    ...
    def save_config(self, values, aether_home):
        """Write config to $AETHER_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(aether_home) / "hindsight"
    ...
    def post_setup(self, aether_home: str, config: dict) -> None:
        ...
        from aether_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        provider_config.setdefault("bank_id", "aether")
        ...
        self.save_config(provider_config, aether_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(aether_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(aether_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "aether"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: aether-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between AETHER and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "aether"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "aether"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "aether", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "aether")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between AETHER and the User")
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "memory daemon. Run AETHER as a non-root user, or switch "
                    "to cloud / local_external mode via 'aether memory setup'."
                )
                logger.warning(msg)
                # Surface to the terminal too — a daemon that never starts
                # would otherwise fail silently and the user would only see
                # AETHER get sluggish. (issue #13125)
                try:
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See aether-agent#6672.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When AETHER created/used that client on the shared async loop,
