...
```bash
aether memory setup    # select "mem0"
```
...
Behavioral settings live in `$AETHER_HOME/mem0.json` (set them via `aether memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.aether/.env`.

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `platform` | `platform` (Mem0 Cloud) or `oss` (self-hosted) |
| `user_id` | `aether-user` | User identifier on Mem0 |
...
```bash
aether memory setup mem0 --mode oss \
...
```bash
aether memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$AETHER_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.aether/mem0_qdrant"}}
  }
}
```
...
```bash
aether memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$AETHER_HOME/mem0.json`).
