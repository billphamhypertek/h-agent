...
Configuration
-------------
Secret (lives in $AETHER_HOME/.env or the environment):
  MEM0_API_KEY       — Mem0 Platform API key (required for platform mode)

Behavioral settings (live in $AETHER_HOME/mem0.json, set via `aether memory
setup`):
  ...
  agent_id           — Agent identifier (default: aether)
...
def _load_config() -> dict:
    """Load config from env vars, with $AETHER_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "aether"),
        "oss": {},
    }
    ...
    config_path = get_aether_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, aether_home):
        """Write config to $AETHER_HOME/mem0.json."""
        import json
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $AETHER_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "aether")
