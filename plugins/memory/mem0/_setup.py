...
def _save_mem0_json(aether_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(aether_home) / "mem0.json"
    existing = {}
...
def _setup_platform(aether_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "aether-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "aether"},
    ...
    provider.save_config(provider_config, aether_home)

    if env_writes:
        _write_env(Path(aether_home) / ".env", env_writes)
...
def _setup_oss(aether_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(aether_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "aether-user")
    ...
    if env_writes:
        _write_env(Path(aether_home) / ".env", env_writes)
    _save_mem0_json(aether_home, {"mode": "oss", "user_id": user_id, "agent_id": "aether", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, aether_home: str) -> str:
...
_PGVECTOR_CONTAINER = "aether-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "aether"
...
def _setup_oss_interactive(aether_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], aether_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], aether_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'aether-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "aether-user")

    agent_id = input("  Agent ID [aether]: ").strip()
    agent_id = agent_id or "aether"
    ...
    if env_writes:
        _write_env(Path(aether_home) / ".env", env_writes)
    _save_mem0_json(aether_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(aether_home: str, config: dict) -> None:
    """Entry point called by aether memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(aether_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(aether_home, config, flags)
        return
