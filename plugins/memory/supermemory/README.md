...
Config file: `$AETHER_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `aether` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `aether-{identity}` → `aether-coder`). |
...
All Supermemory API calls send `x-sm-source: aether`, and document writes stamp
`metadata.sm_source: aether`. This is a **functional routing key, not telemetry**:
it groups AETHER-written memories into a dedicated "AETHER" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per AETHER profile:

```json
{
  "container_tag": "aether-{identity}"
}
```

For a profile named `coder`, this resolves to `aether-coder`. The default profile resolves to `aether-default`. Without `{identity}`, all profiles share the same container.
