...
def _load_supermemory_config(aether_home: str) -> dict:
    config = _default_config()
    config_path = Path(aether_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, aether_home: str) -> None:
    config_path = Path(aether_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "aether"},
        )
    ...
    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        ...
        merged = {"sm_source": "aether", **(metadata or {})}
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `aether memory setup`.
        # All other options are documented for $AETHER_HOME/supermemory.json
    ...
    def save_config(self, values, aether_home):
        ...
        _save_supermemory_config(sanitized, aether_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from aether_constants import get_aether_home
        self._aether_home = kwargs.get("aether_home") or str(get_aether_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._aether_home)
