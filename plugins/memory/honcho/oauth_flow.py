...
``aether://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the AETHER OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "aether-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``aether-desktop`` / ``aether-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to AETHER.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local AETHER_HOME override can't reach.
