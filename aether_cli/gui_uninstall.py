"""
AETHER Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``aether desktop`` / ``aether gui``)
     Built inside the agent checkout under ``$AETHER_HOME/aether-agent/``:
       ...
       - ``$AETHER_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/AETHER.app`` or ``~/Applications/AETHER.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\AETHER`` (NSIS per-user)
...
the app name ("AETHER"), separate from ``$AETHER_HOME``:
  - macOS:   ``~/Library/Application Support/AETHER``
  - Windows: ``%APPDATA%\\AETHER``
  - Linux:   ``$XDG_CONFIG_HOME/AETHER`` (default ``~/.config/AETHER``)
...
``aether uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(aether_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return aether_home / "aether-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "AETHER"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "AETHER"
...
def source_built_gui_artifacts(aether_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``aether desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``aether-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(aether_home)
    ...
    return [
        ...
        aether_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "AETHER" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/AETHER.app"),
            home / "Applications" / "AETHER.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\AETHER).
            local_base / "Programs" / "AETHER",
            # Older / alternate layout some builds used.
            local_base / "aether-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "AETHER")
    else:
        ...
        paths += [
            data_base / "applications" / "aether.desktop",
            data_base / "applications" / "AETHER.desktop",
        ]
    return paths


def agent_is_installed(aether_home: Path) -> bool:
    """Return True when a usable Python agent install exists under AETHER_HOME.
    ...
    if (agent_root / "aether_cli").is_dir():
        return True
...
def gui_is_installed(aether_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(aether_home):
        if p.exists():
            return True
...
def gui_install_summary(aether_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = aether_home if aether_home is not None else get_aether_home()
...
def uninstall_gui(aether_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``aether-agent/aether_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$AETHER_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove aether' or "
            "'sudo dnf remove aether'). AppImage builds are a single file you "
