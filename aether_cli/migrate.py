"""CLI handlers for ``aether migrate ...``.

Currently exposes only ``aether migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``aether migrate <subtype>``."""
    sub = getattr(args, "migrate_type", None)
    if sub == "xai":
        return cmd_migrate_xai(args)

    print("usage: aether migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `aether migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `aether doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0
