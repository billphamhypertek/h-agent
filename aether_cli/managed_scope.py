...
A system-level directory (default ``/etc/aether``, root-owned and not
user-writable) supplies ``config.yaml`` and ``.env`` values that WIN over the
user's ``~/.aether/config.yaml`` and ``~/.aether/.env`` on a per-leaf-key basis.

This is DISTINCT from ``aether_cli.config.is_managed()`` / ``AETHER_MANAGED``,
...
_DEFAULT_MANAGED_DIR = Path("/etc/aether")
...
def _under_pytest() -> bool:
    ...
    Used to ignore the system default ``/etc/aether`` during tests so a real
    managed scope on a developer/CI box can't leak policy into the suite. Tests
    that exercise managed scope set ``AETHER_MANAGED_DIR`` explicitly, which is
...
def get_managed_dir() -> Optional[Path]:
    ...
    Resolution (highest priority first):
      1. ``$AETHER_MANAGED_DIR`` — deployment/bootstrap path override (IT-only;
         never persisted to any .env). Honored only when set to a non-empty value
         AND the directory exists.
      2. ``/etc/aether`` — POSIX default, when it exists. Ignored under pytest so
    ...
    """
    override = os.environ.get("AETHER_MANAGED_DIR", "").strip()
    if override:
        p = Path(override)
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through aether_cli.config.load_config) to honor managed
    scope. Mirrors aether_cli.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from aether_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
