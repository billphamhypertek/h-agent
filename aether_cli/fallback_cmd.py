"""
aether fallback — manage the fallback provider chain.
...
Subcommands:
  aether fallback [list]   Show the current fallback chain (default when no subcommand)
  aether fallback add      Pick provider + model via the same picker as `aether model`,
                           then append the selection to the chain
  aether fallback remove   Pick an entry to delete from the chain
  aether fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.aether/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from aether_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `aether model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  aether fallback add")
        print()
        return
    ...
    print("  Docs: https://aether.hypertek.vn/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `aether model`, then append the selection to the chain."""
    from aether_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `aether model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `aether fallback list` to view, or `aether fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``aether fallback [subcommand]``."""
