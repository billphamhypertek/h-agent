"""On-demand supply-chain audit for AETHER installs.

Scans three surfaces a AETHER user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The AETHER venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.aether/plugins``
...
def _discover_plugins(aether_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.aether/plugins``.
...
def run_audit(
    *,
    ...
    home = aether_home or Path(get_aether_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, aether_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(aether_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `aether security audit`."""
    home = Path(get_aether_home())
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, aether_home=home
    )
    if total == 0:
