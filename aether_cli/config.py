...
- ~/.aether/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.aether/.env         - API keys and secrets

This module provides:
- aether config          - Show current configuration
- aether config edit     - Open config in editor
- aether config set      - Set a specific value
- aether config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``aether config set`` (which rewrites
    ...
    ``config.yaml`` in place: aether never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.aether/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``aether logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  aether config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` AETHER makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. AETHER itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``aether update``, the TUI build.
...
#   on every plugin install / ``aether update``.
...
# * ``AETHER_HOME`` / ``AETHER_PROFILE`` / ``AETHER_CONFIG`` /
#   ``AETHER_ENV`` — AETHER runtime location flags. Writing these into
...
#
# IMPORTANT: ``AETHER_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (AETHER_LANGFUSE_PUBLIC_KEY,
# AETHER_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # AETHER runtime location — never via dashboard env writer.
    # NOT a AETHER_* blanket: integration credentials (AETHER_GEMINI_*,
    # AETHER_LANGFUSE_*, AETHER_SPOTIFY_*, ...) ARE allowed.
    "AETHER_HOME", "AETHER_PROFILE", "AETHER_CONFIG", "AETHER_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or AETHER runtime location "
            "(AETHER_HOME, AETHER_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "AETHER_TOOL_PROGRESS", "AETHER_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `aether plugins enable
    # observability/langfuse` or `aether tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "AETHER_LANGFUSE_ENV",
    "AETHER_LANGFUSE_RELEASE",
    "AETHER_LANGFUSE_SAMPLE_RATE",
    "AETHER_LANGFUSE_MAX_CHARS",
    "AETHER_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from aether_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("AETHER_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_aether_home() / ".managed"
...
def is_managed() -> bool:
    """Check if AETHER is running in package-manager-managed mode.

    Two signals: the AETHER_MANAGED env var (set by the systemd service),
    or a .managed marker file in AETHER_HOME (set by the NixOS activation
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``aether_cli/`` — i.e. the git checkout for source
    installs, ``/opt/aether`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$AETHER_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how AETHER was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$AETHER_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. AETHER_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.aether``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$AETHER_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.aether:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$AETHER_HOME``, a
    ...
    ``docker`` and ``aether update`` refuses to run ("doesn't apply inside the
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $AETHER_HOME.
    try:
    ...
    #    host install whose shared $AETHER_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``aether update``.
    try:
        method = (
            (get_aether_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``aether_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$AETHER_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/aether`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* AETHER lives in a ``uv tool`` layout.

    ``uv tool install aether-agent`` places the install at
    ``.../uv/tools/aether-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``aether-agent`` happens to be uv-tool-installed on the machine while
    the *active* AETHER is a regular pip/venv install, causing
    ``aether update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/aether-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "pip":
        ...
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade aether-agent"
        return "pip install --upgrade aether-agent"
    return "aether update"
...
# Long-form text for ``aether update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# aether_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side AETHER, it doesn't update
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``aether update`` inside Docker.
...
def format_managed_message(action: str = "modify this AETHER installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("AETHER_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this AETHER installation is managed by NixOS "
            f"(AETHER_MANAGED={env_hint}).\n"
            "Edit services.aether-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this AETHER installation is managed by Homebrew "
            f"(AETHER_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade aether-agent"
        )

    return (
        f"Cannot {action}: this AETHER installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall AETHER."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from AETHER_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, aether_bin
    or None if container mode is not active, we're already inside the
    container, or AETHER_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("AETHER_DEV") == "1":
        return None
    ...
    container_mode_file = get_aether_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "aether-agent")
    exec_user = info.get("exec_user", "aether")
    aether_bin = info.get("aether_bin", "/data/current-package/bin/aether")
...
# Re-export from aether_constants — canonical definition lives there.
from aether_constants import get_aether_home  # noqa: F811,E402
...
def _resolve_aether_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the AETHER_UID / AETHER_GID env vars set by Docker deployments.

    Docker containers running AETHER commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level AETHER_HOME once, but
    subdirectories created at runtime by ``ensure_aether_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("AETHER_UID", "").strip()
    gid_str = os.environ.get("AETHER_GID", "").strip()
    try:
...
def _chown_to_aether_uid(path) -> None:
    """Chown ``path`` to ``AETHER_UID:AETHER_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_aether_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_aether_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the aether group can
    share state with the gateway service.

    The mode can be overridden via the AETHER_HOME_MODE environment variable
    (e.g. AETHER_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse AETHER_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``AETHER_UID``/``AETHER_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("AETHER_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_aether_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When AETHER runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("AETHER_CONTAINER") or os.environ.get("AETHER_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set AETHER_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into AETHER_HOME if the user doesn't have one yet."""
...
def ensure_aether_home():
    """Ensure ~/.aether directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_aether_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_aether_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"AETHER_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the AETHER-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps AETHER
        ...
        # identity slot (SOUL.md). Empty by default. The AETHER_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, AETHER adds a coding operating brief
    ...
    "terminal": {
        ...
        #             AETHER_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force AETHER_HOME/home when it exists (old strict
        ...
        # skipped. When empty, AETHER auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), AETHER sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when AETHER
        ...
        # bundled AETHER image, which drops to the `aether` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, AETHER sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and AETHER should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``aether chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.aether/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: aether sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .aether.md, or .cursorrules before AETHER applies
    ...
    # single read_file page exceeds these limits, AETHER truncates the
    ...
    # See tools/kanban_tools.py and aether_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``aether dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``aether kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``aether kanban decompose`` and the kanban
        ...
        # ``aether profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `aether model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `aether` (and `aether chat`) launches by default:
        ...
        # REPL and `--tui` (or AETHER_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `aether --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `aether -c` muscle memory.  Default off so existing
        # users aren't surprised.  AETHER_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `aether --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/aether`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `aether pets`. Disabled until
        ...
        "pet": {
            ...
            # get_aether_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``AETHER_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``AETHER_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``AETHER_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # Public URL override (env: ``AETHER_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/aether``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; AETHER appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.aether/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.aether/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, AETHER feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before AETHER auto-pauses the goal and
    ...
    # always goes to ~/.aether/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${AETHER_SKILL_DIR} and ${AETHER_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `aether curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `aether curator run --consolidate` overrides
        ...
        # normally restored on every `aether update`, so pruning them only
        ...
        # skipped), snapshot ~/.aether/skills/ into
        # ~/.aether/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `aether curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for aether-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "∧ *AETHER*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (AETHER_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace AETHER' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.aether/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or AETHER_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `aether doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``aether_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow AETHER to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $AETHER_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via AETHER_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `aether -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `aether` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `aether kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     aether-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core AETHER tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.aether/logs/.
    ...
    # update model picker lists without shipping a aether-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://aether.hypertek.vn/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `aether model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.aether/.env,
        ...
        # behavior: files must live under the AETHER cache, under
        ...
        # user. Bridged to AETHER_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the AETHER
        # cache (~/.aether/cache/{documents,images,audio,video,screenshots})
        ...
        # to AETHER_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # AETHER_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to AETHER_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.aether/state.db.
    ...
    "sessions": {
        ...
        # default of ``aether sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.aether/sessions/session_{sid}.json`` on every turn
    ...
    # ``aether update`` behaviour.
    "updates": {
        # Run a full ``aether backup``-style zip of AETHER_HOME before every
        # ``aether update``.  Backups land in ``<AETHER_HOME>/backups/`` and
        # can be restored with ``aether import <path>``.  Defaults to true
        # after the #48200 incident: a ``aether update --yes`` run that
        ...
        "backup_keep": 5,
        # What `aether update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<AETHER_HOME>/lsp/bin/`` on first use.
    ...
    # `aether tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.aether/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.aether/.env (or your shell) and never in config.yaml.
            ...
            # ~/.aether/bin/ on first use.  When False you must install
            ...
            # `aether secrets bitwarden setup`.
            "server_url": "",
        },
    },
    ...
    "computer_use": {
        ...
        # by default upstream. AETHER disables it for our users unless they
        # explicitly opt in here. When false (default), AETHER sets
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'aether model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the AETHER auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use AETHER, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @aether:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. AETHER_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #aether)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: aether-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'aether-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote AETHER API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote AETHER API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote AETHER API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # AETHER_TOOL_PROGRESS and AETHER_TOOL_PROGRESS_MODE are deprecated —
    ...
    "AETHER_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.aether.config`` entries, then checks
    ...
    try:
        ...
        # should never break `aether update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older AETHER versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``aether_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'aether setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — AETHER won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'aether doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("AETHER_HOME", "~/.aether")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("AETHER_TOOL_PROGRESS")
            old_mode = get_env_value("AETHER_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from AETHER_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from AETHER_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("AETHER_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from AETHER_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$AETHER_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_aether_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `aether plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `aether curator status` has no
    ...
    #   3. Creates `~/.aether/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_aether_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_aether_home() / "logs" / "curator"
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from aether_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: aether config set <key> <value>")
    ...
    # metadata.aether.config in their SKILL.md frontmatter.
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``aether config set``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _aether_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.aether/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.aether/config.yaml.
    ...
    (which change ``AETHER_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/aether/config.yaml invalidates the
...
#   openai-codex (OAuth — aether auth) — OpenAI Codex
#   nous         (OAuth — aether auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.aether/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.aether/.env.
    ...
    interactive menu render (`aether tools`, `aether setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `aether tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real AETHER env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.aether/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.aether/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.aether/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.aether/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to AETHER — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known AETHER vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.aether/.env or environment."""
...
def show_config():
    ...
    print(color("│              ∧ AETHER Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from aether_cli.auth import get_anthropic_key
    ...
    # Warn on stale AETHER_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("AETHER_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale AETHER_MAX_ITERATIONS={_env_ghost} "
                f"(run 'aether doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  aether config edit     # Edit config file", Colors.DIM))
    print(color("  aether config set <key> <value>", Colors.DIM))
    print(color("  aether setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `aether config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `aether config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: aether config set <key> <value>")
            print()
            print("Examples:")
            print("  aether config set model anthropic/claude-sonnet-4")
            print("  aether config set terminal.backend docker")
            print("  aether config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'aether config migrate' to add them")
        ...
        print("  aether config           Show current configuration")
        print("  aether config edit      Open config in editor")
        print("  aether config set <key> <value>   Set a config value")
        print("  aether config check     Check for missing/outdated config")
        print("  aether config migrate   Update config with new options")
        print("  aether config path      Show config file path")
        print("  aether config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``aether config`` UI so users
