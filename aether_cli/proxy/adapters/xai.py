...
# xAI's public API is OpenAI-compatible for the endpoints AETHER commonly
# uses. The Responses endpoint is included because AETHER' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via AETHER-managed OAuth credentials."""

    auth_hint = "aether auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`aether auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`aether auth reset xai-oauth` or re-authenticate with "
                    "`aether auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `aether auth add xai-oauth --type oauth`."
            )
