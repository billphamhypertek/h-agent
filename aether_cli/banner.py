...
Pure display functions with no AetherCLI state dependency.
"""
...
from aether_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built aether — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/NousResearch/hermes-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/aether-agent"
...
def _fetch_pypi_latest(package: str = "aether-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a AETHER update is available.

    Two paths: if ``AETHER_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = aether_home / ".update_check"
    embedded_rev = os.environ.get("AETHER_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # AETHER_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `aether update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/aether/update/check`
    ...
    if embedded_rev:
        ...
        # $AETHER_HOME/aether-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = aether_home / "aether-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active AETHER git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$AETHER_HOME/aether-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``aether_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from aether_cli.build_info import get_build_sha
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    AETHER checkout. Cached per-process. Release URL always points at the
...
def format_banner_version_label() -> str:
    ...
    base = f"AETHER v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else AETHER_CADUCEUS
    except Exception:
        _bskin = None
        _hero = AETHER_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    try:
        from aether_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from aether_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built aether; we know an update
    ...
    # Pip-install warning — `pip install aether-agent` is not the supported
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else AETHER_AGENT_LOGO
        console.print(_logo)
        console.print()
