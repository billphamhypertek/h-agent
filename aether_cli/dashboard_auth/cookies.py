...
Three cookies in play:
  ...
  - aether_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/aether) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/aether`` covers
...
setter wrote ``__Secure-aether_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "aether_session_at"
SESSION_RT_COOKIE = "aether_session_rt"
PKCE_COOKIE = "aether_session_pkce"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /aether`` we want ``Path=/aether`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    issues a 24h rotating refresh token (aether #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/aether``)
