"""``aether debug`` debug tools for AETHER.

Currently supports:
    aether debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.aether/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[aether debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.aether/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``aether debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``aether debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, AETHER version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `aether debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "aether-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``aether debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.aether/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``aether debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "aether-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----AetherDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from aether_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_aether_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``aether dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``aether dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``aether debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "aether debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `aether debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print(f"To delete now:  aether debug delete <url>")

    print(f"\nShare these links with the AETHER team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: aether debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'aether debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``aether debug`` call.
    ...
    # best-effort — any failure is swallowed so ``aether debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: aether debug <command>")
        print()
        print("Commands:")
