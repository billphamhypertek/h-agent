...
The AETHER docs site hosts a JSON manifest of curated models for providers
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.aether/cache/model_catalog.json``.
...
def _cache_path() -> Path:
    ...
    return get_aether_home() / "cache" / "model_catalog.json"
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``aether update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.aether/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``aether model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
