"""
Dump command for aether CLI.

Outputs a compact, plain-text summary of the user's AETHER setup
...
from aether_cli.config import get_aether_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.aether_build_sha`` by the Dockerfile's
    ``AETHER_GIT_SHA`` build-arg (see ``aether_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``aether dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from aether_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(aether_home: Path) -> int:
...
def _cron_summary(aether_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = aether_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"aether_home:      {display_aether_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `aether auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `aether auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["aether-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(aether_home)}")
    lines.append(f"  skills:             {_count_skills(aether_home)}")
