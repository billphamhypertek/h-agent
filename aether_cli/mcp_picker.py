"""MCP picker — interactive `aether mcp picker` (also the default `aether mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``aether mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `aether plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from aether_cli.cli_output import prompt_yes_no
from aether_cli.curses_ui import curses_single_select
from aether_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new AETHER session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from aether_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `aether mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: aether mcp install <name>    Picker: aether mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their AETHER is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer AETHER — run `aether update` "
...
def show_catalog() -> None:
    """`aether mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`aether mcp picker` (and default `aether mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`aether mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `aether mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
