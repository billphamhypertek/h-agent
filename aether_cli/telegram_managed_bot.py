...
manual BotFather token copy-paste. AETHER talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.aether.hypertek.vn"
...
DEFAULT_MANAGER_BOT = "AetherSetupBot"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``aether_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``aether_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"aether_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting AETHER Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the AETHER Telegram onboarding service.")
