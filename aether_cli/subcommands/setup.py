"""``aether setup`` subcommand parser.
...
def build_setup_parser(subparsers, *, cmd_setup: Callable) -> None:
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure AETHER with an interactive wizard. "
        "Run a specific section: aether setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'aether setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
