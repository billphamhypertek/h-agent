"""``aether debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for AETHER. Use 'aether debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    aether debug share              Upload debug report and print URL
    aether debug share --lines 500  Include more log lines
    aether debug share --expire 30  Keep paste for 30 days
    aether debug share --local      Print report locally (no upload)
    aether debug share --no-redact  Disable upload-time secret redaction
    aether debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'aether debug share'",
    )
