...
Pure logic: walks a AETHER config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `aether doctor` and a future `aether migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a AETHER config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a AETHER config and return retirement issues.
