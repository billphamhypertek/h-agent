...
on Japanese locales, etc.).  AETHER's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``aether_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. AETHER is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``AETHER_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("AETHER_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the AETHER-managed Git install directories so
    ...
    # already-running shells don't see the change — which means aether
    ...
    # install without AETHER-managed PortableGit).
    ...
    # stdin is re-configured for completeness; AETHER's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and AETHER's
    ``aether config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching AETHER (User env var in Windows Settings, or
    export in a PowerShell profile) and AETHER picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known AETHER-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\aether\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``aether`` from right after install) retain their old PATH.

    Any subprocess AETHER spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known AETHER-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "aether", "git", "cmd"),
        os.path.join(local_appdata, "aether", "git", "bin"),
        os.path.join(local_appdata, "aether", "git", "usr", "bin"),
        # AETHER venv Scripts directory — host of the aether.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes aether, but harmless to include.
        os.path.join(local_appdata, "aether", "aether-agent", "venv", "Scripts"),
