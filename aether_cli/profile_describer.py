...
Used by ``aether profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``aether_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the AETHER kanban board.
...
Rules:
  ...
  - Never write "AETHER profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_aether_home())
        else:
