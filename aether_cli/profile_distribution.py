"""Profile distributions — shareable, packaged AETHER profiles via git.

A distribution is a AETHER profile published as a git repository (or
...
* ``aether profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``aether skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``aether profile``, not a parallel tree):

    aether profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "aether-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            aether_requires=str(data.get("aether_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.aether_requires:
            out["aether_requires"] = self.aether_requires
        if self.author:
...
def check_aether_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires AETHER {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``aether profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a AETHER profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a AETHER distribution."
        )

    # Version check up-front so we fail fast
    check_aether_requires(manifest.aether_requires, aether_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.aether).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="aether_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `aether profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`aether profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="aether_dist_update_") as tmp:
        plan = plan_install(
