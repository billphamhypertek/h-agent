"""
Unified self-relaunch for AETHER CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``aether sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``aether`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
from aether_cli._parser import (
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched aether."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_aether_bin() -> Optional[str]:
    """Find the aether entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("aether")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m aether_cli.main``.
    ...
    ``aether.exe`` on PATH or the ``sys.executable -m aether_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with aether.
    ...
    Args:
        ...
        original_argv: The original argv to scan for flags (defaults to
            ``sys.argv[1:]``).
    """
    bin_path = resolve_aether_bin()

    if bin_path:
        argv = [bin_path]
    else:
        argv = [sys.executable, "-m", "aether_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh aether invocation.
    ...
    relaunch contract wants: "run aether again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``aether.exe`` (a Python console-script shim that wraps
    ``python -m aether_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "aether exited, then
    new aether started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``aether`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nAETHER relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run aether.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
