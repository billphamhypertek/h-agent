...
AETHER owns its own uv binary at ``$AETHER_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$AETHER_HOME/bin`` so the installer writes directly there — no PATH
...
def managed_uv_path() -> Path:
    """Return the path where AETHER keeps *its* uv binary.

    ``$AETHER_HOME/bin/uv`` on POSIX, ``$AETHER_HOME\\bin\\uv.exe`` on
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``aether update`` runs
    the call site from the *old*, already-imported ``aether_cli.main`` against
...
def update_managed_uv() -> Optional[str]:
    ...
    Call this during ``aether update`` so the managed copy stays current.
...
def _install_uv(target: Path) -> None:
    ...
    ``$AETHER_HOME/bin/`` instead of ``~/.local/bin/``.
    """
