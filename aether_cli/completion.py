"""Shell completion script generation for aether CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_aether_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# AETHER bash completion
# Add to ~/.bashrc:
#   eval "$(aether completion bash)"

_aether_profiles() {{
    local profiles_dir="$HOME/.aether/profiles"
...
_aether_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_aether_profiles)" -- "$cur"))
        return
    fi
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef aether
# AETHER zsh completion
# Add to ~/.zshrc:
...
_aether_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.aether/profiles" ]]; then
        profiles+=($HOME/.aether/profiles/*(N/:t))
    fi
...
_aether() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_aether_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'aether command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# AETHER fish completion",
        "# Add to your config:",
        "#   aether completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __aether_profiles",
        "    echo default",
        "    if test -d $HOME/.aether/profiles",
        "        for d in $HOME/.aether/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c aether -f -s p -l profile"
        " -d 'Profile name' -xa '(__aether_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__aether_profiles)' -d 'Profile name'"
                )
