"""`aether checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.aether/checkpoints/``.  Actions:

    aether checkpoints               # same as `status`
    aether checkpoints status        # total size, project count, breakdown
    aether checkpoints list          # per-project checkpoint counts + workdir
    aether checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    aether checkpoints clear [-f]    # nuke the entire base (asks first)
    aether checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    aether checkpoints
    aether checkpoints prune --retention-days 3 --max-size-mb 200
    aether checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: aether checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``aether checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `aether checkpoints` → status
