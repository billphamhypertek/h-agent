...
const { buildDesktopBackendEnv, normalizeAetherHomeRoot } = require('./backend-env.cjs')
...
const USER_DATA_OVERRIDE = process.env.AETHER_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.AETHER_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with AETHER_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[aether] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}

ipcMain.handle('aether:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[aether] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[aether] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[aether] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// AETHER_HOME — the user-facing root for everything AETHER-related. Mirrors
// scripts/install.ps1's $AetherHome and scripts/install.sh's $AETHER_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\aether (matches install.ps1)
//   macOS / Linux: ~/.aether (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.aether directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\aether yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// AETHER_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// AETHER_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.aether / %LOCALAPPDATA%\aether.
function resolveAetherHome() {
  if (process.env.AETHER_HOME) return normalizeAetherHomeRoot(process.env.AETHER_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'aether-home')
  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a AETHER_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\aether and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('AETHER_HOME')
    if (fromRegistry) return normalizeAetherHomeRoot(fromRegistry)
  }
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'aether')
    const legacy = path.join(app.getPath('home'), '.aether')
    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.aether setup (no LOCALAPPDATA install yet) so users don't lose state.
    if (!directoryExists(localappdata) && directoryExists(legacy)) return legacy
    return localappdata
  }
  return path.join(app.getPath('home'), '.aether')
}

const AETHER_HOME = resolveAetherHome()

function aetherManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_aether_node_dirs() in
  // aether_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(AETHER_HOME, 'node')
  const bin = path.join(root, 'bin')
  const entries = IS_WINDOWS ? [root, bin] : [bin, root]
  return entries.filter(directoryExists)
}

function pathWithAetherManagedNode(...entries) {
  return [...aetherManagedNodePathEntries(), ...entries, process.env.PATH]
    .filter(Boolean)
    .join(path.delimiter)
}

// ACTIVE_AETHER_ROOT — the canonical mutable AETHER install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_AETHER_ROOT = path.join(AETHER_HOME, 'aether-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_AETHER_ROOT, 'venv')
...
// boots, going straight to `resolveAetherBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_AETHER_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_AETHER_ROOT, '.aether-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
const DESKTOP_WINDOW_STATE_PATH = path.join(app.getPath('userData'), 'window-state.json')
// active-profile.json records which AETHER profile the desktop launches its
// local backend as. When set, startAETHER() passes `aether --profile <name>
// dashboard …`, which deterministically pins AETHER_HOME (see
// _apply_profile_override in aether_cli/main.py) and bypasses the sticky
// ~/.aether/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors aether_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// aetherDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under AETHER_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by aether_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(AETHER_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (aether_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.AETHER_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.AETHER_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
// 'aether:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.aether) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.aether')
}
// Seed the native About panel with the live AETHER version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `aether update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveAetherVersion(),
...
const MEDIA_PROTOCOL = 'aether-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(aether-media://…)`.
...
// (the desktop's launch profile) stays managed by aetherProcess +
// connectionPromise + startAETHER(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.AETHER_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.AETHER_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startAETHER() calls (e.g. the renderer's
...
let bootProgressState = {
  ...
  message: 'Waiting to start AETHER backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  if (!text) return
  const lines = text.split(/\r?\n/).map(line => `[aether] ${line}`)
  aetherLog.push(...lines)
  if (aetherLog.length > 300) {
    aetherLog.splice(0, aetherLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-aether-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('aether:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the aether:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('aether:bootstrap:event', ev)
}
...
// The Tauri updater writes AETHER_HOME/.aether-update-in-progress for the whole
...
// (`force_kill_other_aether`, taskkill /IM aether.exe) kills it, the launch
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(AETHER_HOME)
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — AETHER will start automatically when it completes…',
      12
    )
    await new Promise(r => setTimeout(r, UPDATE_WAIT_POLL_MS))
    marker = readLiveUpdateMarker(AETHER_HOME)
  }
  if (marker) {
...
function isAetherSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'aether_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.AETHER_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several AETHER deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. AETHER' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\aether\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'aether', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'aether', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// install only has PortableGit under %LOCALAPPDATA%\aether\git (never on
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'aether', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'aether', 'git', 'bin', 'git.exe'))
  }
...
function recentAetherLog() {
  return aetherLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's aether root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_AETHER_ROOT.
// AETHER_DESKTOP_AETHER_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.AETHER_DESKTOP_AETHER_ROOT && path.resolve(process.env.AETHER_DESKTOP_AETHER_ROOT),
    !IS_PACKAGED && isAetherSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isAetherSourceRoot(ACTIVE_AETHER_ROOT) ? ACTIVE_AETHER_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_AETHER_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('aether:updates:progress', merged)
  }
}
...
// AETHER_HOME/aether-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_aether_home). That binary owns
// ALL repo mutation — running `aether update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'aether-setup.exe' : 'aether-setup'
  const candidate = path.join(AETHER_HOME, name)
...
// Path to the venv shim whose lock decides whether `aether update` can write
// fresh entry points. On Windows this is the file the running backend
// `aether.exe` holds open; on POSIX it's never mandatory-locked.
function venvAetherShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'aether.exe')
    : path.join(updateRoot, 'venv', 'bin', 'aether')
}
...
// only signals the direct child, so on Windows a backend `aether.exe` that
// spawned its own grandchildren (a `aether` REPL, a pty terminal session, the
...
// `aetherProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `aether.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a aether
// REPL, a pty terminal, the gateway) keep `aether.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (aetherProcess && Number.isInteger(aetherProcess.pid)) pids.push(aetherProcess.pid)
  ...
  if (aetherProcess && !aetherProcess.killed) {
    try {
      aetherProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvAetherShimPath(updateRoot)
...
// `aether update`). Instead we spawn the staged AETHER-Setup binary with
// --update and quit, so it can run `aether update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri aether-setup. Unlike Windows
      ...
      // whole update itself: `aether update` (backend) + `aether desktop
    ...
    if (!updater) {
      ...
      // `aether desktop`, never the Tauri installer that self-copies
      // aether-setup.exe into AETHER_HOME). They DO have a working `aether`
      ...
      // checkout they're on — bare `aether update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'aether update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `aether update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `aether update` if branch detection fails.
      }
      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })
      return { ok: true, manual: true, command, aetherRoot: updateRoot }
    }

    emitUpdateProgress({
      stage: 'restart',
      message: 'Updating AETHER — this window will close and the updater will open. Don’t reopen AETHER yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // aether.exe (held by the backend child / its grandchildren) and the update
    // bricks. See releaseBackendLockForUpdate for the full failure analysis.
    await releaseBackendLockForUpdate(updateRoot)

    // Detached so the updater outlives this process — it needs us GONE before
    // `aether update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: AETHER_HOME,
      env: {
        ...process.env,
        AETHER_HOME,
        PATH: pathWithAetherManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvAETHER = path.join(venvBin, IS_WINDOWS ? 'aether.exe' : 'aether')
  const updaterArgs = fileExists(venvAETHER) ? ['--update', '--branch', branch] : ['--repair', '--branch', branch]
...
// Resolve the aether CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `aether` on PATH.
function resolveAetherCliBinary(updateRoot) {
  const venvAETHER = path.join(updateRoot, 'venv', 'bin', 'aether')
  if (fileExists(venvAETHER)) return venvAETHER
  return findOnPath('aether') || null
}
...
// macOS/Linux in-app update: backend (`aether update`) + OS-aware GUI rebuild
// (`aether desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const aether = resolveAetherCliBinary(updateRoot)
  if (!aether) {
    emitUpdateProgress({ stage: 'manual', message: 'aether update', percent: null })
    return { ok: true, manual: true, command: 'aether update', aetherRoot: updateRoot }
  }

  // Put the AETHER-managed Node and the venv on PATH so `aether desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\aether\node, not node\bin.
  const env = {
    AETHER_HOME,
    PATH: pathWithAetherManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `aether update` reaps stale `aether dashboard` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads AETHER_DESKTOP_CHILD_PID and excludes
  ...
  if (aetherProcess && Number.isInteger(aetherProcess.pid)) {
    desktopChildPids.push(aetherProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.AETHER_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating AETHER (git + dependencies)…', percent: 10 })
  const updated = await runStreamedUpdate(aether, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'aether update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'aether update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(aether, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart AETHER to retry.',
      error: rebuilt.error || 'rebuild-failed'
    })
    return { ok: false, backendUpdated: true, error: 'desktop rebuild failed' }
  }

  // Linux in-app update terminal state (#45205). `aether desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting AETHER…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `aether-desktop-update-${Date.now()}.sh`)
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen AETHER to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the AETHER desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen AETHER to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'AETHER.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'AETHER.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart AETHER to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.aether-update-new"; then
    rm -rf "$DST.aether-update-old" 2>/dev/null || true
    mv "$DST" "$DST.aether-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.aether-update-new" "$DST"
    rm -rf "$DST.aether-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart AETHER to load the new version.',
      percent: 100
    })
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `aether
  ...
  return isAetherSourceRoot(ACTIVE_AETHER_ROOT) && fileExists(getVenvPython(VENV_ROOT))
}
...
function resolveWebDist() {
  const override = process.env.AETHER_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set AETHER_DESKTOP_WEB_DIST.`
    )
  }
  return fallback
}
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: aether desktop --force-build`
  )
  return candidates[0]
}
...
function resolveAetherCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/AETHER.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.AETHER_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveAetherCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, dashboardArgs, options = {}) {
  ...
  return {
    ...
    label,
    command: python,
    args: ['-m', 'aether_cli.main', ...dashboardArgs],
    env: buildDesktopBackendEnv({
      aetherHome: AETHER_HOME,
...
// createActiveBackend — build a backend pointing at ACTIVE_AETHER_ROOT, the
...
function createActiveBackend(dashboardArgs) {
  ...
  return {
    kind: 'python',
    label: `AETHER at ${ACTIVE_AETHER_ROOT}`,
    command: fileExists(venvPython) ? venvPython : findSystemPython(),
    args: ['-m', 'aether_cli.main', ...dashboardArgs],
    env: buildDesktopBackendEnv({
      aetherHome: AETHER_HOME,
      pythonPathEntries: [ACTIVE_AETHER_ROOT],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_AETHER_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveAetherBackend(dashboardArgs) {
  // 1. Explicit override -- AETHER_DESKTOP_AETHER_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.AETHER_DESKTOP_AETHER_ROOT && path.resolve(process.env.AETHER_DESKTOP_AETHER_ROOT)
  if (overrideRoot && isAetherSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `AETHER source at ${overrideRoot}`, dashboardArgs)
  ...
  //    installed `aether` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isAetherSourceRoot.)
  if (!IS_PACKAGED && isAetherSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `AETHER source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_AETHER_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\aether\aether-agent (Windows) or ~/.aether/aether-agent.
  ...
  //    to spawning aether. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `aether update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(dashboardArgs)
  }

  // 4. Existing `aether` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    AETHER_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.AETHER_DESKTOP_IGNORE_EXISTING !== '1') {
    let aetherCommand = null
    const aetherOverride = process.env.AETHER_DESKTOP_AETHER

    if (aetherOverride) {
      const resolvedOverride = findOnPath(aetherOverride)
      if (resolvedOverride) {
        aetherCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(aetherOverride, { isWsl: IS_WSL })) {
        aetherCommand = aetherOverride
      } else {
        rememberLog(`Ignoring Windows AETHER override under WSL: ${aetherOverride}`)
      }
    } else {
      aetherCommand = findOnPath('aether')
    }

    if (aetherCommand) {
      if (looksLikeDesktopAppBinary(aetherCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving AETHER CLI: ${aetherCommand}`)
        aetherCommand = null
      }
    }

    if (aetherCommand) {
      // Smoke-test the candidate before trusting it. A `aether` shim
      ...
      const shellForProbe = isCommandScript(aetherCommand)
      if (verifyAetherCli(aetherCommand, { shell: shellForProbe })) {
        return {
          label: `existing AETHER CLI at ${aetherCommand}`,
      ...
      rememberLog(
        `Ignoring existing AETHER CLI at ${aetherCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed aether_cli module via system Python.
  ...
  if (python) {
    ...
    // having aether_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\aether\aether-agent\venv.
    if (canImportAetherCli(python)) {
      return {
        kind: 'python',
        label: `installed aether_cli module via ${python}`,
        command: python,
        args: ['-m', 'aether_cli.main', ...dashboardArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: aether_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveAetherBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'AETHER not installed yet; bootstrap required',
    command: null,
    args: dashboardArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveAetherBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no AETHER install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError = new Error('AETHER recovery was handed off to AETHER Setup. The desktop will restart when recovery completes.')
    ...
    const bootstrapResult = await runBootstrap({
      ...
      aetherHome: AETHER_HOME,
      logRoot: path.join(AETHER_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('AETHER install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `AETHER bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(AETHER_HOME, 'logs', 'desktop.log')} for the full transcript.`
      )
      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startAETHER() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // aether:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveAetherBackend(backend.args))
  }
  ...
  if (!isAetherSourceRoot(ACTIVE_AETHER_ROOT)) {
    throw new Error(
      `AETHER install at ${ACTIVE_AETHER_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. AETHER' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\aether\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `aether` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for AETHER on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // isAetherSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `AETHER venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = venvPython
  backend.label = `AETHER at ${ACTIVE_AETHER_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'AETHER runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported AETHER backend URL protocol: ${parsed.protocol}`))
      return
    }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the AETHER backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to AETHER backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-AETHER-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('aether:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveAetherCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('aether:preview-file-changed', payload)
}
...
async function waitForAETHER(baseUrl, token) {
  ...
  throw new Error(`AETHER backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('aether:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('aether:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting AETHER...".
function sendPowerResume() {
  if (!mainWindow || mainWindow.isDestroyed()) return
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('aether:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('aether:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('aether:window-state-changed', state)
}
...
const ZOOM_STORAGE_KEY = 'aether:desktop:zoomLevel'
...
//
// Hosted AETHER gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``aether_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / aether #37247).
//     Both are set as HttpOnly cookies (``aether_session_at`` ~15 min,
//     ``aether_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:aether-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to AETHER gateway',
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to AETHER backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// a valid stored value (pins the root AETHER_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.AETHER_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.AETHER_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    // scope is never overridden by AETHER_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (aether #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote AETHER gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote AETHER gateway is selected, but no session token is saved. ' +
...
//   2. env override (AETHER_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.AETHER_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.AETHER_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'AETHER_DESKTOP_REMOTE_URL is set but AETHER_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote AETHER backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.AETHER_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every AETHER
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startAETHER())
  ...
  // connect to AETHER gateway". Mirror the renderer's connect here so the test
...
function resetAetherConnection() {
...
// startAETHER() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetAetherConnection() nulls aetherProcess.
  const dying = aetherProcess && !aetherProcess.killed ? aetherProcess : null
  resetAetherConnection()
...
// profile to startAETHER() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startAETHER() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForAETHER(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
      logs: aetherLog.slice(-80),
      ...getWindowState()
    }
  }

  const token = crypto.randomBytes(32).toString('base64url')
  // --profile wins over the inherited AETHER_HOME env (see _apply_profile_override
  // step 3 in aether_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const dashboardArgs = ['--profile', profile, 'dashboard', '--no-open', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveAetherBackend(dashboardArgs))
  const aetherCwd = resolveAetherCwd()
  const webDist = resolveWebDist()

  rememberLog(`Starting AETHER backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: aetherCwd,
        AETHER_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        AETHER_DESKTOP: '1',
        AETHER_WEB_DIST: webDist
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`AETHER backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`AETHER backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(
        new Error(`AETHER backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForAETHER(baseUrl, token), startFailed])
  ready = true
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `AETHER backend for profile "${profile}"`,
    rememberLog
  })
...
async function startAETHER() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startAETHER() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving AETHER backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote AETHER backend at ${remote.baseUrl}`, 24)
      await waitForAETHER(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote AETHER backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.aether/active_profile file) and
    // resolves AETHER_HOME the same way `aether -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving AETHER runtime', 28)
    const backend = await ensureRuntime(resolveAetherBackend(dashboardArgs))
    const aetherCwd = resolveAetherCwd()
    const webDist = resolveWebDist()

    await advanceBootProgress('backend.spawn', `Starting AETHER backend via ${backend.label}`, 84)
    rememberLog(`Starting AETHER backend via ${backend.label}`)
    ...
    aetherProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin AETHER_HOME for the child so Python's get_aether_home()
          // resolves to the SAME location our resolveAetherHome() picked. Without
          // this pin, Python falls back to ~/.aether on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\aether, which differs from C:\Users\<u>\.aether.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets AETHER_HOME via setx; the desktop
    ...
    aetherProcess.stdout.on('data', rememberLog)
    aetherProcess.stderr.on('data', rememberLog)
    let backendReady = false
    let rejectBackendStart = null
    const backendStartFailed = new Promise((_resolve, reject) => {
      rejectBackendStart = reject
    })
    aetherProcess.once('error', error => {
      rememberLog(`AETHER backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `AETHER backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    aetherProcess.once('exit', (code, signal) => {
      rememberLog(`AETHER backend exited (${signal || code})`)
      aetherProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `AETHER backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `AETHER backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentAetherLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for AETHER backend to launch', 86)
    // Discover the ephemeral port the child bound to
    const port = await Promise.race([waitForDashboardPort(aetherProcess), backendStartFailed])

    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for AETHER backend to become ready', 90)
    await Promise.race([waitForAETHER(baseUrl, token), backendStartFailed])
    backendReady = true
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null aetherProcess when the child dies.
      childAlive: () => aetherProcess !== null && aetherProcess.exitCode === null && !aetherProcess.killed,
      rememberLog
    })
    updateBootProgress({
      phase: 'backend.ready',
      message: 'AETHER backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while AETHER is
...
// pushes pet state over IPC (aether:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via aether:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main AETHER app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the AETHER icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see aether:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep AETHER in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('aether:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startAETHER().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('aether:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting AETHER…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('aether:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetAetherConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote AETHER backend failed liveness probe; dropping stale connection.')
    resetAetherConnection()
    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('aether:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)
  return { ok: true }
})
ipcMain.handle('aether:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('aether:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('aether:window:openNewSession', async () => {
...
ipcMain.handle('aether:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('aether:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('aether:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('aether:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('aether:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('aether:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('aether:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('aether:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('aether:pet-overlay:control', payload)
})
ipcMain.handle('aether:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startAETHER() call restarts the
...
ipcMain.handle('aether:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startAETHER() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('aether:bootstrap:cancel', async () => {
...
ipcMain.handle('aether:boot-progress:get', async () => bootProgressState)
ipcMain.handle('aether:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('aether:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('aether:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('aether:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('aether:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('aether:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('aether:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('aether:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('aether:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('aether:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new AETHER_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('aether:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('aether:requestMicrophoneAccess', async () => {
...
ipcMain.handle('aether:api', async (_event, request) => {
...
ipcMain.handle('aether:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('aether:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('aether:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
  return true
...
ipcMain.handle('aether:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('aether:readFileText', async (_event, filePath) => {
...
ipcMain.handle('aether:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('aether:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('aether:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('aether:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('aether:saveClipboardImage', async () => {
...
ipcMain.handle('aether:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('aether:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('aether:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('aether:titlebar-theme', (_event, payload) => {
...
ipcMain.on('aether:native-theme', (_event, mode) => {
...
ipcMain.on('aether:translucency', (_event, payload) => {
...
ipcMain.handle('aether:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('aether:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveAetherCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('aether:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveAetherCwd()
}))

ipcMain.handle('aether:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('aether:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('aether:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('aether:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('aether:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('aether:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: aetherLog.slice(-200) }))
...
function terminalShellCommand() {
  // AETHER_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.AETHER_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip AETHER' TUI into
  ...
  env.TERM_PROGRAM = 'AETHER'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a aether/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from AETHER_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.AETHER_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `aether:terminal:${id}:${suffix}`
}
...
ipcMain.handle('aether:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('aether:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

ipcMain.handle('aether:fs:worktrees', async (_event, cwds) => worktreesForIpc(cwds))

ipcMain.handle('aether:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart AETHER.')
  }
...
ipcMain.handle('aether:terminal:write', (_event, id, data) => {
...
ipcMain.handle('aether:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('aether:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('aether:updates:check', async () =>
...
ipcMain.handle('aether:updates:apply', async (_event, payload) =>
...
ipcMain.handle('aether:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('aether:updates:branch:set', async (_event, name) => {
  const branch = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
  writeDesktopUpdateConfig({ branch })
  return { branch }
})

// Resolve the canonical AETHER version (the one `release.py` bumps in
// aether_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real AETHER version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveAetherVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'aether_cli', '__init__.py')
...
// Re-resolve the live AETHER version and push it into the native About panel
// just before showing it, so an in-place `aether update` is reflected without
...
ipcMain.handle('aether:version', async () => ({
  appVersion: resolveAetherVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  aetherRoot: resolveUpdateRoot()
}))
...
// the actual removal via `aether uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (aether_cli/uninstall.py + aether_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_AETHER_ROOT
  ...
  const fallback = () => ({
    ...
    agent_installed: isAetherSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'aether_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, AETHER_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
      child.stdout.on('data', chunk => {
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no AETHER agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import aether_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_AETHER_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `aether-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `aether-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('aether:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('aether:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('aether:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('aether:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// aether:// deep links (e.g. aether://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const AETHER_PROTOCOL = 'aether'
let _pendingDeepLink = null
let _rendererReadyForDeepLink = false

function _extractDeepLink(argv) {
  if (!Array.isArray(argv)) return null
  return argv.find(a => typeof a === 'string' && a.startsWith(`${AETHER_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // aether://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('aether:deep-link', payload)
...
ipcMain.handle('aether:deep-link-ready', () => {
  _rendererReadyForDeepLink = true
  if (_pendingDeepLink) {
    const queued = _pendingDeepLink
    _pendingDeepLink = null
    handleDeepLink(
      `${AETHER_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(AETHER_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(AETHER_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `aether://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching aether:// URL is in our own argv.
