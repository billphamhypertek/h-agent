 ...
 *
 * Drives apps/desktop's first-launch install of AETHER by spawning
...
function bootstrapCacheDir(aetherHome) {
  return path.join(aetherHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.aether/aether-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(aetherHome) {
  if (!aetherHome) return null
  const candidate = path.join(aetherHome, 'aether-agent', 'scripts', installScriptName())
  try {
    fs.accessSync(candidate, fs.constants.R_OK)
    return candidate
  } catch {
...
function cachedScriptPath(aetherHome, commit) {
  return path.join(bootstrapCacheDir(aetherHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
async function resolveInstallScript({ installStamp, sourceRepoRoot, aetherHome, emit, _download = downloadInstallScript }) {
  ...
  const cached = cachedScriptPath(aetherHome, installStamp.commit)
  try {
  ...
  try {
    ...
    const installed = installedAgentInstallScript(aetherHome)
    if (installed) {
      emit({
        type: 'log',
        line:
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, aetherHome } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(ps, fullArgs, hiddenWindowsChildOptions({
      ...
      env: {
        ...process.env,
        // Pass AETHER_HOME through so install.ps1 respects the caller's
        // choice rather than re-computing the default.
        AETHER_HOME: aetherHome || process.env.AETHER_HOME || ''
      }
    }))
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, aetherHome } = {}) {
  return new Promise((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, aetherHome }) {
  const args = ['--dir', activeRoot, '--aether-home', aetherHome]
  if (installStamp && installStamp.branch) {
    args.push('--branch', installStamp.branch)
  }
  if (installStamp && installStamp.commit) {
...
async function fetchManifest({ scriptPath, installerKind, emit, aetherHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, aetherHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, aetherHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, aetherHome })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(aetherHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, aetherHome, emit })
