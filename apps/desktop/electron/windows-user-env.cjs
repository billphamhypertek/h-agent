...
// even though a fresh shell — and the AETHER CLI — sees it immediately. The
// desktop's AETHER_HOME resolution relies on process.env, so that stale-snapshot
// gap silently sends the backend to the default %LOCALAPPDATA%\aether. Reading
