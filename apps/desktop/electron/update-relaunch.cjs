 ...
 * Background
 * ----------
 * After `aether update` + `aether desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything AETHER_DESKTOP_* is preserved
// plus AETHER_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['AETHER_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['AETHER_DESKTOP_']
