 ...
 *      internals; AETHER_HOME / AETHER_DESKTOP_* env + cwd preserved) and is
 *      safely shell-quoted.
...
const ROOT = '/home/u/.aether/aether-agent'
...
test('resolveUnpackedRelease returns the dir for a binary UNDER release/<plat>-unpacked', () => {
  const exec = path.join(UNPACKED, 'aether')
...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  // AppImage mount
  assert.equal(resolveUnpackedRelease('/tmp/.mount_AETHER12345/AppRun', ROOT, 'linux'), null)
  // .deb / .rpm system install
  assert.equal(resolveUnpackedRelease('/usr/lib/aether/aether', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease('/opt/AETHER/aether', ROOT, 'linux'), null)
  // dev electron
  assert.equal(resolveUnpackedRelease('/home/u/.aether/aether-agent/node_modules/electron/dist/electron', ROOT, 'linux'), null)
  // empty / missing
  assert.equal(resolveUnpackedRelease('', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease(path.join(UNPACKED, 'aether'), '', 'linux'), null)
})
...
test('resolveUnpackedRelease is not fooled by a sibling prefix dir', () => {
  ...
  const sneaky = path.join(ROOT, 'apps', 'desktop', 'release', 'linux-unpacked-evil', 'aether')
...
test('collectRelaunchArgs drops Electron internals, keeps user/launcher args', () => {
  const argv = [
    ...
    '--lang=en-US',
    'aether://open/agent/42', // deep link — keep
    '--profile=work', // app flag — keep
    '--remote-debugging-port=9222' // internal — drop
  ]
  assert.deepEqual(collectRelaunchArgs(argv), ['--no-sandbox', 'aether://open/agent/42', '--profile=work'])
  assert.deepEqual(collectRelaunchArgs(undefined), [])
})

test('collectRelaunchEnv preserves AETHER_HOME + AETHER_DESKTOP_* + sandbox opt-out only', () => {
  const env = {
    AETHER_HOME: '/home/u/.aether',
    AETHER_DESKTOP_REMOTE_URL: 'http://box:9119',
    AETHER_DESKTOP_REMOTE_TOKEN: 'secret',
    AETHER_DESKTOP_AETHER_ROOT: '/home/u/dev/aether',
...
test('buildRelaunchScript embeds pid/exec/args/env/cwd and is valid bash', () => {
  const script = buildRelaunchScript({
    pid: 4242,
    execPath: '/home/u/.aether/aether-agent/apps/desktop/release/linux-unpacked/AETHER',
    args: ['aether://open/agent/42', "--note=it's fine"],
    env: { AETHER_HOME: '/home/u/.aether', AETHER_DESKTOP_REMOTE_URL: 'http://box:9119' },
    cwd: '/home/u/work dir'
  })
  ...
  assert.match(script, /export AETHER_HOME='\/home\/u\/\.aether'/)
  assert.match(script, /export AETHER_DESKTOP_REMOTE_URL='http:\/\/box:9119'/)
  assert.match(script, /cd '\/home\/u\/work dir'/)
  assert.match(script, /exec '.*\/linux-unpacked\/AETHER' 'aether:\/\/open\/agent\/42' '--note=it'\\''s fine'/)

  // It must be syntactically valid bash (`bash -n`). Write to a temp file and lint.
  const tmp = path.join(os.tmpdir(), `aether-relaunch-test-${Date.now()}.sh`)
...
test('buildRelaunchScript with no args/env still lints clean', () => {
  const script = buildRelaunchScript({
    pid: 1,
    execPath: '/opt/AETHER/AETHER',
    args: [],
    env: {},
    cwd: ''
  })
  const tmp = path.join(os.tmpdir(), `aether-relaunch-test2-${Date.now()}.sh`)
  fs.writeFileSync(tmp, script)
  try {
    execFileSync('bash', ['-n', tmp], { stdio: 'pipe' })
  } finally {
  ...
  assert.match(script, /exec '\/opt\/AETHER\/AETHER'\n/)
})
