...
function markerPath(aetherHome) {
  return path.join(aetherHome, '.aether-update-in-progress')
}
 ...
 * clock for tests.
 */
function readLiveUpdateMarker(aetherHome, { kill, now = Date.now, maxAgeMs = UPDATE_MARKER_MAX_AGE_MS } = {}) {
  const file = markerPath(aetherHome)
  let raw
  try {
