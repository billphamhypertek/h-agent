...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'aether-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/aether-runner-test',
    sourceRepoRoot: null,
    aetherHome: '/tmp/aether-runner-test',
    logRoot: '/tmp/aether-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'aether-agent', 'scripts')
