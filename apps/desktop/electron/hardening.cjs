...
function encryptDesktopSecret(value, safeStorageApi) {
  ...
  if (!encryptionAvailable) {
    throw new Error(
      'Secure token storage is unavailable, so AETHER Desktop cannot save remote gateway tokens. ' +
        'Set AETHER_DESKTOP_REMOTE_URL and AETHER_DESKTOP_REMOTE_TOKEN in your environment, or enable OS keychain access and try again.'
    )
  }
  ...
  try {
    ...
    throw new Error(
      `Failed to encrypt the remote gateway token for secure storage${detail}. ` +
        'Set AETHER_DESKTOP_REMOTE_URL and AETHER_DESKTOP_REMOTE_TOKEN in your environment as a fallback.'
    )
  }
}
