...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'aether_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'aether_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'aether_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/AETHER.app/Contents/MacOS/AETHER', 'darwin'),
    '/Applications/AETHER.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/AETHER.app/Contents/MacOS/AETHER', 'darwin'),
    '/Users/x/Applications/AETHER.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\AETHER\\AETHER.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\AETHER'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\aether-desktop\\AETHER.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\aether-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\AETHER.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_AetherXXXX/aether', 'linux', { APPIMAGE: '/home/x/Apps/AETHER.AppImage' }),
    '/home/x/Apps/AETHER.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(
    resolveRemovableAppPath('/opt/aether/linux-unpacked/aether', 'linux', {}),
    '/opt/aether/linux-unpacked'
  )
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/aether', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/AETHER.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/AETHER.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.aether/aether-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.aether/aether-agent',
    uninstallArgs: ['-m', 'aether_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/aether/linux-unpacked',
    aetherHome: '/home/x/.aether'
  })
  ...
  assert.match(script, /'-m' 'aether_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/aether\/linux-unpacked'/)
  assert.match(script, /export AETHER_HOME='\/home\/x\/\.aether'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.aether/aether-agent',
    agentRoot: '/home/x/.aether/aether-agent',
    uninstallArgs: ['-m', 'aether_cli.uninstall', '--mode', 'full'],
    appPath: null,
    aetherHome: '/home/x/.aether'
  })
  // System python + source on PYTHONPATH so import aether_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.aether\/aether-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'aether_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'aether_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    aetherHome: '/h'
  })
  assert.doesNotMatch(script, /rm -rf '\//)
  // Still runs the uninstall.
  assert.match(script, /'-m' 'aether_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\aether',
    uninstallArgs: ['-m', 'aether_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\AETHER',
    aetherHome: 'C:\\Users\\x\\AppData\\Local\\aether'
  })
  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import aether_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\aether;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "aether_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\AETHER" >nul 2>&1/)
