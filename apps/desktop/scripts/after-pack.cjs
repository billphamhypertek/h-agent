 ...
 *
 * Stamps the AETHER icon + identity onto the packed Windows AETHER.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `aether desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'AETHER')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'AETHER'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); AETHER.exe keeps the stock Electron icon`)
  }
}
