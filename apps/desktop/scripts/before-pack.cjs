 ...
 * product name (`AETHER`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/AETHER'
 *
 * This is a hard failure with no obvious cause for the user — `aether desktop`
 ...
 * (the mac-unpacked AETHER.app bundle) and Windows (win-unpacked), so we
