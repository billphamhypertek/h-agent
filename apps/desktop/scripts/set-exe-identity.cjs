#!/usr/bin/env node
// set-exe-identity.cjs — stamp the AETHER icon + version metadata onto the
// built AETHER.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked AETHER.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `aether desktop`, the installer's
...
// path (which rebuilds via `aether desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.cjs <path-to-AETHER.exe>
//
...
// Stamp the AETHER icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'AETHER',
      FileDescription: 'AETHER',
      CompanyName: 'Nous Research',
      LegalCopyright: 'Copyright (c) 2026 Nous Research'
    }
  })

  console.log('[set-exe-identity] done — AETHER icon + identity stamped')
}
