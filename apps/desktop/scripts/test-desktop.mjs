...
// no longer bundles the AETHER Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'AETHER.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'AETHER'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'AETHER.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'aether'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default AETHER_HOME for non-sandboxed runs -- matches main.cjs's
// resolveAetherHome(). On Windows it's %LOCALAPPDATA%\aether; elsewhere
// it's ~/.aether. The fresh-install sandbox launchFresh() sets its own
// AETHER_HOME and never touches this.
const DEFAULT_AETHER_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'aether')
  }
  return path.join(os.homedir(), '.aether')
})()
const VENV_ROOT = path.join(DEFAULT_AETHER_HOME, 'aether-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'aether-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.AETHER_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `AETHER-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `AETHER-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `AETHER-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'AETHER-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.AETHER_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.AETHER_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const aetherHome = path.join(sandbox, 'aether-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(aetherHome, { recursive: true })
  ...
  env.AETHER_DESKTOP_CWD = cwd
  env.AETHER_DESKTOP_IGNORE_EXISTING = '1'
  env.AETHER_DESKTOP_TEST_MODE = 'fresh-install'
  env.AETHER_DESKTOP_USER_DATA_DIR = userDataDir
  env.AETHER_HOME = aetherHome
  delete env.AETHER_DESKTOP_AETHER
  delete env.AETHER_DESKTOP_AETHER_ROOT
  ...
  console.log(`  AETHER_HOME: ${aetherHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(aetherHome, 'aether-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The AETHER Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of aether_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'aether-agent', 'aether_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing AETHER
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + AETHER_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  AETHER_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
