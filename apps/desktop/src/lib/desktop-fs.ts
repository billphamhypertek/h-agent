import type {
  AetherConnection,
  AetherReadDirResult,
  AetherReadFileTextResult,
  AetherSelectPathsOptions,
  AetherWorktreeInfo
} from '@/global'
...
function connectionCacheKey(connection: AetherConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
function bridge() {
  ...
  if (!desktop) {
    throw new Error('AETHER Desktop bridge is unavailable')
  }
  return desktop
}

export async function readDesktopDir(path: string): Promise<AetherReadDirResult> {
  const desktop = bridge()
  if (!isDesktopFsRemoteMode()) {
    return desktop.readDir(path)
  }
  return desktop.api<AetherReadDirResult>({ path: fsPath('list', path) })
}

export async function readDesktopFileText(path: string): Promise<AetherReadFileTextResult> {
  const desktop = bridge()
  if (!isDesktopFsRemoteMode()) {
    return desktop.readFileText(path)
  }
  return desktop.api<AetherReadFileTextResult>({ path: fsPath('read-text', path) })
}
...
export async function desktopWorktrees(cwds: string[]): Promise<Record<string, AetherWorktreeInfo | null>> {
  if (isDesktopFsRemoteMode()) {
    return {}
  }
...
export async function selectDesktopPaths(options?: AetherSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
  if (!isDesktopFsRemoteMode()) {
