...
/** Why a known AETHER command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active AETHER profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show AETHER version', surface: exec() },
...
function isKnownAetherSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * AETHER' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownAetherSlashCommand(normalized)
}
