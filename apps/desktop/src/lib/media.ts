...
export function mediaStreamUrl(path: string): string {
  return `aether-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// expose GET /api/media (aether_cli/web_server.py).
export async function gatewayMediaDataUrl(path: string): Promise<string> {
  const file = filePathFromMediaPath(path)

  const result = await window.aetherDesktop!.api<{ data_url: string }>({
