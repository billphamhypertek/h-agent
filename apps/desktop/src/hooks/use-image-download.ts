...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'aether:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.aetherDesktop?.saveImageFromUrl) {
        if (await window.aetherDesktop.saveImageFromUrl(src)) {
