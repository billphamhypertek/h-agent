...
export interface I18nConfigClient {
  getConfig: () => Promise<AetherConfigRecord>
  saveConfig: (config: AetherConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.aetherDesktop?.api) {
      return Promise.resolve({})
    }

    return getAetherConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.aetherDesktop?.api) {
    ...
    return saveAetherConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: AetherConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: AetherConfigRecord, locale: Locale): AetherConfigRecord {
