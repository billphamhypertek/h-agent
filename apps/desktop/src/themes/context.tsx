...
const SKIN_KEY = 'aether-desktop-theme-v2'
const MODE_KEY = 'aether-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'aether-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'aether-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'aether-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.aetherTheme = skinName
  root.dataset.aetherMode = rendered
  ...
  window.aetherDesktop?.setTitleBarTheme?.({
  ...
  try {
    window.localStorage.setItem('aether-boot-background', c.background)
    window.localStorage.setItem('aether-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.aetherThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.aetherDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active AETHER backend theme on connect. */
