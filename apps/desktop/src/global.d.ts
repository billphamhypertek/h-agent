...
declare global {
  interface Window {
    aetherDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<AetherConnection>
      ...
      profile: {
        ...
        // backend under the new AETHER_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: AetherApiRequest) => Promise<T>
      notify: (payload: AetherNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<AetherReadFileTextResult>
      selectPaths: (options?: AetherSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<AetherPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<AetherPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: AetherTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<AetherReadDirResult>
      ...
      worktrees?: (cwds: string[]) => Promise<Record<string, AetherWorktreeInfo | null>>
      terminal: {
        dispose: (id: string) => Promise<boolean>
        onData: (id: string, callback: (payload: string) => void) => () => void
        onExit: (id: string, callback: (payload: AetherTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<AetherTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: AetherWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: AetherPreviewFileChanged) => void) => () => void
...
export interface AetherTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface AetherTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `aether update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  aetherRoot?: string
...
export interface AetherConnection {
  baseUrl: string
...
export interface AetherTitleBarTheme {
  background: string
  foreground: string
}

export interface AetherWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface AetherApiRequest {
  path: string
  method?: string
  body?: unknown
  timeoutMs?: number
...
export interface AetherPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface AetherReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface AetherPreviewWatch {
  id: string
  path: string
}

export interface AetherWorktreeInfo {
...
export interface AetherReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface AetherReadDirResult {
  entries: AetherReadDirEntry[]
  error?: string
}

export interface AetherPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface AetherSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
