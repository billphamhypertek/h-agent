...
// /api/profiles/active `current`). "default" is the root ~/.aether. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'aether.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'aether.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.aetherDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
// Persist the choice and relaunch the backend under the new AETHER_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.aetherDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.aetherDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'aether.desktop.showAllProfiles'
...
// Switch to the default (root ~/.aether) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.aetherDesktop?.touchBackend?.(target).catch(() => undefined)
}
