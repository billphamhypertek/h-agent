 ...
 * app's bounds and stays visible while AETHER is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'aether.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'aether.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.aetherDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.aetherDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.aetherDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.aetherDesktop?.petOverlay?.close()
}
