...
import type { SessionInfo, UsageStats } from '@/types/aether'

type Updater<T> = T | ((current: T) => T)

const WORKSPACE_CWD_KEY = 'aether.desktop.workspace-cwd'
...
const COMPOSER_MODEL_KEY = 'aether.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'aether.desktop.composer.provider'
const COMPOSER_EFFORT_KEY = 'aether.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'aether.desktop.composer.fast'

let configuredDefaultProjectDir = ''

function workspaceCwdKey(connection: AetherConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.aetherDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.aetherDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<AetherConnection | null>(null)
...
export const setConnection = (next: Updater<AetherConnection | null>) => updateAtom($connection, next)
