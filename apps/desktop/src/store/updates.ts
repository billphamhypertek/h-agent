...
import type { BackendUpdateCheckResponse } from '@/types/aether'
...
const UPDATE_TOAST_SNOOZE_KEY = 'aether:update-toast-snooze-until'
...
const SKEW_TOAST_SNOOZE_KEY = 'aether:backend-skew-toast-snooze-until'
...
export function reportBackendContract(contract: number | undefined): void {
  ...
  notify({
    action: {
      label: translateNow('notifications.updateAETHER'),
      onClick: () => {
        snoozeSkewToast()
...
export async function refreshDesktopVersion(): Promise<DesktopVersionInfo | null> {
  ...
  try {
    const next = await window.aetherDesktop?.getVersion?.()
...
export async function checkBackendUpdates(): Promise<DesktopUpdateStatus | null> {
  ...
  try {
    const status = mapBackendCheck(await checkAetherUpdate(true))
...
export async function checkUpdates(): Promise<DesktopUpdateStatus | null> {
  const bridge = window.aetherDesktop?.updates
...
export async function applyUpdates(opts: DesktopUpdateApplyOptions = {}): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    // `aether update` themselves. Land on a dedicated manual state so the
    ...
    if (result?.manual) {
      $updateApply.set({
        ...
        applying: false,
        stage: 'manual',
        message: result.command ?? 'aether update',
        command: result.command ?? 'aether update'
      })
...
export async function applyBackendUpdate(): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    const started = await updateAETHER()

    if (!started.ok) {
      const message = (started as { message?: string }).message || translateNow('updates.applyStatus.notAvailable')
      const command = (started as { update_command?: string }).update_command || 'aether update'
