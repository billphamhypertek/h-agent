...
const remoteConn = (over: Partial<AetherConnection> = {}): AetherConnection =>
  ({ baseUrl: 'https://aether-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as AetherConnection

const localConn = (over: Partial<AetherConnection> = {}): AetherConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as AetherConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<AetherConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { aetherDesktop: { getConnection } })
})
