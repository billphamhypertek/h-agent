...
const isOpen = (gateway: AetherGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<AetherGateway | null>(null)
...
let primaryGateway: AetherGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: AetherGateway | null, profile = 'default'): void {
...
export function activeGateway(): AetherGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<AetherGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
