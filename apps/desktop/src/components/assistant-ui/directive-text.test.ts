...
import { formatRefValue, aetherDirectiveFormatter } from './directive-text'
...
describe('aetherDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = aetherDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = aetherDirectiveFormatter.parse('@file:src/main.tsx the entry point')
