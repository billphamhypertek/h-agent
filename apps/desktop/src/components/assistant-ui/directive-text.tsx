...
const AETHER_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type AetherRefType = (typeof AETHER_REF_TYPES)[number]
...
const ICON_PATHS: Record<AetherRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as AetherRefType] ?? ICON_FALLBACK
...
const AETHER_DIRECTIVE_RE = new RegExp(
...
export const aetherDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(AETHER_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as AetherRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: AetherRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...aetherDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing AETHER directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.aetherDesktop && isRemoteGateway()
        ? gatewayMediaDataUrl(id)
        : window.aetherDesktop?.readFileDataUrl(id)
