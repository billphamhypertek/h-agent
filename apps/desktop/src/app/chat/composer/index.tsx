...
import { aetherDirectiveFormatter, type SlashChipKind } from '@/components/assistant-ui/directive-text'
...
import { extractDroppedFiles, AETHER_PATHS_MIME, partitionDroppedFiles } from '../hooks/use-composer-actions'
...
export function ChatBar({
  ...
  // Distinguish a cold start ("Starting AETHER...") from a dropped connection
  ...
  const replaceTriggerWithChip = (item: Unstable_TriggerItem) => {
    ...
    const serialized = aetherDirectiveFormatter.serialize(item)
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLFormElement>) => {
    if (!onAttachDroppedItems || !dragHasAttachments(event.dataTransfer, AETHER_PATHS_MIME)) {
      return
    }
  ...
  const handleInputDragOver = (event: ReactDragEvent<HTMLDivElement>) => {
    if (!dragHasAttachments(event.dataTransfer, AETHER_PATHS_MIME)) {
      return
    }
