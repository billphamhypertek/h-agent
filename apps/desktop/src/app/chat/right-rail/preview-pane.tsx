        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.aetherDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask AETHER to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.aetherDesktop?.watchPreviewFile ||
      !window.aetherDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.aetherDesktop.onPreviewFileChanged(payload => {
    ...
    void window.aetherDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.aetherDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.aetherDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:aether-preview')
