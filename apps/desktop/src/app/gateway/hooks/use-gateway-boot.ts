...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.aetherDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: AetherGateway | null) => void
  refreshAetherConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: AetherConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting AETHER...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting AETHER…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to AETHER gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshAetherConfig().catch(() => undefined)
    ...
    const gateway = new AetherGateway()
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshAetherConfig()
