...
// bug that drives the REAL useGatewayBoot hook + REAL AetherGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshAetherConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { aetherDesktop?: unknown }).aetherDesktop = fakeDesktop()
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForAETHER) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startAETHER()'s remote branch awaits waitForAETHER() for 45s before it
    ...
    ;(window as { aetherDesktop?: unknown }).aetherDesktop = desktop
    ...
    // After ~45s waitForAETHER gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('AETHER backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
