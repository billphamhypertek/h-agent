...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.aetherDesktop?.terminal)
...
//      in aether_cli/provider_catalog.py) — the SAME provider identity
//      `aether model` uses. This is authoritative: a provider tagged by the
...
// same recommended card, same provider rows, same "Other providers"
...
// components (from @/components/provider-setup), so the two surfaces stay
// visually identical.
...
function ConnectedProviderRow({
  ...
  // AETHER can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider AETHER can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // AETHER never deletes creds another tool owns behind a silent API call.
