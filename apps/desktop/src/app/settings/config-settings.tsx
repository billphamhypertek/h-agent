...
import type { ConfigFieldSchema, AetherConfigRecord } from '@/types/aether'
...
export function voiceFieldVisible(key: string, config: AetherConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [_defaults, setDefaults] = useState<AetherConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getAetherConfigRecord(), getAetherConfigDefaults(), getAetherConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  const updateConfig = (next: AetherConfigRecord) => {
