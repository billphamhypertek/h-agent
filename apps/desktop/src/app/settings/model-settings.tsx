...
import type { AetherConfigRecord } from '@/types/aether'

import { CONTROL_TEXT } from './constants'
import { getNested, setNested } from './helpers'
import { ListRow, LoadingState, Pill, SectionHeading } from './primitives'

// AETHER' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = AETHER default (medium), shown as Medium.
...
// backend now surfaces the full `aether model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in aether_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const [config, setConfig] = useState<AetherConfigRecord | null>(null)
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `aether model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — AETHER runs the flow for you.`}
          </p>
        )}
