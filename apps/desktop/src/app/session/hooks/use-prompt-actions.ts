...
import { getProfiles, transcribeAudio } from '@/aether-api'
...
async function readFileDataUrlForAttach(filePath: string): Promise<string | null> {
  const reader = window.aetherDesktop?.readFileDataUrl
...
export function usePromptActions({
  ...
  // separate `aether gateway` process performs the actual transfer, so we
