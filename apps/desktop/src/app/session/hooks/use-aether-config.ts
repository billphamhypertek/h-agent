...
interface AetherConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useAetherConfig({ activeSessionIdRef, refreshProjectBranch }: AetherConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshAetherConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getAetherConfig(), getAetherConfigDefaults().catch(() => ({}))])
  ...
  return { refreshAetherConfig, sttEnabled, voiceMaxRecordingSeconds }
}
