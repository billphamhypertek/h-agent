...
import type { AetherReadDirResult } from '@/global'

import { clearProjectDirCache, readProjectDir } from './ipc'
import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<AetherReadDirResult>>()

beforeEach(() => {
  $connection.set(null)
  resetProjectTreeState()
  readDir.mockReset()
  ;(window as unknown as { aetherDesktop: { readDir: typeof readDir } }).aetherDesktop = { readDir }
})
...
afterEach(() => {
  ...
  $connection.set(null)
  resetProjectTreeState()
  delete (window as unknown as { aetherDesktop?: unknown }).aetherDesktop
})

function ok(entries: { name: string; path: string; isDirectory: boolean }[]): AetherReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { aetherDesktop: unknown }).aetherDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: AetherReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<AetherReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { aetherDesktop: unknown }).aetherDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.aetherDesktop is missing', async () => {
    delete (window as unknown as { aetherDesktop?: unknown }).aetherDesktop
