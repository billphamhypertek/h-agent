...
import type { AetherReadDirEntry, AetherReadDirResult } from '@/global'

export type ProjectTreeEntry = AetherReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: AetherReadDirEntry) {
...
async function filterIgnored(entries: AetherReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<AetherReadDirResult> {
