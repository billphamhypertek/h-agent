...
import { getSessionMessages, listAllProfileSessions } from '@/aether-api'
...
import type { SessionInfo, SessionMessage } from '@/types/aether'
  ...
  const openArtifact = useCallback(async (href: string) => {
    try {
      if (window.aetherDesktop?.openExternal) {
        await window.aetherDesktop.openExternal(href)
      } else {
