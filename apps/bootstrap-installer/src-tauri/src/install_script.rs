...
//!
//! Resolution order:
//!   1. Dev shortcut: a sibling repo checkout via $AETHER_SETUP_DEV_REPO_ROOT
...
//! Electron app's APP_ROOT/../.. trick, because AETHER-Setup.exe is meant
...
///
/// `pin` is the commit-or-branch from either AETHER-Setup's build-time
...
pub async fn resolve(
    ...
    // 1. Dev shortcut.
    if let Ok(repo_root) = std::env::var("AETHER_SETUP_DEV_REPO_ROOT") {
...
async fn download(kind: ScriptKind, commit_or_ref: &str, dest_path: &Path) -> Result<()> {
    ...
    let response = reqwest::Client::new()
        .get(&url)
        .header("User-Agent", "aether-setup/0.0.1")
        .send()
        .await
