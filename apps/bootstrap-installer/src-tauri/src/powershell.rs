...
///
/// `aether_home_override` propagates to the child as $AETHER_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, aether_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = aether_home_override {
        cmd.env("AETHER_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, aether_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = aether_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_aether_home() {
