...
//!
//! Mirrors `aether_constants.get_aether_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\aether
//!   macOS:   ~/.aether
//!   Linux:   ~/.aether  (override via $AETHER_HOME)
//!
//! NOTE (macOS): Python's get_aether_home(), scripts/install.sh, and the
//! Electron desktop's resolveAetherHome() ALL use ~/.aether on macOS — there
...
/// Returns the canonical AETHER home directory, respecting $AETHER_HOME if set.
pub fn aether_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("AETHER_HOME") {
    ...
    {
        // %LOCALAPPDATA%\aether — matches scripts/install.ps1's $AetherHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("aether");
        }
    }

    // macOS + Linux + fallback: ~/.aether (matches Python get_aether_home(),
    // install.sh, and the Electron desktop's resolveAetherHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".aether");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    aether_home().join("bootstrap-cache")
}
...
/// AETHER_HOME so it survives repo checkout deletion (unlike anything under
/// aether-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\aether\aether-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_aether`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under AETHER_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves AETHER_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    aether_home().join(".aether-update-in-progress")
}
...
pub fn copy_self_to_aether_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to AETHER_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_AETHER_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".aether-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under AETHER_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[aether-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("AETHER_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_aether_home() -> String {
    aether_home().to_string_lossy().into_owned()
}
