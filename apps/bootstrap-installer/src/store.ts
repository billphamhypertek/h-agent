...
export const $aetherHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, aetherHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_aether_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $aetherHome.set(aetherHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch AETHER" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  // Update is driven by the desktop handing off (AETHER-Setup.exe --update);
...
export async function launchAetherDesktop(): Promise<void> {
  const installRoot = $bootstrap.get().installRoot
  if (!installRoot) throw new Error('no install root')
  await invoke('launch_aether_desktop', { installRoot })
}
