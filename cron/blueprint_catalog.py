...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``aether://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``aether://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"aether://blueprint/{quote(blueprint.key)}{qs}"
