...
# from shadowing AETHER's own top-level modules.  ``aether_bootstrap`` lives at
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `AETHER_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("AETHER_TUI_SIDECAR_URL")
...
# tested; override via ``AETHER_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("AETHER_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``AETHER_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``aether --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    CLI path via ``aether_cli.mcp_startup``); ``timeout`` overrides it.
    """
    thread = _mcp_discovery_thread
    if thread is None or not thread.is_alive():
        return
    try:
        from aether_cli.mcp_startup import _resolve_discovery_timeout
...
def main():
    ...
    # server stalled the whole shell on "summoning aether…" for the full
