"""Persistent slash-command worker — one AetherCLI per TUI session.
...
def _env_float(name: str, default: float) -> float:
    ...
    import time on a typo (e.g. ``AETHER_SLASH_WATCHDOG_POLL_S=2s``) and kill
...
_WATCHDOG_POLL_S = max(0.05, _env_float("AETHER_SLASH_WATCHDOG_POLL_S", 2.0))
_ORPHAN_GRACE_S = max(0.0, _env_float("AETHER_SLASH_WATCHDOG_GRACE_S", 5.0))
...
def _run(cli: AetherCLI, command: str) -> str:
...
def main():
    ...
    os.environ["AETHER_SESSION_KEY"] = args.session_key
    os.environ["AETHER_INTERACTIVE"] = "1"

    # Start before the (hundreds-of-ms) AetherCLI build — that window is itself
    ...
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = AetherCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
