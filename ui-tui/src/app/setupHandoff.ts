import type { RunExternalProcess } from '@aether/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`aether ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching aether: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`aether ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
