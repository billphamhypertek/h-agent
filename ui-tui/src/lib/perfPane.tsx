...
//
// Both gate on AETHER_DEV_PERF=1 and dump JSON-lines (default ~/.aether/perf.log,
// override AETHER_DEV_PERF_LOG). Tagged { src: 'react' | 'frame' } for jq.
// AETHER_DEV_PERF_MS (default 2) skips sub-ms idle frames; set 0 to capture all.
//
...
import { scrollFastPathStats } from '@aether/ink'
import { Profiler, type ProfilerOnRenderCallback, type ReactNode } from 'react'

const ENABLED = /^(?:1|true|yes|on)$/i.test((process.env.AETHER_DEV_PERF ?? '').trim())
const THRESHOLD_MS = Number(process.env.AETHER_DEV_PERF_MS ?? '2') || 0
const LOG_PATH = process.env.AETHER_DEV_PERF_LOG?.trim() || join(homedir(), '.aether', 'perf.log')
