...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.AETHER_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep AETHER on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via AETHER_TUI_TRUECOLOR=1.
