...
const logDir = join(process.env.AETHER_HOME?.trim() || join(homedir(), '.aether'), 'logs')
...
// a real ~/.aether (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('aether-tui: parent lifecycle log unavailable\n')
    }
  }
}
