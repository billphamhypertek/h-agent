...
await build({
  ...
  // Skip the prebuilt @aether/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@aether/ink': resolve(root, 'packages/aether-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The aether_cli launcher
