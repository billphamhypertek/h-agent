...
React + Ink terminal UI for AETHER. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `AETHER_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `aether setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.aether/.aether_history` or under `AETHER_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `aether setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/aether-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external aether setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.aether/tui-parent.log
    ...
    types/
      aether-ink.d.ts            type declarations for @aether/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent AetherCLI subprocess for slash commands
```