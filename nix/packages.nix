# nix/packages.nix — AETHER package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    { pkgs, lib, inputs', ... }:
    let
      aetherAgent = pkgs.callPackage ./aether-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = aetherAgent.aetherTui;
        web = aetherAgent.aetherWeb;
        desktop = aetherAgent.aetherDesktop;
      };
    };
}
