# nix/nixosModules.nix — NixOS module for aether-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: aether runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $AETHER_HOME/.env
# and read by aether at startup — no container recreation needed for env changes.
#
# Tool resolution: the aether wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.aether-agent;
    effectivePackage =
    ...
    aether-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-aether-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "aether-config-attrs";
      description = "AETHER YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "aether-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # AETHER_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "aether-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'AETHER_DOC_EOF'\n${value}\nAETHER_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "aether-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/aether";
    ...
    # aether user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "aether-container-entrypoint" ''
      set -eu

      AETHER_UID="''${AETHER_UID:?AETHER_UID must be set}"
      AETHER_GID="''${AETHER_GID:?AETHER_GID must be set}"

      # ── Group: ensure a group with GID=$AETHER_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$AETHER_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="aether"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$AETHER_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$AETHER_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$AETHER_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$AETHER_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="aether"
        TARGET_HOME="/home/aether"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$AETHER_UID" -g "$AETHER_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$AETHER_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$AETHER_UID:$AETHER_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure AETHER_HOME is owned by the target user.
      ...
      if [ -n "''${AETHER_HOME:-}" ] && [ -d "$AETHER_HOME" ]; then
        find "$AETHER_HOME" \! -user "$AETHER_UID" -exec chown "$AETHER_UID:$AETHER_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/aether-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/aether-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/aether ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/aether
        chmod 0440 /etc/sudoers.d/aether
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$AETHER_UID" --regid="$AETHER_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $AETHER_HOME/.env.
    ...
    # Default: /var/lib/aether/workspace → /data/workspace.
  ...
  in {
    options.services.aether-agent = with lib; {
      enable = mkEnableOption "AETHER gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = aether-agent;
        description = "The aether-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/aether";
        description = "State directory. Contains .aether/ subdir (AETHER_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative AETHER config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $AETHER_HOME/.env at activation time.
          AETHER reads this file on every startup via load_aether_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $AETHER_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $AETHER_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `aether gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the aether user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the aether plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. AETHER discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          aether_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as aether (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in aether-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the aether CLI to environment.systemPackages and export
          AETHER_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.aether symlink to the service
            stateDir. These users are automatically added to the aether group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.aether-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the aether CLI to system PATH and export AETHER_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.aether/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.AETHER_HOME = "${cfg.stateDir}/.aether";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.aether-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the aether user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.aether-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed aether
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure aether is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.aether        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.aether/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.aether/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.aether/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.aether/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.aether/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."aether-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.aether
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.aether ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.aether ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.aether -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.aether/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.aether/$_subdir"
            chmod 2770 "${cfg.stateDir}/.aether/$_subdir"
            find "${cfg.stateDir}/.aether/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # aether-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.aether/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.aether/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.aether/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.aether/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.aether/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.aether/.managed
          chmod 0644 ${cfg.stateDir}/.aether/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.aether/.container-mode <<'AETHER_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    aether_bin=${containerDataDir}/current-package/bin/aether
    AETHER_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.aether/.container-mode
            chmod 0644 ${cfg.stateDir}/.aether/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.aether/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.aether";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.aether" ]; then
                  rm -f "${symlinkPath}"
                  echo "aether-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.aether -> stateDir/.aether for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.aether";
                target = "${cfg.stateDir}/.aether";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "aether-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.aether/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.aether/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.aether/auth.json
              fi
            ''}
          ''}
          ...
          # AETHER reads $AETHER_HOME/.env at startup via load_aether_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.aether/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'AETHER_NIX_ENV_EOF'
    ${envFileContent}
    AETHER_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.aether/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.aether/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.aether/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.aether-agent = {
          ...
          environment = {
            HOME = cfg.stateDir;
            AETHER_HOME = "${cfg.stateDir}/.aether";
            AETHER_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $AETHER_HOME/.env by the activation script. load_aether_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/aether"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the aether group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.aether-agent = {
          description = "AETHER Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              AETHER_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              AETHER_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env AETHER_UID="$AETHER_UID" \
                --env AETHER_GID="$AETHER_GID" \
                --env AETHER_HOME=${containerDataDir}/.aether \
                --env AETHER_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/aether gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
