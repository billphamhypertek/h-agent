...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      aether-agent = self'.packages.default;
      aetherVenv = aether-agent.aetherVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "aether-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${aetherVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "aether-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "aether-build-package" { } ''
          echo "PASS: package built at ${aether-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "aether-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "aether-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${aether-agent}/bin/aether || (echo "FAIL: aether binary missing"; exit 1)
          test -x ${aether-agent}/bin/aether-agent || (echo "FAIL: aether-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${aether-agent}/bin/aether version 2>&1 | grep -qi "aether" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "aether-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in aether aether-agent aether-acp; do
            test -x ${aether-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "aether-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking aether --help ==="
          ${aether-agent}/bin/aether --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${aether-agent}/bin/aether --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "aether-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${aether-agent}/share/aether-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${aether-agent}/share/aether-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "AETHER_BUNDLED_SKILLS" ${aether-agent}/bin/aether || \
            (echo "FAIL: AETHER_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: AETHER_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "aether-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${aether-agent}/share/aether-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${aether-agent}/share/aether-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "AETHER_BUNDLED_PLUGINS" ${aether-agent}/bin/aether || \
            (echo "FAIL: AETHER_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: AETHER_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "aether-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${aether-agent}/share/aether-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${aether-agent}/share/aether-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${aether-agent}/share/aether-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "AETHER_BUNDLED_LOCALES" ${aether-agent}/bin/aether || \
            (echo "FAIL: AETHER_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: AETHER_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (AETHER_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && AETHER_BUNDLED_LOCALES=${aether-agent}/share/aether-agent/locales \
            ${aetherVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with AETHER_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${aetherVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${aetherVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without AETHER_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "aether-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${aether-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${aether-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "AETHER_TUI_DIR" ${aether-agent}/bin/aether || \
            (echo "FAIL: AETHER_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: AETHER_TUI_DIR set in wrapper"
        ...
        # Verify AETHER_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        aether-node = pkgs.runCommand "aether-node-version" { } ''
          set -e
          echo "=== Checking AETHER_NODE in wrapper ==="
          grep -q "AETHER_NODE" ${aether-agent}/bin/aether || \
            (echo "FAIL: AETHER_NODE not set in wrapper"; exit 1)
          echo "PASS: AETHER_NODE present in wrapper"

          AETHER_NODE=$(sed -n "s/^export AETHER_NODE='\(.*\)'/\1/p" ${aether-agent}/bin/aether)
          test -x "$AETHER_NODE" || (echo "FAIL: AETHER_NODE=$AETHER_NODE not executable"; exit 1)
          echo "PASS: AETHER_NODE executable at $AETHER_NODE"

          NODE_MAJOR=$("$AETHER_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All AETHER_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify AETHER_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "aether-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(AETHER_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking AETHER_MANAGED guards ==="
          check_blocked "config set" ${aether-agent}/bin/aether config set model foo
          check_blocked "config edit" ${aether-agent}/bin/aether config edit
        ...
        extra-python-packages = let
          ...
          aetherWithExtra = aether-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "aether-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${aetherWithExtra}/bin/aether || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${aetherWithExtra}/bin/aether || \
          ...
          if grep -q "PYTHONPATH" ${aether-agent}/bin/aether; then
        ...
        extra-dependency-groups = let
          aetherWithGroups = aether-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "aether-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${aetherWithGroups}"
          echo "venv: ${aetherWithGroups.aetherVenv}"
        ...
        messaging-variant = pkgs.runCommand "aether-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.aetherVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "aether-config-roundtrip" {
          ...
          merge_and_load() {
            local aether_home="$1"
            export AETHER_HOME="$aether_home"
            ${configMergeScript} ${nixSettings} "$aether_home/config.yaml"
            ${aetherVenv}/bin/python3 -c '
import json, sys
