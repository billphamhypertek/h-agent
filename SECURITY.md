# AETHER Security Policy

This document describes AETHER's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/billphamhypertek/h-agent/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **AETHER does not operate a bug
bounty program.**
...
- Environment details (`aether version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted AETHER access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in AETHER's documentation
...
AETHER supports two OS-level isolation postures. They address
...
AETHER supports this in two ways:

- **AETHER's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, AETHER's in-process heuristics
...
AETHER filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in AETHER
itself. Bugs in AETHER's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   AETHER does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, AETHER's own documentation, or reasonable
  operator expectations would predict — including cases where
  AETHER has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  aether-home security, and similar. Reports against those
...
- AETHER includes supply-chain guards for MCP server
