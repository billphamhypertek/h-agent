...
#   - cli.py, aether_cli/*      -> inline on startup (no event loop)
...
def get_tool_definitions(
    ...
    if quiet_mode:
        ...
        cache_key = (
            ...
            cfg_fp,
            bool(os.environ.get("AETHER_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if os.environ.get("AETHER_KANBAN_TASK") and "kanban" not in effective_enabled_toolsets:
            # Dispatcher-spawned workers are scoped by AETHER_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like aether-cli)
    # is enabled, any tools belonging to a disabled toolset are strictly
    # stripped out. See issue #17309.
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                if toolset_name.startswith("aether-"):
                    # Platform bundles (aether-*) include _AETHER_CORE_TOOLS, so
    ...
    # window). Core AETHER tools (toolsets._AETHER_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from aether_cli.config import load_config as _load
        cfg = _load() or {}
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from aether_cli.plugins import has_hook, invoke_hook
...
def handle_function_call(
    ...
    if not skip_tool_request_middleware:
        try:
            from aether_cli.middleware import apply_tool_request_middleware
    ...
    try:
        ...
        try:
            ...
            from aether_cli.middleware import run_tool_execution_middleware
