...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_aether_home() / "sessions")
    ...
    # gateway behaves exactly as before — single AETHER_HOME, no profile stamping.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.aether/config.yaml (primary user-facing config)
    3. ~/.aether/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_aether_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through aether_cli.config.load_config, so without this a managed
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``aether config set gateway.streaming.*``
