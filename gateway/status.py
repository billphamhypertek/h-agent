...
The PID file lives at ``{AETHER_HOME}/gateway.pid``.  AETHER_HOME defaults to
``~/.aether`` but can be overridden via the environment variable.  This means
separate AETHER_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting AETHER_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("AETHER_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "aether" / _LOCKS_DIRNAME
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the AETHER gateway lifecycle subcommand from a command line.
    ...
    test also matched ``aether_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\aether-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- AETHER's
    ...
    for token in tokens:
        ...
        if basename in ("aether-gateway", "aether-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "aether_cli.main" in joined
        or "aether_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("aether", "aether.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `aether gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating AETHER-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the AETHER gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a AETHER_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.aether`` or ``$AETHER_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``aether_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``AETHER_HOME=<path>``) on its argv; the default/root gateway runs
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"aether_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit AETHER_HOME= on the argv. AETHER_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "aether_home=" in command_lc and f"aether_home={home_lc}" not in command_lc:
        return False
    return True
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `aether status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``AETHER_HOME`` in-process.  Defaults to
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    AETHER_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different AETHER_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``aether.service`` + ``aether-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # ``aether gateway stop`` on Windows would be misclassified as an
