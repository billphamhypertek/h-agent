...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-AETHER-Session-Id header; opt-in long-term memory scoping via X-AETHER-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-AETHER-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists aether-agent as an available model
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible AETHER sessions
- POST /api/sessions               — create an empty AETHER session
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to aether-agent
...
def _aether_version() -> str:
    """Return the aether-agent version string, or "dev" if it can't be resolved.

    Tries the installed package metadata first (authoritative for a pip/uv
    install), then the in-tree ``aether_cli.__version__`` (covers editable /
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_aether_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        ...
        # gracefully on NFS/SMB/FUSE-mounted AETHER_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # aether_state._WAL_INCOMPAT_MARKERS).
...
def _derive_chat_session_id(
    ...
    the same AETHER session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through aether-agent's AIAgent.
    """
    ...
    def _resolve_max_concurrent_runs() -> int:
        ...
        default = 10
        try:
            from aether_cli.config import cfg_get, load_config
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-AETHER-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-AETHER-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-AETHER-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-AETHER-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-AETHER-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``aether sessions list``
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the aether-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-AETHER-Session-Key``).  Unlike ``session_id``
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "aether-agent", "version": _aether_version()}
        )
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — return aether-agent as an available model."""
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every AETHER version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side AETHER AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-AETHER-Session-Id",
                "session_key_header": "X-AETHER-Session-Key",
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted AETHER sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions — create an empty AETHER session row."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({"object": "aether.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "aether.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "aether.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "aether.session", "session": self._session_response(fork)}, status=201)
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        headers = {"X-AETHER-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-AETHER-Session-Key"] = gateway_session_key
        return web.json_response(
            {
                "object": "aether.session.chat.completion",
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-AETHER-Session-Key.  This
        # is independent of X-AETHER-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-AETHER-Session-Id.
        ...
        provided_session_id = request.headers.get("X-AETHER-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-AETHER-Session-Id rejected: "
            ...
            # conversation map to the same AETHER session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``aether.tool.progress`` with ``status: running``.
        ...
        response_headers = {
            "X-AETHER-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-AETHER-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            err_body["error"]["aether"] = {
                "completed": completed,
                "partial": is_partial,
                "failed": is_failed,
            }
            response_headers["X-AETHER-Completed"] = "false"
            response_headers["X-AETHER-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + AETHER-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-AETHER-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-AETHER-Error"] = err_msg[:200]
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-AETHER-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-AETHER-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: aether.tool.progress`` SSE event so
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = {"X-AETHER-Session-Id": session_id}
        if gateway_session_key:
            response_headers["X-AETHER-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    async def _run_agent(
        ...
        def _run():
            ...
            try:
                ...
                # (e.g. X-AETHER-Session-Id header) can track compression-
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = (
            {"X-AETHER-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "aether.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local AETHER-Relay
            ...
            # the June 2026 aether-0day hardening (an 8-char key dispatching
            ...
            # that is the exact surface the aether-0day campaign abused to write
            # ~/.aether/config.yaml and plant persistence. Sandboxing (Docker /
            ...
            if is_network_accessible(self._host):
                try:
                    from aether_cli.config import load_config as _load_cfg
                    _backend = (
