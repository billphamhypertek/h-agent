...
# Audio file extensions AETHER recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through AETHER' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. AETHER-created Telegram private-chat
...
from aether_constants import get_default_aether_root, get_aether_dir, get_aether_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.aether/.env manually."
)
...
# Default location: {AETHER_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_aether_dir("cache/images", "image_cache")
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; AetherAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
AUDIO_CACHE_DIR = get_aether_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_aether_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_aether_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_aether_dir("cache/screenshots", "browser_screenshots")
_AETHER_HOME = get_aether_home()
_AETHER_ROOT = get_default_aether_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "AETHER_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "AETHER_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "AETHER_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "AETHER_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _AETHER_HOME / "image_cache",
    _AETHER_HOME / "audio_cache",
    _AETHER_HOME / "video_cache",
    _AETHER_HOME / "document_cache",
    _AETHER_HOME / "browser_screenshots",
    ...
    _AETHER_HOME / "cache" / "images",
    _AETHER_HOME / "cache" / "audio",
    _AETHER_HOME / "cache" / "videos",
    _AETHER_HOME / "cache" / "documents",
    _AETHER_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active AETHER profile and shared AETHER root both contain control
    ...
    # AETHER_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.aether
    ...
    for aether_root in (_AETHER_HOME, _AETHER_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(aether_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and AETHER
    secrets (``~/.aether/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``AETHER_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    AETHER-managed cache, under an operator-allowlisted root
    (``AETHER_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the AETHER root (~/.aether/.env,
    ...
    # ``MEDIA:~/.aether/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?aether\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+aether[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``AETHER_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # AETHER commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("AETHER_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "AETHER_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "AETHER_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: AETHER sends
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.aether/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from aether_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          AETHER_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          AETHER_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          AETHER_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("AETHER_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("AETHER_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("AETHER_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _aether_run_generation is set on
