...
There are two ways to add a platform to the AETHER gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.aether/plugins/` (or under `plugins/platforms/`
...
**zero changes to core AETHER code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `aether gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `aether_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the AETHER core.
...
And add it to the `aether-gateway` composite:

```python
"aether-gateway": {
    "includes": [..., "aether-your-platform"]
}
```
...
## 12. Status Display (`aether_cli/status.py`)
...
## 13. Gateway Setup Wizard (`aether_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ aether_cli/ toolsets.py \
  --include="*.py" -l | sort -u
