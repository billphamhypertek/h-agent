...
# ``sessions_dir / f"{session_id}.json"`` in aether_state, request-dump
...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `aether tools` (opt-in, default OFF).
...
def build_session_context_prompt(
    ...
    *,
    ...
    if context.source.platform == Platform.SLACK:
        ...
        # `discord` / `discord_admin` via `aether tools` AND the bot
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_aether_home()}/cron/output/)"
    )
    ...
    def _save(self) -> None:
        ...
        data = {
            "_README": (
                ...
                "~/.aether/state.db and are shown by `aether sessions list` and "
