...
When running AETHER inside Docker, the default `network_mode: host` gives the
...
The AETHER [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   AETHER_UID=$(id -u) AETHER_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=aether,aether-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://aether-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — AETHER trust model and vulnerability reporting
