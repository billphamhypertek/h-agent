# AETHER Observer Hooks

AETHER observer hooks are the read-only telemetry contract for plugins that
...
They do not replace AETHER' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. AETHER catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so AETHER only builds
