...
This document is the formal interface between the **AETHER gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `aether gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`aether gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `aether` CLI.
