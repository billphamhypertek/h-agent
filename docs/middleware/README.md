...
AETHER middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing AETHER'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before AETHER calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving AETHER retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving AETHER guardrails,
...
- `telemetry_schema_version`: currently `aether.observer.v1`
- `middleware_schema_version`: currently `aether.middleware.v1`
...
AETHER stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, AETHER runs
them as a nested chain in registration order. Middleware failures are fail-open:
AETHER logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, AETHER applies middleware in this order:
...
For each tool call, AETHER applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal AETHER pre-execution path against the effective arguments:
...
```bash
aether plugins enable <plugin-name>
```

For isolated local testing, use one `AETHER_HOME` for plugin enablement and the
agent run:

```bash
export AETHER_HOME=/tmp/aether-middleware-test
mkdir -p "$AETHER_HOME"
aether plugins enable <plugin-name>
aether chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run aether plugins enable <plugin-name>
uv run aether chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["aether_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/aether-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape AETHER expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, AETHER treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, AETHER preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. AETHER does not convert downstream
