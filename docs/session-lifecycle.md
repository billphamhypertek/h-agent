...
The canonical transcript store is SQLite via `SessionDB` (from `aether_state`). The
...
Counts consecutive restarts via a JSON file (`{AETHER_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `aether update`, `aether gateway restart`,
or `/restart`.
