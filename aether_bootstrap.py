"""Windows UTF-8 bootstrap for AETHER entry points.
...
should be imported at the very top of every AETHER entry point
(``aether``, ``aether-agent``, ``aether-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing AETHER modules.

    AETHER ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real AETHER source root
    ...
    """
    root = src_root or os.environ.get("AETHER_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``AETHER_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("AETHER_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import aether_bootstrap``
# (or ``from aether_bootstrap import apply_windows_utf8_bootstrap``) at
