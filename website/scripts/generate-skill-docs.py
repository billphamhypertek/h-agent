...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/billphamhypertek/h-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    aether_meta = (fm.get("metadata") or {}).get("aether") or {}
    tags = aether_meta.get("tags") or []
    related = aether_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `aether skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with AETHER"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "AETHER ships with a large built-in skill library copied into `~/.aether/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "AETHER also syncs bundled skills on `aether update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.aether/skills/` tree, it is still shipped with AETHER; restore it with `aether skills reset <name> --restore`.",
        "",
