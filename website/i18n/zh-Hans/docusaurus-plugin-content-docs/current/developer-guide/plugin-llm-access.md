...
* 参考 plugin（配套仓库）：
  * [`plugin-llm-example`](https://github.com/hypertek/aether-example-plugins/tree/main/plugin-llm-example) — 带图像输入的同步结构化提取
  * [`plugin-llm-async-example`](https://github.com/hypertek/aether-example-plugins/tree/main/plugin-llm-async-example) — 使用 `asyncio.gather()` 的异步示例
* 辅助客户端（底层引擎）：参见
