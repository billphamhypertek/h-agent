...
title: "Git Worktrees"
description: "Run multiple AETHER agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

AETHER is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with AETHER so each session has a clean, isolated working directory.

## Why Use Worktrees with AETHER?

AETHER treats the **current working directory** as the project root:

- CLI: the directory where you run `aether` or `aether chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.aether/config.yaml`
...
git worktree add ../repo-feature feature/aether-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/aether-experiment` checked out in that directory

Now you can `cd` into the new worktree and run AETHER there:

```bash
cd ../repo-feature

# Start AETHER in the worktree
aether
```
...
git worktree add ../repo-experiment-a feature/aether-a
git worktree add ../repo-experiment-b feature/aether-b
```
...
- Works on its own branch (`feature/aether-a` vs `feature/aether-b`).
...
- AETHER checkpoint data under `~/.aether/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per AETHER experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/aether-checkpoints-docs`, `feature/aether-refactor-tests`.
- **Commit frequently**
...
- **Avoid running AETHER from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `aether -w` (Automatic Worktree Mode)

AETHER has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `aether/aether-<hash>`).
...
```bash
aether -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `aether -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each AETHER session its own clean checkout.
