...
| | |
|---|---|
| Source | Optional — install with `aether skills install official/autonomous-ai-agents/grok` |
| Path | `optional-skills/autonomous-ai-agents/grok` |
| Version | `0.1.0` |
| Author | Matt Maximo (MattMaximo), AETHER |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Coding-Agent`, `Grok`, `xAI`, `Code-Review`, `Refactoring`, `Automation` |
| Related skills | [`codex`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`aether-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-aether-agent) |
...
# Grok Build CLI — AETHER Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the AETHER terminal. Grok
...
1. Prepare stable input files first with AETHER tools (`read_file`,
...
2. **Don't conflate AETHER' xAI auth with the `grok` CLI's auth.** AETHER'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume AETHER' xAI auth carries
   over.
