...
description: "Configure and use Honcho memory with AETHER -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with AETHER -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `aether skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for AETHER

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every AETHER profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/aether#running-honcho-locally-with-aether

### Verify

```bash
aether honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this AETHER profile's AI peer
...
Honcho models conversations as interactions between **peers**. AETHER creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this AETHER instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each AETHER run |
| `global` | Single session across all directories |

Manual override: `aether honcho map my-project-name`
...
Each AETHER profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block aether.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `aether.coder` host block in `honcho.json`
...
```bash
aether honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for AETHER when Honcho memory is active.
...
Config file: `$AETHER_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `aether honcho setup`. Ensure `memory.provider: honcho` is in `~/.aether/config.yaml`.

### Memory not persisting across sessions
Check `aether honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `aether profile create <name> --clone`. For existing profiles: `aether honcho sync`.
...
| Command | Description |
|---------|-------------|
| `aether honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `aether honcho status` | Show resolved config, connection test, peer info for active profile |
| `aether honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `aether honcho disable` | Disable Honcho for the active profile |
| `aether honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `aether honcho peers` | Show peer identities across all profiles |
| `aether honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `aether honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `aether honcho sessions` | List known directory-to-session-name mappings |
| `aether honcho map <name>` | Map current working directory to a Honcho session name |
| `aether honcho identity` | Seed AI peer identity or show both peer representations |
| `aether honcho sync` | Create host blocks for all AETHER profiles that don't have one yet |
| `aether honcho migrate` | Step-by-step migration guide from OpenClaw native memory to AETHER + Honcho |
| `aether memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `aether memory status` | Show active memory provider and config |
| `aether memory off` | Disable external memory provider |
