...
| | |
|---|---|
| Source | Optional — install with `aether skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into AETHER or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to AETHER. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.aether/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.aether/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is AETHER integration, either:

- configure the server in `~/.aether/config.yaml` using the `native-mcp` skill, or
...
### AETHER cannot see the deployed server

The server-building part may be correct while the AETHER config is not. Load the `native-mcp` skill and configure the server in `~/.aether/config.yaml`, then restart AETHER.
