...
| | |
|---|---|
| Source | Optional — install with `aether skills install official/productivity/canvas` |
...
4. Name the token (e.g., "AETHER"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${AETHER_HOME:-~/.aether}/.env`:
...
```bash
CANVAS="python $AETHER_HOME/skills/productivity/canvas/scripts/canvas_api.py"
