...
| | |
|---|---|
| Source | Optional — install with `aether skills install official/productivity/shopify` |
...
4. Save to `${AETHER_HOME:-~/.aether}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"AETHER","productType":"Apparel","tags":["test"]}}'
```
