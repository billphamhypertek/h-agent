...
| | |
|---|---|
| Source | Optional — install with `aether skills install official/productivity/siyuan` |
...
3. Store it in `${AETHER_HOME:-~/.aether}/.env`:
   ```
...
```yaml
# In ~/.aether/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
