...
| | |
|---|---|
| Source | Optional — install with `aether skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${AETHER_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${AETHER_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${AETHER_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client aether
```
...
```bash
PUBLISH="${AETHER_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client aether
```
...
```bash
DRIVE="${AETHER_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `aether`)    |
