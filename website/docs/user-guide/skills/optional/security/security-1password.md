...
| | |
|---|---|
| Source | Optional — install with `aether skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by AETHER |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during AETHER terminal calls (desktop app flow only)
...
### Service Account (recommended for AETHER)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${AETHER_HOME:-~/.aether}/.env` (the skill will prompt for this on first load).
...
## AETHER Execution Pattern (desktop app flow)

AETHER terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/aether-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/aether-op.sock"
