---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into AETHER"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into AETHER"
---
...
Migrate a user's OpenClaw customization footprint into AETHER. Imports AETHER-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `aether skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | AETHER (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `AETHER`, `Memory`, `Persona`, `Import` |
| Related skills | [`aether-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-aether-agent) |
...
# OpenClaw -> AETHER Migration

Use this skill when a user wants to move their OpenClaw setup into AETHER with minimal manual cleanup.
...
```bash
aether claw migrate              # Full interactive migration
...
aether claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `aether setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_aether.py` to:

- import `SOUL.md` into the AETHER home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into AETHER memory entries
- merge OpenClaw command approval patterns into AETHER `command_allowlist`
- migrate AETHER-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to AETHER working-directory configuration
- copy OpenClaw skills into `~/.aether/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen AETHER workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.aether/tts/`
- archive non-secret docs that do not have a direct AETHER destination
...
- `scripts/openclaw_to_aether.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.aether/skills/migration/openclaw-migration/scripts/openclaw_to_aether.py`

Do not guess a shorter path like `~/.aether/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.aether/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of AETHER-compatible secrets, currently:
...
AETHER CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in AETHER. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a AETHER workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing AETHER skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.aether/skills/migration/openclaw-migration/scripts/openclaw_to_aether.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.aether/skills/migration/openclaw-migration/scripts/openclaw_to_aether.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.aether/skills/migration/openclaw-migration/scripts/openclaw_to_aether.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.aether/skills/migration/openclaw-migration/scripts/openclaw_to_aether.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.aether/skills/migration/openclaw-migration/scripts/openclaw_to_aether.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.aether/skills/migration/openclaw-migration/scripts/openclaw_to_aether.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of AETHER unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty AETHER targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean AETHER destination. Unsupported auth blobs must still be reported as skipped.
...
- AETHER persona state imported
- AETHER memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.aether/skills/openclaw-imports/`
