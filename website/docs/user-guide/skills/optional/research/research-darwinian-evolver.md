...
| | |
|---|---|
| Source | Optional — install with `aether skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), AETHER |
| License | MIT |
...
import upstream classes into AETHER itself.
...
```bash
mkdir -p ~/.aether/cache/darwinian-evolver && cd ~/.aether/cache/darwinian-evolver
...
```bash
cd ~/.aether/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.aether/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.aether/skills/research/darwinian-evolver
DE_DIR=~/.aether/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside AETHER core.
   Custom driver scripts under `~/.aether/skills/...` are user-side and fine.
