...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _aether_env="${AETHER_HOME:-$HOME/.aether}/.env"; [ -f "$_aether_env" ] && grep -q "^GITHUB_TOKEN=" "$_aether_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_aether_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source "${AETHER_HOME:-$HOME/.aether}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by AETHER. Code looks clean — good test coverage, no security concerns."
