...
- Give it a name like "aether-agent"
- Select scopes:
...
- Give it a title like "aether-agent-&lt;machine-name>"
...
elif _aether_env="${AETHER_HOME:-$HOME/.aether}/.env"; [ -f "$_aether_env" ] && grep -q "^GITHUB_TOKEN=" "$_aether_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_aether_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
