...
sidebar_position: 1
title: "CLI Interface"
description: "Master the AETHER terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

AETHER's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `aether setup --portal` — and you're ready to `aether chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
AETHER also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `aether --tui` — see the [TUI](tui.md) guide.
:::
...
aether chat --model "anthropic/claude-sonnet-4"

# With a specific provider
aether chat --provider nous        # Use Nous Portal
aether chat --provider openrouter  # Force OpenRouter
...
aether chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
aether --continue             # Resume the most recent CLI session (-c)
aether --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
aether chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
aether -w                         # Interactive mode in worktree
aether -w -z "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the AETHER CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The AETHER CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `AETHER_YOLO_MODE` is on (either `aether --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
When resuming a previous session (`aether -c` or `aether --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend AETHER to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. AETHER supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for AETHER replies |
...
```bash
aether -s aether-agent-dev,github-auth
aether chat -s github-pr-workflow -s github-auth
```

AETHER loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.aether/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.aether/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain AETHER..."
  # Add your own!
```
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. AETHER recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches AETHER — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while AETHER is working, AETHER prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend AETHER to the background — just like any terminal process. The shell prints a confirmation:

```
AETHER has been suspended. Run `fg` to bring AETHER back.
```
...
Resume this session with:
  aether --resume 20260225_143052_a1b2c3
...
```bash
aether --continue                          # Resume the most recent CLI session
aether -c                                  # Short form
aether -c "my project"                     # Resume a named session (latest in lineage)
aether --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
aether --resume "refactoring auth"         # Resume by title
aether -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `aether sessions rename <id> <title>` from the command line. Use `aether sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in AETHER's SQLite state database under `~/.aether/state.db`. The database keeps:
...
AETHER immediately confirms the task and gives you back the prompt:
