...
`~/.aether/config.yaml`, `~/.aether/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/aether`:

```text
/etc/aether/
├── config.yaml     # managed config layer (wins over ~/.aether/config.yaml)
└── .env            # managed env layer (wins over ~/.aether/.env + shell)
```
...
The location can be relocated with the `AETHER_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `AETHER_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by AETHER.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export AETHER_MANAGED_DIR=/opt/org/aether-policy
```

:::warning
A user who can set `AETHER_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `aether doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/aether/config.yaml` (managed) | `/etc/aether/.env` (managed) |
| 2 | `~/.aether/config.yaml` (user) | `~/.aether/.env` (user) |
...
```bash
aether config        # shows a header naming the managed source + the pinned keys
aether doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, AETHER refuses and names the source:

```bash
$ aether config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/aether/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `aether config set` / setup will not write
...
sudo tee /etc/aether/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/aether/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/aether
sudo chmod 0644 /etc/aether/config.yaml /etc/aether/.env
```

Changes take effect on the next AETHER start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`aether doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs AETHER as `root`), managed scope is advisory.
