...
If you only run one AETHER agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more AETHER agents that should all
...
(`ai.aether.gateway-<name>.plist`) or systemd user service
(`aether-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
aether profile create coder
aether profile create personal-bot
aether profile create research
...
```bash
aether config set gateway.multiplex_profiles true
aether gateway restart
```

Equivalently, in the default profile's `~/.aether/config.yaml`:
...
You do **not** run `aether gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `aether gateway start` / `run` is a
...
home). `aether status` reports the multiplexer and the profiles it serves;
`aether status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/aether-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: aether-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    aether gateway "$action"
  else
    aether -p "$profile" gateway "$action"
  fi
}
...
```bash
aether-gateways start      # start every configured profile
aether-gateways stop       # stop every configured profile
aether-gateways restart    # restart all
aether-gateways status     # status across all
aether-gateways list       # delegates to `aether gateway list`
```

:::tip
The `default` profile is targeted with `aether gateway <action>` (no `-p`),
not `aether -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `aether -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.aether.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/aether-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.aether.gateway.plist` /
...
```bash
# Default profile
tail -f ~/.aether/logs/gateway.log
tail -f ~/.aether/logs/gateway.error.log

# Named profile
tail -f ~/.aether/profiles/<name>/logs/gateway.log
tail -f ~/.aether/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.aether/logs/gateway.log ~/.aether/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
aether logs -f                  # follow default profile
aether -p coder logs -f         # follow one profile
aether logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
aether profile list             # profiles + model + gateway state
aether-gateways status          # full status across every profile
launchctl list | grep aether    # macOS — PIDs and labels
systemctl --user list-units 'aether-gateway-*'   # Linux — units
```
...
```
~/.aether/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.aether/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
aether config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.aether/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=aether --why="gateways running" \
  sleep infinity &
...
`aether-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.aether/.env ~/.aether/profiles/*/.env
```

## Updating the code

`aether update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `aether gateway start` after a previous `aether gateway stop`. The
...
```bash
ps -ef | grep "aether_cli.*-p <profile>"
cat ~/.aether/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.aether.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.aether.gateway-<profile>.plist

# Linux
systemctl --user restart aether-gateway-<profile>.service
```

### Health check

```bash
aether doctor                  # default profile
aether -p <profile> doctor     # one profile
```
