...
AETHER automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.aether/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
AETHER stores session history so it can resume conversations, but it does not
...
AETHER explicitly injects for that turn.
...
For example, if a user sends an image and asks AETHER to make a meme from it,
AETHER may inspect that image once with vision and run an image-processing
...
`aether sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`aether` or `aether chat`) |
...
aether -c "my project"   # → resumes "my project #3"
```
...
aether --resume 20250305_091523_a1b2c3d4
aether -r 20250305_091523_a1b2c3d4

# Resume by title
aether --resume "refactoring auth"

# Or with the chat subcommand
aether chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `aether sessions list`.

### Conversation Recap on Resume

When you resume a session, AETHER displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a AETHER session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.aether/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `aether -r "<title>"` from the shell) and pick up where the platform left off.
...
AETHER automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `aether sessions list` or `aether sessions browse`.
...
```bash
aether sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), AETHER creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`aether -c "my project"`), it automatically picks the most recent session in the lineage.
...
AETHER provides a full set of session management commands via `aether sessions`:
...
# Filter by platform
aether sessions list --source telegram

# Show more sessions
aether sessions list --limit 50
```
...
aether sessions export backup.jsonl

# Export sessions from a specific platform
aether sessions export telegram-history.jsonl --source telegram

# Export a single session
aether sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
aether sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
aether sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
aether sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
aether sessions prune

# Custom age threshold
aether sessions prune --older-than 30

# Only prune sessions from a specific platform
aether sessions prune --source telegram --older-than 60

# Skip confirmation
aether sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`aether insights`](/reference/cli-commands#aether-insights).
...
When AETHER cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, AETHER uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to AETHER in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.aether/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.aether/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.aether/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |

The SQLite database uses WAL mode for concurrent readers and a single writer, which suits the gateway's multi-platform architecture well.

:::warning `sessions.json` is not the session list
`~/.aether/sessions/sessions.json` is the **gateway routing index** — it maps
...
`aether sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.aether/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `aether sessions list`, the cause is
`state.db` not receiving them — run `aether sessions repair` and watch for a
...
`*.jsonl` files in `~/.aether/sessions/`. They are no longer written or
read by AETHER. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every AETHER process in the same `AETHER_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.aether/config.yaml`:
...
aether sessions delete <session_id>
