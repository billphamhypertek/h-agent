...
sidebar_position: 12
title: "Google Chat"
description: "Set up AETHER as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect AETHER to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your AETHER
...
> Run `aether gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only AETHER can read (e.g.,
`~/.aether/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a aether restart)
...
- Principal: `aether-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — AETHER calls
...
- **App name**: whatever you want users to see ("AETHER" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/aether-chat-events`.
...
`ADDED_TO_SPACE` event that AETHER uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure AETHER

Add the Google Chat section to `~/.aether/.env`:
...
Install the dependencies the Google Chat adapter needs (no AETHER extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "AETHER is thinking…" marker, then
...
Thread support: when a user replies inside a thread, AETHER detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate AETHER session.
...
3. Download the JSON. Move it onto the host that runs AETHER.
4. Register the client with AETHER (run under the profile you want it scoped to):
...
aether -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's AETHER home (e.g.
`~/.aether/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.aether/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.aether/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, AETHER isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `aether gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: AETHER will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.aether/google_chat_user_tokens/<sanitized_email>.json` (filesystem
