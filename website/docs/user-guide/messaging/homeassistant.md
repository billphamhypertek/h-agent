...
description: Control your smart home with AETHER via Home Assistant integration.
...
AETHER integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "AETHER"
5. Copy the token
...
AETHER registers four tools for smart home control:
...
Configure which events the agent sees in `~/.aether/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "AETHER".
...
The adapter reads credentials from `~/.aether/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active AETHER profile
...
running AETHER — `curl -H "Authorization: Bearer <token>" <url>/api/` should
