...
sidebar_position: 16
title: "Yuanbao"
description: "Connect AETHER to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect AETHER to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.aether/.env`:
...
All standard AETHER commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.aether/logs/gateway.log`
...
4. Consider enabling verbose logging: `AETHER_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. AETHER automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
AETHER_LOG_LEVEL=debug aether gateway
```
...
```bash
aether chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
