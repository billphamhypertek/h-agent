...
sidebar_position: 15
title: "Weixin (WeChat)"
description: "Connect AETHER to personal WeChat accounts via the iLink Bot API"
---

# Weixin (WeChat)

Connect AETHER to [WeChat](https://weixin.qq.com/) (微信), Tencent's personal messaging platform. The adapter uses Tencent's **iLink Bot API** for personal WeChat accounts — this is distinct from WeCom (Enterprise WeChat). Messages are delivered via long-polling, so no public endpoint or webhook is required.
...
QR login connects AETHER to an **iLink bot identity** (e.g. `a5ace6fd482e@im.bot`), **not** a fully scriptable ordinary personal WeChat account. Consequences:
...
- The `WEIXIN_GROUP_POLICY` / `WEIXIN_GROUP_ALLOWED_USERS` settings below only take effect when iLink actually returns group events for your account type. If it doesn't, group messages will never reach AETHER regardless of policy.

In practice, most deployments only get DMs to the iLink bot working reliably. If group delivery doesn't work after configuration, the limitation is on the iLink side, not in AETHER. The gateway logs a `WARNING` at startup whenever `WEIXIN_GROUP_POLICY` is set to anything other than `disabled`.
:::
...
- Terminal QR rendering is included when AETHER is installed with the `messaging` extra
...
5. Save the account credentials automatically to `~/.aether/weixin/accounts/`
...
After initial QR login, set at minimum the account ID in `~/.aether/.env`:
...
- **QR code login** — scan-to-connect setup via `aether gateway setup`
...
login connects one iLink bot identity to AETHER. Other people do not scan the
AETHER QR code with their own accounts; they must message the connected iLink
bot/contact through WeChat, and AETHER will process the DM only if the sender's
Weixin user ID is present in `WEIXIN_ALLOWED_USERS`.

A practical setup flow is:

1. Pair AETHER once with `aether gateway setup` and note the connected iLink bot
   account.
...
If only the account that scanned the QR code can talk to AETHER, verify that the
...
- Tokens are saved per account+peer to `~/.aether/weixin/accounts/<account_id>.context-tokens.json`
...
| `Weixin startup failed: WEIXIN_TOKEN is required` | Run `aether gateway setup` to complete QR login, or set `WEIXIN_TOKEN` manually |
| `Weixin startup failed: WEIXIN_ACCOUNT_ID is required` | Set `WEIXIN_ACCOUNT_ID` in your `.env` or run `aether gateway setup` |
| `Another local AETHER gateway is already using this Weixin token` | Stop the other gateway instance first — only one poller per token is allowed |
| Session expired (`errcode=-14`) | Your login session has expired. Re-run `aether gateway setup` to scan a new QR code |
...
| Terminal QR code doesn't render | Reinstall with the messaging extra: `cd ~/.aether/aether-agent && uv pip install -e ".[messaging]"`. Alternatively, open the URL printed above the QR |
