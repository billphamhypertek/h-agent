...
Connect AETHER to **iMessage** through [Photon][photon], a managed
...
your first iMessage from AETHER — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, AETHER runs it in a
...
aether photon setup --phone +15551234567
```
...
2. **Finds or creates** the `AETHER` project on your account.
...
Runtime credentials are written to `~/.aether/.env`
...
(device token, dashboard project id) lives in `~/.aether/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other AETHER
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, AETHER replies with a pairing code. Approve it with:

```bash
aether pairing approve photon <CODE>
```

Use `aether pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.aether/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.aether/.env`):
...
By default AETHER responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `AETHER` and
`@AETHER agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and AETHER will reply.

## Status & troubleshooting

```bash
aether photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line AETHER uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run aether photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `aether photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `aether photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `aether photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** AETHER sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | AETHER wake words  | JSON list / comma / newline regex patterns for group mentions |
