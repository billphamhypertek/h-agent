...
sidebar_position: 8
title: "Open WebUI"
description: "Connect Open WebUI to AETHER via the OpenAI-compatible API server"
---

# Open WebUI Integration

[Open WebUI](https://github.com/open-webui/open-webui) (126k★) is the most popular self-hosted chat interface for AI. With AETHER's built-in API server, you can use Open WebUI as a polished web frontend for your agent — complete with conversation management, user accounts, and a modern chat interface.
...
flowchart LR
    ...
    B["aether-agent<br/>gateway API server<br/>port 8642"]
    A -->|POST /v1/chat/completions| B
    B -->|SSE streaming response| A
```

Open WebUI connects to AETHER's API server just like it would connect to OpenAI. AETHER handles the requests with its full toolset — terminal, file operations, web search, memory, skills — and returns the final response.

:::important Runtime location
The API server is a **AETHER agent runtime**, not a pure LLM proxy. For each request, AETHER creates a server-side `AIAgent` on the API-server host. Tool calls run where that API server is running.

For example, if a laptop points Open WebUI or another OpenAI-compatible client at a AETHER API server on a remote machine, `pwd`, file tools, browser tools, local MCP tools, and other workspace tools run on the remote API-server host, not on the laptop.
:::

Open WebUI talks to AETHER server-to-server, so you do not need `API_SERVER_CORS_ORIGINS` for this integration.

## Quick Setup

### One-command local bootstrap (macOS/Linux, no Docker)

If you want AETHER + Open WebUI wired together locally with a reusable launcher, run:
...
- ensures `~/.aether/.env` contains `API_SERVER_ENABLED`, `API_SERVER_HOST`, `API_SERVER_KEY`, `API_SERVER_PORT`, and `API_SERVER_MODEL_NAME`
- restarts the AETHER gateway so the API server comes up
- installs Open WebUI into `~/.local/open-webui-venv`
- writes a launcher at `~/.local/bin/start-open-webui-aether.sh`
- on macOS, installs a `launchd` user service; on Linux with `systemd --user`, installs a user service there

Defaults:

- AETHER API: `http://127.0.0.1:8642/v1`
- Open WebUI: `http://127.0.0.1:8080`
- model name advertised to Open WebUI: `AETHER`

Useful overrides:

```bash
OPEN_WEBUI_NAME='My AETHER UI' \
OPEN_WEBUI_ENABLE_SIGNUP=true \
AETHER_API_MODEL_NAME='My AETHER' \
bash scripts/setup_open_webui.sh
```
...
```bash
aether config set API_SERVER_ENABLED true
aether config set API_SERVER_KEY your-secret-key
```

`aether config set` auto-routes the flag to `config.yaml` and the secret to `~/.aether/.env`. If the gateway is already running, restart it so the change takes effect:

```bash
aether gateway stop && aether gateway
```
...
# {"object":"list","data":[{"id":"aether-agent", ...}]}
```
...
Go to **http://localhost:3000**. Create your admin account (the first user becomes admin). You should see your agent in the model dropdown (named after your profile, or **aether-agent** for the default profile). Start chatting!
...
6. Enter:
   ...
   - **API Key**: the exact same value as `API_SERVER_KEY` in AETHER
7. Click the **checkmark** to verify the connection
8. **Save**

Your agent model should now appear in the model dropdown (named after your profile, or **aether-agent** for the default profile).
...
This is the default and requires no extra configuration. Open WebUI sends standard OpenAI-format requests and AETHER responds accordingly. Each request includes the full conversation history.
...
2. Edit your aether-agent connection
3. Change **API Type** from "Chat Completions" to **"Responses (Experimental)"**
4. Save

With the Responses API, Open WebUI sends requests in the Responses format (`input` array + `instructions`), and AETHER can preserve full tool call history across turns via `previous_response_id`. When `stream: true`, AETHER also streams spec-native `function_call` and `function_call_output` items, which enables custom structured tool-call UI in clients that render Responses events.
...
2. AETHER creates a server-side `AIAgent` instance using the API server's profile, model/provider config, memory, skills, and configured API-server toolsets
...
Your agent has access to the same tools and capabilities as that API-server AETHER instance. If the API server is remote, those tools are remote too.

If you need tools to run against your **local** workspace today, run AETHER locally and point it at a pure LLM provider or pure OpenAI-compatible model proxy (for example vLLM, LiteLLM, Ollama, llama.cpp, OpenAI, OpenRouter, etc.). A future split-runtime mode for "remote brain, local hands" is being tracked in [#18715](https://github.com/NousResearch/hermes-agent/issues/18715); it is not the behavior of the current API server.
...
|----------|-------------|
| `OPENAI_API_BASE_URL` | AETHER's API URL (include `/v1`) |
...
- **Check model listing**: `curl -H "Authorization: Bearer your-secret-key" http://localhost:8642/v1/models` should return a list with `aether-agent`
...
- **Empty Ollama backend shadowing the picker**: If you omitted `ENABLE_OLLAMA_API=false`, Open WebUI shows an empty Ollama section above your AETHER models. Restart the container with `-e ENABLE_OLLAMA_API=false` or disable Ollama in **Admin Settings → Connections**.
...
AETHER may be executing multiple tool calls (reading files, running commands, searching the web) before producing its final response. This is normal for complex queries. The response appears all at once when the agent finishes.

### "Invalid API key" errors

Make sure your `OPENAI_API_KEY` in Open WebUI matches the `API_SERVER_KEY` in AETHER.
...
To run separate AETHER instances per user — each with their own config, memory, and skills — use [profiles](/user-guide/profiles). Each profile runs its own API server on a different port and automatically advertises the profile name as the model in Open WebUI.
...
```bash
aether profile create alice
cat >> ~/.aether/profiles/alice/.env <<EOF
API_SERVER_ENABLED=true
API_SERVER_PORT=8650
API_SERVER_KEY=alice-secret
EOF

aether profile create bob
cat >> ~/.aether/profiles/bob/.env <<EOF
API_SERVER_ENABLED=true
...
```bash
aether -p alice gateway &
aether -p bob gateway &
```
...
The model dropdown will show `alice` and `bob` as distinct models. You can assign models to Open WebUI users via the admin panel, giving each user their own isolated AETHER agent.
...
```bash
aether -p alice config set API_SERVER_MODEL_NAME "Alice's Agent"
```
:::
