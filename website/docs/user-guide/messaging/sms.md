...
title: "SMS (Twilio)"
description: "Set up AETHER as an SMS chatbot via Twilio"
---

# SMS Setup (Twilio)

AETHER connects to SMS through the [Twilio](https://www.twilio.com/) API. People text your Twilio phone number and get AI responses back — same conversational experience as Telegram or Discord, but over standard text messages.
...
- **aiohttp** — `cd ~/.aether/aether-agent && uv pip install -e ".[sms]"`
...
If you're running AETHER locally, use a tunnel to expose the webhook:
...
Text your Twilio number — AETHER will respond via SMS.
...
AETHER validates that inbound webhooks genuinely originate from Twilio by verifying the `X-Twilio-Signature` header (HMAC-SHA1). This prevents attackers from injecting forged messages.
...
3. Check AETHER gateway logs for Twilio API errors
