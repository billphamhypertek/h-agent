...
Connect AETHER to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
AETHER supports two WeCom integration modes:
...
> Run `aether gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. AETHER decrypts the message, queues it for the agent
...
(Use `aether gateway start` only after `aether gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in AETHER and the token the
...
in `~/.aether/.env` values around `=` will also break signature checks. After
fixing, restart `aether gateway run`.
...
Check `aether gateway run` logs for the bound host/port. If the adapter bound to
