...
description: "Chat with AETHER from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with AETHER from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with AETHER](/guides/use-voice-mode-with-aether).
...
For group chats, hooks, and automation flows, AETHER supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. AETHER keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; AETHER does not hide failures just because the text resembles a silence token.
...
```bash
aether gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
aether gateway              # Run in foreground
aether gateway setup        # Configure messaging platforms interactively
aether gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo aether gateway install --system   # Linux only: install a boot-time system service
aether gateway start        # Start the default service
aether gateway stop         # Stop the default service
aether gateway status       # Check default service status
aether gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update AETHER to the latest version |
...
Configure per-platform overrides in `~/.aether/gateway.json`:
...
aether pairing approve telegram XKGH5N7P

# Other pairing commands:
aether pairing list          # View pending + approved users
aether pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, AETHER appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.aether/config.yaml`:
...
Off by default. When enabled, AETHER prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.aether/config.yaml`:
...
```bash
AETHER_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
aether gateway install               # Install as user service
aether gateway start                 # Start the service
aether gateway stop                  # Stop the service
aether gateway status                # Check status
journalctl --user -u aether-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo aether gateway install --system
sudo aether gateway start --system
sudo aether gateway status --system
journalctl -u aether-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `aether update`. When `aether update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart aether-gateway` command (it never blocks on an interactive password prompt).
...
```bash
aether gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `aether update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo aether update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/aether-gateway`:

```
aether ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed aether-gateway*, /usr/bin/systemctl --no-ask-password start aether-gateway*, /usr/bin/systemctl --no-ask-password restart aether-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. AETHER will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple AETHER installations on the same machine (with different `AETHER_HOME` directories), each gets its own systemd service name. The default `~/.aether` uses `aether-gateway`; other installations use `aether-gateway-<hash>`. The `aether gateway` commands automatically target the correct service for your current `AETHER_HOME`.
:::

### macOS (launchd)

```bash
aether gateway install               # Install as launchd agent
aether gateway start                 # Start the service
aether gateway stop                  # Stop the service
aether gateway status                # Check status
tail -f ~/.aether/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.aether.gateway.plist`. It includes three environment variables:
...
- **AETHER_HOME** — scopes the gateway to your AETHER installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `aether gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `AETHER_HOME` directory gets its own launchd label. The default `~/.aether` uses `ai.aether.gateway`; other installations use `ai.aether.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `aether-cli` | Full access |
| Telegram | `aether-telegram` | Full tools including terminal |
| Discord | `aether-discord` | Full tools including terminal |
| WhatsApp | `aether-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `aether-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `aether-slack` | Full tools including terminal |
| Google Chat | `aether-google_chat` | Full tools including terminal |
| Signal | `aether-signal` | Full tools including terminal |
| SMS | `aether-sms` | Full tools including terminal |
| Email | `aether-email` | Full tools including terminal |
| Home Assistant | `aether-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `aether-mattermost` | Full tools including terminal |
| Matrix | `aether-matrix` | Full tools including terminal |
| DingTalk | `aether-dingtalk` | Full tools including terminal |
| Feishu/Lark | `aether-feishu` | Full tools including terminal |
| WeCom | `aether-wecom` | Full tools including terminal |
| WeCom Callback | `aether-wecom-callback` | Full tools including terminal |
| Weixin | `aether-weixin` | Full tools including terminal |
| BlueBubbles | `aether-bluebubbles` | Full tools including terminal |
| QQBot | `aether-qqbot` | Full tools including terminal |
| Yuanbao | `aether-yuanbao` | Full tools including terminal |
| Microsoft Teams | `aether-teams` | Full tools including terminal |
| API Server | `aether-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `aether-webhook` | Full tools including terminal |
| Raft | `aether-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.aether/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
