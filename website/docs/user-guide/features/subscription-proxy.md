...
chat completions — use your AETHER-managed provider subscription as their
...
| Use case | "Use AETHER as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. AETHER stores
the refresh token in `~/.aether/auth.json` — the same place all AETHER
...
interface in `aether_cli/proxy/adapters/`.
...
If you see `not logged in`, run `aether portal`. If you see
...
`aether_cli/proxy/adapters/<provider>.py` and registering it in
