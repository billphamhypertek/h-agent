...
AETHER runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. AETHER
...
1. AETHER captures a baseline of current diagnostics for the file.
...
…). AETHER auto-detects the binary on PATH or in
`<AETHER_HOME>/lsp/bin/`.
...
`node_modules` tree — AETHER installs both packages together when you
run `aether lsp install typescript` or auto-install fires on first
use.

## CLI

```
aether lsp status          # service state + per-server install status
aether lsp list            # registry, optionally --installed-only
aether lsp install <id>    # eagerly install one server
aether lsp install-all     # try every server with a known recipe
aether lsp restart         # tear down running clients
aether lsp which <id>      # print resolved binary path
```

`aether lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <AETHER_HOME>/lsp/bin
...
When `install_strategy: auto`, AETHER installs binaries into
`<AETHER_HOME>/lsp/bin/`. NPM packages land in
`<AETHER_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully AETHER-owned and is
...
Servers are kept alive for the life of the AETHER process. There's
...
**`aether lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<AETHER_HOME>/lsp/bin/`. Run
`aether lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `aether lsp status`**
...
When `aether lsp status` shows a `Backend warnings` section, install
...
`~/.aether/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.aether/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `aether lsp restart` to clear the set;
