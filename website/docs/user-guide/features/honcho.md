...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of AETHER's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple AETHER instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
aether memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `aether` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — AETHER syncs them back at session init.
...
The `aether honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `aether memory setup honcho` (or run `aether memory setup` and pick it from the list); the `aether honcho` subcommand then appears on the next invocation.

```bash
aether memory setup honcho    # Configure Honcho directly (works before activation)
aether honcho status          # Connection status, config, and key settings
aether honcho setup           # Redirects to `aether memory setup` (post-activation alias)
aether honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
aether honcho peer            # Show or update peer names + dialectic reasoning level
aether honcho mode            # Show or set recall mode (hybrid/context/tools)
aether honcho tokens          # Show or set token budget for context and dialectic
aether honcho identity        # Seed or show the AI peer's Honcho identity
aether honcho sync            # Sync Honcho config to all existing profiles
aether honcho peers           # Show peer identities across all profiles
aether honcho sessions        # List known Honcho session mappings
aether honcho map             # Map current directory to a Honcho session name
aether honcho enable          # Enable Honcho for the active profile
aether honcho disable         # Disable Honcho for the active profile
aether honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `aether honcho`

If you previously used the standalone `aether honcho setup`:
...
No re-login or re-setup needed. Run `aether memory setup` and select "honcho" — the wizard detects your existing config.
