...
AETHER ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
aether memory setup      # interactive picker + configuration
aether memory status     # check what's active
aether memory off        # disable external provider
```

You can also select the active memory provider via `aether plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.aether/config.yaml`:
...
When a memory provider is active, AETHER automatically:
...
**Setup Wizard:**
```bash
aether memory setup        # select "honcho" — runs the Honcho-specific post-setup
```
...
:::tip Migrating from `aether honcho`
If you previously used `aether honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per AETHER profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All AETHER profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per AETHER profile. Host key `aether` → default; `aether.<profile>` for others. |
...
```bash
aether profile create coder --clone
```

`--clone` creates a `aether.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every AETHER profile, creates host blocks for any profile without one, inherits settings from the default `aether` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `aether memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
aether memory setup    # select "openviking"
# Or manually:
aether config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.aether/.env
# Authenticated servers should use a user/admin API key:
echo "OPENVIKING_API_KEY=..." >> ~/.aether/.env
```
...
`OPENVIKING_AGENT` is AETHER' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
aether memory setup    # select "mem0" → "Platform"
# Or manually:
aether config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.aether/.env
```

**Setup (OSS):**
```bash
aether memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
aether memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
aether memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Config:** `$AETHER_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.aether/.env`.

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `platform` | `platform` (Mem0 Cloud) or `oss` (self-hosted) |
| `user_id` | `aether-user` | User identifier |
| `agent_id` | `aether` | Agent identifier |
...
**Switching modes:** Re-run `aether memory setup mem0 --mode <platform|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
aether memory setup    # select "hindsight"
# Or manually:
aether config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.aether/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p aether ui start`

**Config:** `$AETHER_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `aether` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between AETHER and the User` | Context label for retained memories |
...
**Setup:**
```bash
aether memory setup    # select "holographic"
# Or manually:
aether config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.aether-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$AETHER_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
aether memory setup    # select "retaindb"
# Or manually:
aether config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.aether/.env
```
...
# Then configure AETHER
aether memory setup    # select "byterover"
# Or manually:
aether config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$AETHER_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
aether memory setup    # select "supermemory"
# Or manually:
aether config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.aether/.env
```

**Config:** `$AETHER_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `aether` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `aether-{identity}` → `aether-coder`) to isolate memories per AETHER profile
...
| **Requires** | `pip install aether-memori` + `aether-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install aether-memori
aether-memori install
aether config set memory.provider memori
aether memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `aether-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$AETHER_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$AETHER_HOME/` so each profile has its own credentials
