...
description: "Customize AETHER's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

AETHER's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `AETHER_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who AETHER is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

AETHER now seeds a default `SOUL.md` automatically in:

```text
~/.aether/SOUL.md
```

More precisely, it uses the current instance's `AETHER_HOME`, so if you run AETHER with a custom home directory, it will use:
...
- AETHER creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- AETHER loads `SOUL.md` only from `AETHER_HOME`
- AETHER does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, AETHER falls back to a built-in default identity
...
If AETHER loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `AETHER_HOME`, the personality belongs to the AETHER instance itself.

That also makes it easier to teach users:
- "Edit `~/.aether/SOUL.md` to change AETHER' default personality."
...
- how AETHER should handle uncertainty, disagreement, or ambiguity
...
## What AETHER injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, AETHER falls back to a built-in default identity ("You are AETHER, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
AETHER ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain AETHER, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives AETHER its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.aether/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.aether/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-aether)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how AETHER speaks
- `display.skin` and `/skin` affect how AETHER looks in the terminal
