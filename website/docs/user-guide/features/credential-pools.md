...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, AETHER automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, AETHER auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
aether auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
aether auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
aether auth add anthropic --type oauth
...
anthropic (3 credentials):
  #1  aether_pkce          oauth   aether_pkce ←
...
Run `aether auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `aether auth` | Interactive pool management wizard |
| `aether auth list` | Show all pools and credentials |
| `aether auth list <provider>` | Show a specific provider's pool |
| `aether auth add <provider>` | Add a credential (prompts for type and key) |
| `aether auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `aether auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `aether auth remove <provider> <index>` | Remove credential by 1-based index |
| `aether auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `aether auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `aether model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via aether model:
aether auth list
# Shows:
...
aether auth add Together.ai --api-key sk-together-second-key
```
...
AETHER automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| AETHER PKCE OAuth | `~/.aether/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `aether auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `aether auth add`) are never auto-pruned.

Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. AETHER can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and AETHER-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
2. **`aether_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`aether_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`run_agent.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.aether/auth.json` under the `credential_pool` key:
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to AETHER' credential store, so its token remains persistable.
