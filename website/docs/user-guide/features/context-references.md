...
Type `@` followed by a reference to inject content directly into your message. AETHER expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- AETHER env: `$AETHER_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$AETHER_HOME/skills/.hub/`
