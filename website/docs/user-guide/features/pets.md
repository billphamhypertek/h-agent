...
AETHER can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<AETHER_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), AETHER renders the sprite at full fidelity when your
...
aether pets list
aether pets list cat

# Install a pet and make it active in one step
aether pets install boba --select
...
## `aether pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `aether pets list [query] [--limit N]` |
| List installed pets | `aether pets list --installed` |
| Install a pet | `aether pets install <slug> [--select] [--force]` |
...
| Preview/animate | `aether pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `aether pets off` |
| Remove an installed pet | `aether pets remove <slug>` |
| Diagnose setup | `aether pets doctor` |

`aether pets show` flags:
...
always-on-top desktop window. Out there it stays visible while AETHER is
...
Run `aether pets doctor` — it reports:
