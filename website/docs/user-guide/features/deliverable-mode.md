...
When AETHER runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.aether/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.aether/config.yaml` (switchable per session
via `/personality`).
...
If you use AETHER' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.aether/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. AETHER's deliverable
