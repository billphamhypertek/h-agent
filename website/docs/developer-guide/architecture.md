...
sidebar_position: 1
title: "Architecture"
description: "AETHER internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of AETHER internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ aether_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # AetherCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── aether_state.py           # SQLite session/state database with FTS5
├── aether_constants.py       # AETHER_HOME, profile-aware paths
...
│
├── aether_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `aether` subcommands (large file)
...
│   ├── skills_config.py      # aether skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # aether tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # aether gateway start/stop
│
...
```text
User input → AetherCLI.process_input()
...
Three discovery sources: `~/.aether/plugins/` (user), `.aether/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `aether plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-aether-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes AETHER as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`aether -p <name>`) gets its own AETHER_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
