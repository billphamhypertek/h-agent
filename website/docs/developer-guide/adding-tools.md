...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to AETHER — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in AETHER tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying AETHER core, use the plugin route instead:

- [Plugins](/user-guide/features/plugins)
- [Build a AETHER Plugin](/guides/build-a-aether-plugin)
...
2. **`toolsets.py`** — add tool name to `_AETHER_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `aether_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `aether_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `aether chat -q "Use the weather tool for London"`
