...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept AETHER already has applies. The plugin never sees a
...
[`aether-example-plugins`](https://github.com/NousResearch/aether-example-plugins/tree/main/plugin-llm-example)
repo (companion repo for reference plugins — not bundled with
aether-agent itself). For the async surface (`acomplete()` /
`acomplete_structured()` with `asyncio.gather()`), see
[`plugin-llm-async-example`](https://github.com/NousResearch/aether-example-plugins/tree/main/plugin-llm-async-example)
in the same repo.
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — AETHER provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different AETHER provider
        # (must be one AETHER already knows about — same names as
        # `aether model` and config.yaml model.provider).
        ...
        # plugin sends — AETHER does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.aether/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through AETHER' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing AETHER subsystem:
...
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/NousResearch/aether-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/NousResearch/aether-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
