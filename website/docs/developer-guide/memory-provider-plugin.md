...
description: "How to build a memory provider plugin for AETHER"
---

# Building a Memory Provider Plugin

Memory provider plugins give AETHER persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `aether plugins`.
:::
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          aether_home (str): Active AETHER_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `aether memory setup` | **Yes** |
| `save_config(values, aether_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
`get_config_schema()` returns a list of field descriptors used by `aether memory setup`:
...
Every field in `get_config_schema()` is prompted during `aether memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$AETHER_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::

## Save Config

```python
def save_config(self, values: dict, aether_home: str) -> None:
    """Write non-secret config to your native location."""
    import json
    from pathlib import Path
    config_path = Path(aether_home) / "my-provider.json"
...
the `messages` parameter; AETHER will continue calling them with the legacy
signature.
...
All storage paths **must** use the `aether_home` kwarg from `initialize()`, not hardcoded `~/.aether`:

```python
# CORRECT — profile-scoped
from aether_constants import get_aether_home
data_dir = get_aether_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.aether/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `aether my-provider status`, `aether my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `aether <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `aether --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: aether my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the aether my-provider argparse tree.
