...
description: "How AETHER stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `aether_cli/cron.py` | CLI `aether cron` subcommands |
...
Jobs are stored in `~/.aether/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `aether cron` commands are run or during active CLI sessions.
...
```python
# ~/.aether/scripts/check_competitors.py
...
2. **Environment variable** — `AETHER_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.aether/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `aether cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `aether cron` CLI provides direct job management:

```bash
aether cron list                    # Show all jobs
aether cron create                  # Interactive job creation (alias: add)
aether cron edit <job_id>           # Edit job configuration
aether cron pause <job_id>          # Pause a running job
aether cron resume <job_id>         # Resume a paused job
aether cron run <job_id>            # Trigger immediate execution
aether cron remove <job_id>         # Delete a job
```
