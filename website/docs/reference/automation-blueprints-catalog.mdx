...
of fields, and AETHER schedules it as a cron job — no cron syntax required.
...
- **CLI, TUI, and messengers** — type `/blueprint <name>` (e.g.
  `/blueprint morning-brief`) and AETHER asks you for what it needs, one
...
A blueprint is just a skill with a `metadata.aether.blueprint` block in its
