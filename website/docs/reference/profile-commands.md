...
This page covers all commands related to [AETHER profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `aether profile` without a subcommand shows help.
...
```bash
aether profile use <name>
```

Sets `<name>` as the active profile. All subsequent `aether` commands (without `-p`) will use this profile.
...
aether profile use default
```

## `aether profile create`

```bash
aether profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `aether profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `aether update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.aether`), use `aether skills opt-out` / `aether skills opt-in`. |
...
aether profile create work2 --clone-from work

# Clone everything from a specific profile
aether profile create work2-backup --clone-from work --clone-all
```

## `aether profile describe`

```bash
aether profile describe [<name>] [options]
```
...
aether profile describe researcher

# Set it explicitly
aether profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
aether profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
aether profile describe --all --auto
```

## `aether profile delete`

```bash
aether profile delete <name> [options]
```
...
```bash
aether profile delete mybot
aether profile delete mybot --yes
```
...
```bash
aether profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's AETHER home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ aether profile show work
Profile: work
Path:    ~/.aether/profiles/work
...
## `aether profile alias`

```bash
aether profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your AETHER installation.
...
```bash
aether profile alias work
# Creates/updates ~/.local/bin/work

aether profile alias work --name mywork
# Creates ~/.local/bin/mywork

aether profile alias work --remove
# Removes the wrapper script
```

## `aether profile rename`

```bash
aether profile rename <old-name> <new-name>
```
...
```bash
aether profile rename mybot assistant
# ~/.aether/profiles/mybot → ~/.aether/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `aether profile export`

```bash
aether profile export <name> [options]
```
...
```bash
aether profile export work
# Creates work.tar.gz in the current directory

aether profile export work -o ./work-2026-03-29.tar.gz
```

## `aether profile import`

```bash
aether profile import <archive> [options]
```
...
```bash
aether profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

aether profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`aether profile export` / `import` are still the right commands for
...
### `aether profile install`

```bash
aether profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `aether -p telemetry`). |
...
aether profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
aether profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
aether profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
aether profile install ./telemetry/
```

### `aether profile update`

```bash
aether profile update <name> [--force-config] [--yes]
```
...
```bash
aether profile info <name>
```

Prints the profile's distribution manifest — name, version, required
AETHER version, author, env var requirements, the source URL/path, and
...
`aether profile list` also shows the distribution name and version in a
`Distribution` column, and `aether profile show <name>` / `delete <name>`
...
aether profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
aether profile install https://github.com/your-org/internal-assistant.git
```
...
`aether_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
AETHER version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host AETHER can clone from.
3. Tell recipients to run `aether profile install <your-repo-url>`.
...
## `aether -p` / `aether --profile`

```bash
aether -p <name> <command> [options]
aether --profile <name> <command> [options]
```

Global flag to run any AETHER command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
aether -p work chat -q "Check the server status"
aether --profile dev gateway start
aether -p personal skills list
aether -p work config edit
```
...
- `aether profile <TAB>` — subcommands (list, use, create, etc.)
- `aether profile use <TAB>` — profile names
- `aether -p <TAB>` — profile names
