---
sidebar_position: 8
title: "Use Voice Mode with AETHER"
description: "A practical guide to setting up and using AETHER voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with AETHER
...
- you want AETHER sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in AETHER.
...
## Step 1: make sure normal AETHER works first
...
AETHER supports both local and cloud speech stacks.
...
### If you use `aether setup`

If you choose NeuTTS in the setup wizard, AETHER checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. AETHER transcribes and responds
...
- asking AETHER to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full AETHER loop.

## Tuning CLI behavior

### Silence threshold

If AETHER starts/stops too aggressively, tune:
...
AETHER stays a normal chat bot, but can speak replies.
...
- you want AETHER to function like a portable research or ops assistant
...
AETHER joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- AETHER detects speech boundaries
- transcripts are posted in the associated text channel
- AETHER responds in text and audio
...
1. get text AETHER working
2. install `aether-agent[voice]`
