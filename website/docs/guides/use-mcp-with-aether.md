---
sidebar_position: 6
title: "Use MCP with AETHER"
description: "A practical guide to connecting MCP servers to AETHER, filtering their tools, and using them safely in real workflows"
---

# Use MCP with AETHER

This guide shows how to actually use MCP with AETHER in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native AETHER tool
- you want AETHER to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect AETHER to internal APIs, databases, or company systems without modifying AETHER core

Do not use MCP when:
- a built-in AETHER tool already solves the job well
...
- AETHER remains the agent
- MCP servers contribute tools
- AETHER discovers those tools at startup or reload time
...
If you installed AETHER with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- AETHER banner/status should show MCP integration when configured
- ask AETHER what tools it has available
...
## WSL2: bridge AETHER in WSL to Windows Chrome

This is the practical setup when:

- AETHER runs inside WSL2
...
In this setup, AETHER does **not** connect to Chrome directly. Instead:

- AETHER runs in WSL
- AETHER starts a local stdio MCP server
...
```text
AETHER (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- AETHER stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on AETHER core browser transport
...
```bash
aether mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
aether mcp test chrome-devtools-win
```

Then start a fresh AETHER session or run:
...
Once loaded, AETHER can use the MCP-prefixed browser tools directly. For example:
...
If AETHER runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start AETHER from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start AETHER from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in AETHER:
...
2. AETHER-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So AETHER will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want AETHER to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want AETHER to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. AETHER remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
aether mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
aether mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `aether mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want AETHER to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize AETHER to merge, publish, deploy, or spawn runtimes.
...
Now AETHER can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing AETHER core.
...
Because AETHER now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
