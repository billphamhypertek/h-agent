---
sidebar_position: 7
title: "Use SOUL.md with AETHER"
description: "How to use SOUL.md to shape AETHER's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with AETHER

`SOUL.md` is the **primary identity** for your AETHER instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want AETHER to feel like the same assistant every time you talk to it — or if you want to replace the AETHER persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm AETHER should be
- what AETHER should avoid stylistically
- how AETHER should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who AETHER is and how AETHER speaks
...
AETHER now uses only the global SOUL file for the current instance:

```text
~/.aether/SOUL.md
```

If you run AETHER with a custom home directory, it becomes:
...
AETHER automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, AETHER does not overwrite it
- if the file exists but is empty, AETHER adds nothing from it to the prompt

## How AETHER uses it

When AETHER starts a session, it reads `SOUL.md` from `AETHER_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how AETHER feels.
...
AETHER already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How AETHER should sound.

# Avoid
What AETHER should not do.

# Defaults
How AETHER should behave when ambiguity appears.
```
...
Then restart AETHER or start a new session.
...
### I edited SOUL.md but AETHER still sounds the same

Check:
- you edited `~/.aether/SOUL.md` or `$AETHER_HOME/SOUL.md`
...
### AETHER is ignoring parts of my SOUL.md
