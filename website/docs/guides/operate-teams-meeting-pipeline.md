...
```bash
aether teams-pipeline validate
```
...
```bash
aether teams-pipeline token-health
aether teams-pipeline token-health --force-refresh
```
...
```bash
aether teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
aether teams-pipeline maintain-subscriptions
aether teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: AETHER cron (recommended if you already run the AETHER gateway)

AETHER ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.aether/scripts/`. First create the script:

```bash
mkdir -p ~/.aether/scripts
cat > ~/.aether/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec aether teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.aether/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
aether cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/aether-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=AETHER Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=aether
EnvironmentFile=/etc/aether/env
ExecStart=/usr/local/bin/aether teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/aether-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run AETHER Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now aether-teams-pipeline-maintain.timer
systemctl list-timers aether-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/aether teams-pipeline maintain-subscriptions >> /var/log/aether/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.aether/.env` at the top of a wrapper script that crontab calls.
...
```bash
aether teams-pipeline subscriptions   # should show expirationDateTime advanced
aether teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
aether teams-pipeline list
aether teams-pipeline list --status failed
aether teams-pipeline show <job-id>
```

### Replay a stored job

```bash
aether teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
aether teams-pipeline fetch --meeting-id <meeting-id>
aether teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `aether teams-pipeline maintain-subscriptions --dry-run`
- inspect `aether teams-pipeline list --status failed`
...
- run `aether teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `aether teams-pipeline run`
...
- [ ] `aether teams-pipeline validate` returns an OK snapshot
- [ ] `aether teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (AETHER cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
