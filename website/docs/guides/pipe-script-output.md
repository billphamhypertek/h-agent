...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `aether send`."
---

# Pipe Script Output to Messaging Platforms

`aether send` is a small, scriptable CLI that pushes a message to any
messaging platform AETHER is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | aether send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `aether
...
aether send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
aether send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
aether send --list

# Filter by platform
aether send --list telegram
```
...
Any platform AETHER ships adapters for works as a target:
...
`aether send` resolves the message body in this order:

1. **Positional argument** — `aether send --to telegram "hi"`
2. **`--file PATH`** — `aether send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | aether send --to telegram`

When stdin is a TTY (no pipe), AETHER does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  aether send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `aether send` reuses your AETHER config, the same script works on
any host where AETHER is installed — no need to export bot tokens into
...
`aether send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  aether send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | aether send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.aether/bin/aether send \
...
```bash
./train.py --epochs 200 && \
  aether send --to telegram "training done" || \
  aether send --to telegram "training failed (exit $?)"
```
...
aether send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(aether send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `aether send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `aether send` calls
the platform's REST endpoint directly using credentials from
`~/.aether/.env` and `~/.aether/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `aether gateway start` and retry.
...
# Machine-readable
aether send --list --json
```

The listing is built from `~/.aether/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`aether gateway
...
| Approach | Multi-platform | Reuses AETHER creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `aether send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`aether send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `aether send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `aether send` shares with cron delivery.
