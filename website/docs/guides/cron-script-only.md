...
AETHER calls this **no-agent mode**. It's the cron system minus the LLM.
...
- **Bash or Python.** `.sh` / `.bash` files run under `/bin/bash`; any other extension runs under the current Python interpreter. Anything in `~/.aether/scripts/` is accepted.
...
The real win of no-agent mode is that the agent itself can set up the watchdog for you — no editor, no shell, no remembering CLI flags. You describe what you want, AETHER writes the script, schedules it, and tells you when it'll fire.

### Example transcript

> **You:** ping me on telegram if RAM is over 85% every 5 minutes
>
> **AETHER:** *(writes `~/.aether/scripts/memory-watchdog.sh`, then calls `cronjob(...)` with `no_agent=true`)*
>
...
write_file(
    path="~/.aether/scripts/memory-watchdog.sh",
...
>
> **AETHER:** *(calls `cronjob(action='pause', job_id='abc123')`)*
>
> Paused. Resume with "turn it back on" or via `aether cron resume abc123`.

> **You:** change it to every 15 minutes
>
> **AETHER:** *(calls `cronjob(action='update', job_id='abc123', schedule='every 15m')`)*
...
```bash
# 1. Write your script
cat > ~/.aether/scripts/memory-watchdog.sh <<'EOF'
#!/usr/bin/env bash
...
EOF
chmod +x ~/.aether/scripts/memory-watchdog.sh
...
# 3. Verify
aether cron list
aether cron run <job_id>    # fire it once to test
```
...
Scripts must live in `~/.aether/scripts/`. This is enforced at both job-creation time and run time — absolute paths, `~/` expansion, and path-traversal patterns (`../`) are rejected. The same directory is shared with the pre-check script gate used by LLM jobs.
...
```bash
aether cron create "every 5m"        # interval
aether cron create "every 2h"
aether cron create "0 9 * * *"       # standard cron: 9am daily
aether cron create "30m"             # one-shot: run once in 30 minutes
```
...
--deliver local                          # just save to ~/.aether/cron/output/
```

No running gateway is required at script-run time for bot-token platforms (Telegram, Discord, Slack, Signal, SMS, WhatsApp) — the tool calls each platform's REST endpoint directly using the credentials already in `~/.aether/.env` / `~/.aether/config.yaml`.

## Editing and Lifecycle

```bash
aether cron list                                    # see all jobs
aether cron pause <job_id>                          # stop firing, keep definition
aether cron resume <job_id>
aether cron edit <job_id> --schedule "every 10m"    # adjust cadence
aether cron edit <job_id> --agent                   # flip to LLM mode
aether cron edit <job_id> --no-agent --script …     # flip back
aether cron remove <job_id>                         # delete it
```
...
```bash
cat > ~/.aether/scripts/disk-alert.sh <<'EOF'
#!/usr/bin/env bash
...
'
EOF
chmod +x ~/.aether/scripts/disk-alert.sh

aether cron create "*/15 * * * *" \
  --no-agent \
...
|----------|-----------|-------------|
| `cronjob --no-agent` (this page) | Your script on AETHER' schedule | Recurring watchdogs / alerts / metrics that don't need reasoning |
| `cronjob` (default, LLM) | Agent with optional pre-check script | When the message content requires reasoning over data |
| OS cron + `curl` to a [webhook subscription](/user-guide/messaging/webhooks) | Your script on the OS schedule | When AETHER might be unhealthy (the thing you're monitoring) |

For critical system-health watchdogs that must fire *even when the gateway is down*, use OS-level cron with a plain `curl` to a AETHER webhook subscription (or any external alerting endpoint) — those run as independent OS processes and don't depend on AETHER being up. The in-gateway scheduler is the right choice when the thing being monitored is external.
