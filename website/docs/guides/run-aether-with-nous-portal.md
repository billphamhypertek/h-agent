...
3. Sets `model.provider: nous` in `~/.aether/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where AETHER is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
aether setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: manual paste (for Cloud Shell, Codespaces, EC2 Instance Connect)
aether auth add nous --type oauth --manual-paste
# Then re-run `aether setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "AETHER release notes" and summarize the top 3 hits.
```

You should see AETHER call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`aether setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
These rows appear in `aether tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, AETHER runs the Portal login inline (without changing your inference provider or your other tools).
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and AETHER will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
```bash
aether cron create "every day at 9am" \
...
If you use [AETHER profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.aether/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `aether portal info` shows "not logged in" after `aether setup --portal`
...
```bash
aether config set model.provider nous
```
...
Re-verify with `aether portal info`.
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so AETHER doesn't replay it endlessly. Just log in again:

```bash
aether auth add nous
```
...
If a model is genuinely unavailable, [open an issue](https://github.com/billphamhypertek/h-agent/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`aether portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple AETHER profiles where you're using the wrong one (check `aether profile list`)

### Want to revoke and start clean

```bash
aether auth logout nous       # wipes the local refresh token
...
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-AETHER tools
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple AETHER configurations
