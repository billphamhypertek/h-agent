...
description: "Sign in with your SuperGrok or X Premium+ subscription to use Grok models in AETHER — no API key required"
---

# xAI Grok OAuth (SuperGrok / X Premium+)

AETHER supports xAI Grok through a browser-based OAuth login flow against [accounts.x.ai](https://accounts.x.ai), using either a **SuperGrok subscription** ([grok.com](https://x.ai/grok)) or an **X Premium+ subscription** (linked X account). No `XAI_API_KEY` is required — log in once and AETHER automatically refreshes your session in the background.
...
The same OAuth bearer token is also reused by every direct-to-xAI surface in AETHER — TTS, image generation, video generation, and transcription — so a single login covers all four.
...
# → AETHER opens your browser to accounts.x.ai
...
After the first login, credentials are stored under `~/.aether/auth.json` and refreshed automatically before they expire.
...
```bash
aether auth add xai-oauth
```

### Remote / headless sessions

On servers, containers, or SSH sessions where no browser is available, AETHER detects the remote environment and prints the authorization URL instead of opening a browser.
...
aether auth add xai-oauth --no-browser
...
If you don't have a regular SSH client (e.g. you're running AETHER inside GCP Cloud Shell, GitHub Codespaces, AWS EC2 Instance Connect, Gitpod, or another browser-based console), the `ssh -L` recipe above isn't available. Use `--manual-paste` instead — AETHER skips the loopback listener and lets you paste the failed callback URL straight from your browser:

```bash
aether auth add xai-oauth --manual-paste
# Or via the model picker:
aether model --manual-paste
```
...
If the consent page renders the authorization code directly on the page (xAI's current behavior on browser-based consoles) instead of redirecting to your `127.0.0.1:56121/callback`, paste **just the bare code value** at the `Callback URL:` prompt — AETHER accepts the full URL, a bare `?code=...&state=...` query fragment, or a bare code interchangeably.

## How the Login Works

1. AETHER opens your browser to `accounts.x.ai`.
2. You sign in (or confirm your existing session) and approve access.
3. xAI redirects back to AETHER and the tokens are saved to `~/.aether/auth.json`.
4. From then on, AETHER refreshes the access token in the background — you stay signed in until you `aether auth logout xai-oauth` or revoke access from your xAI account settings.
...
```bash
aether config set model.default grok-build-0.1
aether config set model.provider xai-oauth
```

## Configuration Reference

After login, `~/.aether/config.yaml` will contain:
...
```bash
aether --provider xai-oauth        # canonical
aether --provider grok-oauth       # alias
aether --provider x-ai-oauth       # alias
aether --provider xai-grok-oauth   # alias
```
...
The `video_gen` toolset is disabled by default. Enable it in `aether tools` → `🎬 Video Generation` (press space) before the agent can call `video_generate`. Otherwise the agent may fall back to the bundled ComfyUI skill, which is also tagged for video generation.
:::

:::note X search auto-enables when xAI credentials are present
The `x_search` toolset auto-enables whenever xAI credentials (a SuperGrok / X Premium+ OAuth token or `XAI_API_KEY`) are configured. Disable explicitly via `aether tools` → `🐦 X (Twitter) Search` (press space) if you don't want this. The tool routes through xAI's built-in `x_search` Responses API — it works with **either** your SuperGrok / X Premium+ OAuth login or a paid `XAI_API_KEY`, and prefers OAuth when both are configured (uses your subscription quota instead of API spend). The tool schema is hidden from the model when no xAI credentials are configured, regardless of whether the toolset is enabled.
:::
...
To select xAI as the active provider, set `model.provider: xai-oauth` in `config.yaml` (use `aether setup` for the guided flow) or pass `--provider xai-oauth` for a single invocation.

## Troubleshooting

### Token expired — not re-logging in automatically

AETHER refreshes the token before each session and again reactively on a 401. If refresh fails with `invalid_grant` (the refresh token was revoked, or the account was rotated), AETHER surfaces a typed re-auth message instead of crashing.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), AETHER marks the refresh token as dead and quarantines it locally — subsequent calls skip the doomed refresh attempt instead of replaying the same 401 over and over. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `aether auth add xai-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The loopback listener has a finite expiry window (default 180 s). If you don't approve the login in time, AETHER raises a timeout error.

**Fix:** re-run `aether auth add xai-oauth` (or `aether model`). The flow starts fresh.

### State mismatch (possible CSRF)

AETHER detected that the `state` value returned by the authorization server doesn't match what it sent.
...
On SSH or container sessions AETHER prints the authorization URL instead of opening a browser. The loopback callback listener still binds `127.0.0.1:56121` on the remote host — your laptop's browser can't reach it without an SSH local-forward:
...
This is **not** a stale-token problem — re-running `aether model` won't change it. xAI's backend has been seen to restrict OAuth API access to specific SuperGrok tiers despite the in-app subscription being active (issue [#26847](https://github.com/NousResearch/hermes-agent/issues/26847)).

**Fix:** set `XAI_API_KEY` and switch to the API-key path:

```bash
export XAI_API_KEY=xai-...
aether config set model.provider xai
```
...
**Fix:** run `aether model` and pick the xAI Grok OAuth provider, or run `aether auth add xai-oauth`.
...
```bash
aether auth logout xai-oauth
```

This clears both the singleton OAuth entry in `auth.json` and any credential-pool rows for `xai-oauth`. Use `aether auth remove xai-oauth <index|id|label>` if you only want to drop a single pool entry (run `aether auth list xai-oauth` to see them).

## See Also

- [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md) — required reading if AETHER is on a different machine than your browser
