...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `aether setup --portal`. See [Nous Portal](/integrations/nous-portal).
:::
...
2. **AETHER spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.aether/cron/output/` and you can read them anytime.
:::
...
AETHER will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by AETHER
```
...
Before creating cron jobs, ensure AETHER has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell AETHER what you want:
...
AETHER will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell AETHER to delegate each topic to a sub-agent:
...
AETHER will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
